...
(async()=>{
    ...
    async function checkWebLaunchAd() {
        ...
        try {
            ...
            console.log(`cachedCreatives:`, cachedCreatives);
    ...
    function getCachedLaunchSchedule() {
        ...
        try {
            ...
            return (parsed && Array.isArray(parsed.creatives)) ? parsed.creatives : null;
        } catch (err) {
            console.log(err);
            return null;
        }
    ...
    function saveLaunchSchedule(creatives) {
        if (!Array.isArray(creatives) || typeof localStorage === 'undefined') {return;}
        try {
    ...
    function refreshLaunchSchedule(url) {
        if (!url) {return;}
        fetch(url)
            .then(response => {
                if (!response.ok) {throw new Error('Failed to refresh launch schedule');}
                return response.json();
            })
            .then(creatives => {
                saveLaunchSchedule(creatives);
            })
            .catch(err => {
                console.log(err);
            });
    ...
    function pickoutCreative(creatives) {
        ...
        let sortedCreatives = creatives
            .filter(x=>{
                ...
                const dates = x?.dates ?? '';
                const isSupportedFile = /\.(jpg|jpeg|png|gif|mp4)$/gi.test(fileName);
        ...
        if (sortedCreatives.length === 0) {return null;} 
    ...
    function renderCreative(creative) {
        ...
        console.log(`continue rendering...`);
        ...
        closeButton.onclick = () => closeLaunchScreenSafely(true);
        ...
        setTimeout(() => closeLaunchScreenSafely(true), displaySeconds * 1000);
    }
