/* jshint ignore:start */

function localize(status, fallback, dict = {}) {
  if (!status) {
    return;
  }

  // Assume preferredLanguage is correctly set through application logic
  let language = preferredLanguage || 'en';

  if (language === 'Chinese') {
    language = 'zh';
  }

  // console.log(status, language);

  // Normalize language code by removing regional codes
  const languagePrefix = language.replace(/\-.*$/g, '');
  const statusKey = status.toLowerCase();
  const statusTranslations = statusDict[statusKey];

  if (statusTranslations) {
    let translation = statusTranslations[language] || statusTranslations[languagePrefix];
    // Fallback to English if no translation is found
    if (translation === undefined) {
      translation = statusTranslations.en;
    }
    translation = translation || translation === '' ? translation : status;
    for (const key of Object.keys(dict)) {
      const value = dict[key];
      const regexKey = new RegExp(`\\\[${key}\\\]`, 'g'); // Escape the square brackets
      translation = translation.replace(regexKey, value);
    }
    return translation;
  }

  // Use fallback if provided and valid, otherwise use the original status
  return (typeof fallback === 'string' && fallback !== '') ? fallback : status;
}


const inductionData = {
  name: "user_induction",
  intro: "induction_intro",
  questions: [
    {
      text: "select_language",
      type: "single_choice",
      options: window?.languageOptions ?? [],
      key: 'Language',
      variable: 'preferredLanguage',
      disable_input: true
    },
    {
      text: "select_follows",
      type: "multiple_choices",
      options: window?.recommendedAnnotations ?? [],
      disable_input: true,
      isLayoutGrid: true
    },
    {
      text: "Topic collections by industry",
      type: "multiple_choices",
      options: window?.recommendedAnnotationsByIndustry ?? [],
      disable_input: true,
      follow_all: true,
      isLayoutGrid: true
    },
    {
      text: "input_custom_interests",
      type: "custom_input",
      detail: "input_custom_interests_detail",
      disable_input: true
    }
  ]
};

window.languageOptions = [
  { value: 'zh-CN', name: '简体中文' },
  { value: 'zh-TW', name: '台灣正體' },
  { value: 'zh-HK', name: '香港繁體' },
  { value: 'en', name: 'English' },
  { value: 'es', name: 'Español' },
  { value: 'fr', name: 'Français' },
  { value: 'de', name: 'Deutsch' },
  { value: 'ja', name: '日本語' },
  { value: 'ko', name: '한국어' },
  { value: 'pt', name: 'Português' },
  { value: 'it', name: 'Italiano' },        
  { value: 'ru', name: 'Русский' }
];




const randomPromptDict = {
    greeting: {
      'en': [
        'Hello, how can I help you?',
        'Hi, how may I help you?',
        'Hello, how may I assist you?',
        'Greetings, how can I be of service?',
        'Hey there, how can I help you today?'
      ],
      'es': [
        'Hola, ¿en qué puedo ayudarte?',
        '¡Hola! ¿Cómo puedo ayudarte?',
        'Bienvenido, ¿en qué puedo ayudarte hoy?',
        '¿En qué puedo asistirte?',
        'Buenos días, ¿cómo puedo ayudarte?'
      ],
      'fr': [
        'Bonjour, comment puis-je vous aider ?',
        'Salut, comment puis-je vous aider ?',
        'Bonjour, en quoi puis-je vous aider ?',
        'Bonjour, que puis-je faire pour vous ?',
        'Bonjour, comment je peux vous aider aujourd\'hui ?'
      ],
      'de': [
        'Hallo, wie kann ich Ihnen helfen?',
        'Guten Tag, wie kann ich Ihnen helfen?',
        'Hallo, wie kann ich Ihnen behilflich sein?',
        'Guten Morgen, wie kann ich Ihnen helfen?',
        'Hi, was kann ich für Sie tun?'
      ],
      'ja': [
        'こんにちは、何かお手伝いできますか？',
        'はじめまして、何かご質問はありますか？',
        'お問い合わせありがとうございます。どういったことでお困りですか？',
        'こんにちは。何かお探しですか？',
        'こんにちは。ご相談はありますか？'
      ],
      'ko': [
        '안녕하세요, 무엇을 도와드릴까요?',
        '안녕하세요, 무엇이 문제인가요?',
        '안녕하세요, 무엇을 도와드릴까요?',
        '안녕하세요, 어떤 문제가 있으신가요?',
        '안녕하세요, 도움이 필요하시면 언제든지 말씀해주세요.'
      ],
      'pt': [
        'Olá, em que posso ajudar?',
        'Oi, como posso ajudar?',
        'Boa tarde, como posso ajudá-lo?',
        'Posso ajudar em alguma coisa?',
        'Olá, posso ajudar em algo?'
      ],
      'it': [
        'Ciao, come posso aiutarti?',
        'Salve, di cosa hai bisogno?',
        'Buongiorno, in che modo posso aiutarti?',
        'Ciao, posso aiutarti in qualche modo?',
        'Salve, di che hai bisogno?'
      ],
      'zh-TW': ['你好，有什麼可以幫助你的嗎？', '您好，需要我幫忙嗎？', '哈囉，有什麼我可以幫忙的嗎？', '您好，我能為您做些什麼？', '歡迎詢問，我有什麼可以幫忙的嗎？'],
      'zh-HK': ['你好，有什麼可以幫助你的嗎？', '您好，需要我幫忙嗎？', '哈囉，有什麼我可以幫忙的嗎？', '您好，我能為您做些什麼？', '歡迎詢問，我有什麼可以幫忙的嗎？'],
      'zh': [
        '你好，有什么需要帮助的吗？',
        '您好，需要我帮忙吗？',
        '你好，请问有什么可以帮您的？',
        '您好，我能为您提供什么服务？',
        '嗨，有什么我可以帮您的吗？'
      ],
      'ru': [
        'Здравствуйте, чем я могу вам помочь?',
        'Привет, чем я могу вам помочь?',
        'Здравствуйте, как я могу вам помочь?',
        'Приветствую, чем я могу быть полезен?',
        'Здравствуйте, чем я могу вам помочь сегодня?'
      ]
    },
    ending: {
      'en': [
      'What else can I do for you?',
      'Is there anything else I can assist you with?',
      'Do you require any further assistance from me?',
      'Are there any other ways I can be of service to you?',
      'Would you like me to do anything else for you?'
      ],
      'es': [
      '¿Qué más puedo hacer por ti?',
      '¿Hay algo más en lo que pueda ayudarte?',
      '¿Necesitas alguna otra asistencia de mi parte?',
      '¿Hay alguna otra manera en la que pueda ser de servicio para ti?',
      '¿Te gustaría que hiciera algo más por ti?'
      ],
      'fr': [
      `Que puis-je faire d'autre pour vous ?`,
      'Est-ce que je peux vous aider avec autre chose ?',
      `Avez-vous besoin d'une assistance supplémentaire de ma part ?`,
      `Y a-t-il d'autres moyens par lesquels je peux être utile pour vous ?`,
      'Voulez-vous que je fasse autre chose pour vous ?'
      ],
      'de': [
      'Was kann ich noch für Sie tun?',
      'Kann ich Ihnen noch in anderer Weise behilflich sein?',
      'Benötigen Sie weitere Unterstützung von mir?',
      'Gibt es noch andere Möglichkeiten, wie ich Ihnen dienen kann?',
      'Möchten Sie, dass ich noch etwas anderes für Sie tue?'
      ],
      'ja': [
      '他に何かご用件はありますか？',
      '他に何かお手伝いできることはありますか？',
      '私に他に何かお力になることはありますか？',
      '他にも何かお役に立てる方法はありますか？',
      '他に何かしてほしいことはありますか？'
      ],
      'ko': [
      '더 도와 드릴 게 있나요?',
      '다른 무언가 도와 드릴 수 있나요?',
      '제가 더 도움을 드릴 수 있는 것이 있나요?',
      '다른 방법으로도 봉사할 수 있는 게 있나요?',
      '제가 더 해 줄 일이 있나요?'
      ],
      'pt': [
      'O que mais posso fazer por você?',
      'Existe algo mais em que posso ajudá-lo?',
      'Você precisa de mais alguma assistência minha?',
      'Existem outras maneiras pelas quais posso ser útil para você?',
      'Você gostaria que eu fizesse algo mais por você?'
      ],
      'it': [
      'Cosa altro posso fare per te?',
      `C'è qualcos'altro in cui posso aiutarti?`,
      'Hai bisogno di ulteriore assistenza da parte mia?',
      'Ci sono altri modi in cui posso esserti utile?',
      'Desideri che io faccia altro per te?'
      ],
      'zh-TW': [
      '還有什麼我可以為您做的嗎？',
      '還有什麼我可以幫您的嗎？',
      '您需要我提供什麼其他的協助嗎？',
      '還有其他什麼方式我可以為您服務嗎？',
      '您還需要我為您做些什麼嗎？'
      ],
      'zh-HK': [
      '我還可以為您做些什麼嗎？',
      '還有什麼我可以協助您的嗎？',
      '您需要我提供其他的幫助嗎？',
      '還有其他方法可以令我為您服務嗎？',
      '您還需要我為您做其他事嗎？'
      ],
      'zh': [
      '还有什么我可以为您做的吗？',
      '还有什么我可以帮您的吗？',
      '您需要我提供什么其他的协助吗？',
      '还有其他什么方式我可以为您服务吗？',
      '您还需要我为您做些什么吗？'
      ],
      'ru': [
      'Чем еще я могу Вам помочь?',
      'Нужна ли Вам еще какая-либо помощь с моей стороны?',
      'Требуется ли Вам дополнительная помощь от меня?',
      'Есть ли другие способы, которыми я могу быть Вам полезен?',
      'Хотели бы Вы, чтобы я что-то еще сделал для Вас?'
      ]
    }
  };
  
let statusDictOriginal = {
  induction_intro: {
    zh: '欢迎访问FT！我是您的智能助理，将为您提供定制化的内容推荐。',
    en: 'Welcome to FT! I am your intelligent assistant, here to provide you with customized content recommendations.',
    es: '¡Bienvenido a FT! Soy tu asistente inteligente, aquí para ofrecerte recomendaciones de contenido personalizadas.',
    fr: 'Bienvenue chez FT! Je suis votre assistant intelligent, ici pour vous fournir des recommandations de contenu personnalisées.',
    de: 'Willkommen bei FT! Ich bin Ihr intelligenter Assistent, hier um Ihnen maßgeschneiderte Inhaltempfehlungen zu bieten.',
    ja: 'FTへようこそ！私はあなたのインテリジェントアシスタントで、カスタマイズされたコンテンツの推薦を提供します。',
    ko: 'FT에 오신 것을 환영합니다! 저는 여러분의 지능형 조수로, 맞춤형 콘텐츠 추천을 제공하겠습니다.',
    pt: 'Bem-vindo à FT! Sou seu assistente inteligente, aqui para fornecer recomendações de conteúdo personalizadas.',
    it: 'Benvenuto in FT! Sono il tuo assistente intelligente, qui per fornirti raccomandazioni di contenuto personalizzate.',
    'zh-TW': '歡迎訪問FT！我是您的智能助理，將為您提供定制化的內容推薦。',
    'zh-HK': '歡迎訪問FT！我是您的智能助理，將為您提供定制化的內容推薦。',
    ru: 'Добро пожаловать в FT! Я ваш умный помощник, здесь чтобы предложить вам персонализированные рекомендации по контенту.'
  },
  select_language: {
    zh: '请首先选择您喜欢的阅读语言。',
    en: 'Please first select your preferred reading language.',
    es: 'Por favor, seleccione primero su idioma de lectura preferido.',
    fr: `Veuillez d'abord sélectionner votre langue de lecture préférée.`,
    de: `Bitte wählen Sie zunächst Ihre bevorzugte Lesesprache aus.`,
    ja: 'まず、読みたい言語を選択してください。',
    ko: '먼저 선호하는 독서 언어를 선택해 주세요.',
    pt: 'Por favor, selecione primeiro o seu idioma de leitura preferido.',
    it: 'Si prega di selezionare prima la lingua di lettura preferita.',
    'zh-TW': '請首先選擇您喜歡的閱讀語言。',
    'zh-HK': '請首先選擇您喜歡的閱讀語言。',
    ru: 'Пожалуйста, сначала выберите предпочитаемый язык чтения.'
  },
  select_follows: {
    zh: '以下是FT的主要内容分类，您可以选择自己感兴趣的领域。',
    en: 'Here are the main content categories of FT. You can select the areas that interest you.',
    es: 'Aquí están las principales categorías de contenido de FT. Puede seleccionar las áreas que le interesan.',
    fr: 'Voici les principales catégories de contenu de FT. Vous pouvez sélectionner les domaines qui vous intéressent.',
    de: 'Hier sind die Hauptinhaltskategorien von FT. Sie können die Bereiche auswählen, die Sie interessieren.',
    ja: 'こちらがFTの主要なコンテンツカテゴリです。興味のある分野を選択できます。',
    ko: '다음은 FT의 주요 콘텐츠 카테고리입니다. 관심 있는 분야를 선택할 수 있습니다.',
    pt: 'Aqui estão as principais categorias de conteúdo da FT. Você pode selecionar as áreas que lhe interessam.',
    it: 'Ecco le principali categorie di contenuto di FT. Puoi selezionare le aree di tuo interesse.',
    'zh-TW': '以下是FT的主要內容分類，您可以選擇自己感興趣的領域。',
    'zh-HK': '以下是FT的主要內容分類，您可以選擇自己感興趣的領域。',
    ru: 'Вот основные категории контента FT. Вы можете выбрать интересующие вас области.'
  },
  'Topic collections by industry': {
    en: 'Topic collections by industry',
    zh: '按行业分类的话题集合',
    es: 'Colecciones de temas por industria',
    fr: 'Collections de sujets par industrie',
    de: 'Themensammlungen nach Industrie',
    ja: '産業別トピックコレクション',
    ko: '산업별 주제 모음',
    pt: 'Coleções de tópicos por indústria',
    it: 'Collezioni di argomenti per industria',
    'zh-TW': '按行業分類的話題集合',
    'zh-HK': '按行業分類的話題集合',
    ru: 'Коллекции тем по отраслям'
  },
  "Accountancy & tax advisory": {
    en: "Accountancy & tax advisory",
    zh: "会计与税务咨询",
    es: "Asesoría contable y fiscal",
    fr: "Conseil en comptabilité et fiscalité",
    de: "Buchhaltungs- und Steuerberatung",
    ja: "会計および税務アドバイザリー",
    ko: "회계 및 세무 자문",
    pt: "Assessoria contábil e fiscal",
    it: "Consulenza contabile e fiscale",
    'zh-TW': "會計與稅務諮詢",
    'zh-HK': "會計與稅務諮詢",
    ru: "Бухгалтерский и налоговый консалтинг"
  },
  "Accountancy": {
    en: "Accountancy",
    zh: "会计",
    es: "Contabilidad",
    fr: "Comptabilité",
    de: "Buchhaltung",
    ja: "会計",
    ko: "회계",
    pt: "Contabilidade",
    it: "Contabilità",
    'zh-TW': "會計",
    'zh-HK': "會計",
    ru: "Бухгалтерский учет"
  },
  "International tax": {
    en: "International tax",
    zh: "国际税务",
    es: "Impuestos internacionales",
    fr: "Fiscalité internationale",
    de: "Internationale Steuern",
    ja: "国際税務",
    ko: "국제 세금",
    pt: "Tributação internacional",
    it: "Tassazione internazionale",
    'zh-TW': "國際稅務",
    'zh-HK': "國際稅務",
    ru: "Международное налогообложение"
  },
  "Tax evasion and avoidance": {
    en: "Tax evasion and avoidance",
    zh: "避税与逃税",
    es: "Evasión y elusión fiscal",
    fr: "Évasion et optimisation fiscales",
    de: "Steuerhinterziehung und -vermeidung",
    ja: "税逃れと節税",
    ko: "탈세 및 조세 회피",
    pt: "Evasão e elisão fiscal",
    it: "Evasione e elusione fiscale",
    'zh-TW': "避稅與逃稅",
    'zh-HK': "避稅與逃稅",
    ru: "Налоговое уклонение и оптимизация"
  },
  "Financial & markets regulation": {
    en: "Financial & markets regulation",
    zh: "金融与市场监管",
    es: "Regulación financiera y de mercados",
    fr: "Régulation financière et des marchés",
    de: "Finanz- und Marktaufsicht",
    ja: "金融および市場規制",
    ko: "금융 및 시장 규제",
    pt: "Regulação financeira e de mercados",
    it: "Regolamentazione finanziaria e dei mercati",
    'zh-TW': "金融與市場監管",
    'zh-HK': "金融與市場監管",
    ru: "Регулирование финансов и рынков"
  },
  "Mergers & Acquisitions": {
    en: "Mergers & Acquisitions",
    zh: "并购",
    es: "Fusiones y adquisiciones",
    fr: "Fusions et acquisitions",
    de: "Fusionen und Übernahmen",
    ja: "合併および買収",
    ko: "합병 및 인수",
    pt: "Fusões e aquisições",
    it: "Fusioni e acquisizioni",
    'zh-TW': "併購",
    'zh-HK': "併購",
    ru: "Слияния и поглощения"
  },
  "Aerospace & defence": {
    en: "Aerospace & defence",
    zh: "航空航天与国防",
    es: "Aeroespacial y defensa",
    fr: "Aérospatial et défense",
    de: "Luft- und Raumfahrt und Verteidigung",
    ja: "航空宇宙と防衛",
    ko: "항공우주 및 방위",
    pt: "Aeroespacial e defesa",
    it: "Aerospaziale e difesa",
    'zh-TW': "航空航天與國防",
    'zh-HK': "航空航天與國防",
    ru: "Аэрокосмическая и оборонная промышленность"
  },
  "Aircraft manufacturing": {
    en: "Aircraft manufacturing",
    zh: "飞机制造",
    es: "Fabricación de aeronaves",
    fr: "Fabrication d'avions",
    de: "Flugzeugherstellung",
    ja: "航空機製造",
    ko: "항공기 제조",
    pt: "Fabricação de aeronaves",
    it: "Produzione di aeromobili",
    'zh-TW': "飛機製造",
    'zh-HK': "飛機製造",
    ru: "Производство самолетов"
  },
  "Drones": {
    en: "Drones",
    zh: "无人机",
    es: "Drones",
    fr: "Drones",
    de: "Drohnen",
    ja: "ドローン",
    ko: "드론",
    pt: "Drones",
    it: "Droni",
    'zh-TW': "無人機",
    'zh-HK': "無人機",
    ru: "Беспилотники"
  },
  "Terrorism": {
    en: "Terrorism",
    zh: "恐怖主义",
    es: "Terrorismo",
    fr: "Terrorisme",
    de: "Terrorismus",
    ja: "テロリズム",
    ko: "테러리즘",
    pt: "Terrorismo",
    it: "Terrorismo",
    'zh-TW': "恐怖主義",
    'zh-HK': "恐怖主義",
    ru: "Терроризм"
  },
  "Virtual and Augmented Reality": {
    en: "Virtual and Augmented Reality",
    zh: "虚拟与增强现实",
    es: "Realidad virtual y aumentada",
    fr: "Réalité virtuelle et augmentée",
    de: "Virtuelle und Erweiterte Realität",
    ja: "バーチャルリアリティと拡張現実",
    ko: "가상 현실 및 증강 현실",
    pt: "Realidade virtual e aumentada",
    it: "Realtà virtuale e aumentata",
    'zh-TW': "虛擬與增強現實",
    'zh-HK': "虛擬與增強現實",
    ru: "Виртуальная и дополненная реальность"
  },
  "Space industry": {
    en: "Space industry",
    zh: "航天产业",
    es: "Industria espacial",
    fr: "Industrie spatiale",
    de: "Raumfahrtindustrie",
    ja: "宇宙産業",
    ko: "우주 산업",
    pt: "Indústria espacial",
    it: "Industria spaziale",
    'zh-TW': "航天產業",
    'zh-HK': "航天產業",
    ru: "Космическая отрасль"
  },
  "Automobiles": {
    en: "Automobiles",
    zh: "汽车",
    es: "Automóviles",
    fr: "Automobiles",
    de: "Automobile",
    ja: "自動車",
    ko: "자동차",
    pt: "Automóveis",
    it: "Automobili",
    'zh-TW': "汽車",
    'zh-HK': "汽車",
    ru: "Автомобили"
  },
  "Hydrogen vehicles": {
    en: "Hydrogen vehicles",
    zh: "氢能汽车",
    es: "Vehículos de hidrógeno",
    fr: "Véhicules à hydrogène",
    de: "Wasserstofffahrzeuge",
    ja: "水素自動車",
    ko: "수소 차량",
    pt: "Veículos a hidrogênio",
    it: "Veicoli a idrogeno",
    'zh-TW': "氫能汽車",
    'zh-HK': "氫能汽車",
    ru: "Водородные автомобили"
  },
  "Driverless vehicles": {
    en: "Driverless vehicles",
    zh: "无人驾驶车辆",
    es: "Vehículos sin conductor",
    fr: "Véhicules autonomes",
    de: "Fahrerlose Fahrzeuge",
    ja: "自動運転車",
    ko: "무인 운전 차량",
    pt: "Veículos autônomos",
    it: "Veicoli senza guidatore",
    'zh-TW': "無人駕駛車輛",
    'zh-HK': "無人駕駛車輛",
    ru: "Беспилотные транспортные средства"
  },
  "Batteries": {
    en: "Batteries",
    zh: "电池",
    es: "Baterías",
    fr: "Batteries",
    de: "Batterien",
    ja: "電池",
    ko: "배터리",
    pt: "Baterias",
    it: "Batterie",
    'zh-TW': "電池",
    'zh-HK': "電池",
    ru: "Батареи"
  },
  "Banking": {
    en: "Banking",
    zh: "银行业",
    es: "Banca",
    fr: "Banque",
    de: "Bankwesen",
    ja: "銀行業",
    ko: "은행업",
    pt: "Bancário",
    it: "Bancario",
    'zh-TW': "銀行業",
    'zh-HK': "銀行業",
    ru: "Банковское дело"
  },
  "Investment Banking": {
    en: "Investment Banking",
    zh: "投资银行业",
    es: "Banca de inversión",
    fr: "Banque d'investissement",
    de: "Investmentbanking",
    ja: "投資銀行",
    ko: "투자 은행",
    pt: "Banca de investimento",
    it: "Banca d'investimento",
    'zh-TW': "投資銀行業",
    'zh-HK': "投資銀行業",
    ru: "Инвестиционное банковское дело"
  },
  "Fintech": {
    en: "Fintech",
    zh: "金融科技",
    es: "Fintech",
    fr: "Fintech",
    de: "Fintech",
    ja: "フィンテック",
    ko: "핀테크",
    pt: "Fintech",
    it: "Fintech",
    'zh-TW': "金融科技",
    'zh-HK': "金融科技",
    ru: "Финтех"
  },
  "Central banks": {
    en: "Central banks",
    zh: "中央银行",
    es: "Bancos centrales",
    fr: "Banques centrales",
    de: "Zentralbanken",
    ja: "中央銀行",
    ko: "중앙 은행",
    pt: "Bancos centrais",
    it: "Banche centrali",
    'zh-TW': "中央銀行",
    'zh-HK': "中央銀行",
    ru: "Центральные банки"
  },
  "Basic resources/Mining": {
    en: "Basic resources/Mining",
    zh: "基础资源/采矿",
    es: "Recursos básicos/Minería",
    fr: "Ressources de base/Mines",
    de: "Grundstoffe/Bergbau",
    ja: "基本資源/鉱業",
    ko: "기초 자원/광업",
    pt: "Recursos básicos/Mineração",
    it: "Risorse di base/Miniere",
    'zh-TW': "基礎資源/採礦",
    'zh-HK': "基礎資源/採礦",
    ru: "Основные ресурсы/Добыча"
  },
  "Industrial metals": {
    en: "Industrial metals",
    zh: "工业金属",
    es: "Metales industriales",
    fr: "Métaux industriels",
    de: "Industriemetalle",
    ja: "産業用金属",
    ko: "산업 금속",
    pt: "Metais industriais",
    it: "Metalli industriali",
    'zh-TW': "工業金屬",
    'zh-HK': "工業金屬",
    ru: "Промышленные металлы"
  },
  "Renewable energy": {
    en: "Renewable energy",
    zh: "可再生能源",
    es: "Energía renovable",
    fr: "Énergie renouvelable",
    de: "Erneuerbare Energien",
    ja: "再生可能エネルギー",
    ko: "재생 에너지",
    pt: "Energia renovável",
    it: "Energia rinnovabile",
    'zh-TW': "可再生能源",
    'zh-HK': "可再生能源",
    ru: "Возобновляемая энергия"
  },
  "Oil": {
    en: "Oil",
    zh: "石油",
    es: "Petróleo",
    fr: "Pétrole",
    de: "Öl",
    ja: "石油",
    ko: "석유",
    pt: "Petróleo",
    it: "Petrolio",
    'zh-TW': "石油",
    'zh-HK': "石油",
    ru: "Нефть"
  },
  "Shale Oil & Gas": {
    en: "Shale Oil & Gas",
    zh: "页岩油与气",
    es: "Petróleo y gas de esquisto",
    fr: "Pétrole et gaz de schiste",
    de: "Schieferöl und -gas",
    ja: "シェールオイル・ガス",
    ko: "셰일 오일 & 가스",
    pt: "Óleo e gás de xisto",
    it: "Petrolio e gas di scisto",
    'zh-TW': "頁岩油與氣",
    'zh-HK': "頁岩油與氣",
    ru: "Сланцевая нефть и газ"
  },
  "Coal": {
    en: "Coal",
    zh: "煤炭",
    es: "Carbón",
    fr: "Charbon",
    de: "Kohle",
    ja: "石炭",
    ko: "석탄",
    pt: "Carvão",
    it: "Carbone",
    'zh-TW': "煤炭",
    'zh-HK': "煤炭",
    ru: "Уголь"
  },
  "Chemicals": {
    en: "Chemicals",
    zh: "化学品",
    es: "Químicos",
    fr: "Produits chimiques",
    de: "Chemikalien",
    ja: "化学製品",
    ko: "화학 제품",
    pt: "Produtos químicos",
    it: "Prodotti chimici",
    'zh-TW': "化學品",
    'zh-HK': "化學品",
    ru: "Химикаты"
  },
  "Comms/Publishing/Media": {
    en: "Comms/Publishing/Media",
    zh: "通信/出版/媒体",
    es: "Comunicaciones/Publicaciones/Medios",
    fr: "Comms/Publication/Médias",
    de: "Kommunikation/Verlag/Medien",
    ja: "通信/出版/メディア",
    ko: "통신/출판/미디어",
    pt: "Comunicações/Publicações/Mídia",
    it: "Comunicazioni/Editoria/Media",
    'zh-TW': "通信/出版/媒體",
    'zh-HK': "通信/出版/媒體",
    ru: "Связь/Издательство/Медиа"
  },
  "Digital Media": {
    en: "Digital Media",
    zh: "数字媒体",
    es: "Medios digitales",
    fr: "Médias numériques",
    de: "Digitale Medien",
    ja: "デジタルメディア",
    ko: "디지털 미디어",
    pt: "Mídia Digital",
    it: "Media digitali",
    'zh-TW': "數字媒體",
    'zh-HK': "數字媒體",
    ru: "Цифровые медиа"
  },
  "Social Media": {
    en: "Social Media",
    zh: "社交媒体",
    es: "Redes sociales",
    fr: "Réseaux sociaux",
    de: "Soziale Medien",
    ja: "ソーシャルメディア",
    ko: "소셜 미디어",
    pt: "Mídias sociais",
    it: "Social media",
    'zh-TW': "社交媒體",
    'zh-HK': "社交媒體",
    ru: "Социальные сети"
  },
  "Advertising": {
    en: "Advertising",
    zh: "广告",
    es: "Publicidad",
    fr: "Publicité",
    de: "Werbung",
    ja: "広告",
    ko: "광고",
    pt: "Publicidade",
    it: "Pubblicità",
    'zh-TW': "廣告",
    'zh-HK': "廣告",
    ru: "Реклама"
  },
  "Luxury goods": {
    en: "Luxury goods",
    zh: "奢侈品",
    es: "Bienes de lujo",
    fr: "Produits de luxe",
    de: "Luxusgüter",
    ja: "高級品",
    ko: "럭셔리 상품",
    pt: "Produtos de luxo",
    it: "Beni di lusso",
    'zh-TW': "奢侈品",
    'zh-HK': "奢侈品",
    ru: "Товары роскоши"
  },
  "Dow Chemical Co": {
    en: "Dow Chemical Co",
    zh: "陶氏化学公司",
    es: "Dow Chemical Co",
    fr: "Dow Chemical Co",
    de: "Dow Chemical Co",
    ja: "ダウ・ケミカル社",
    ko: "다우 케미컬 회사",
    pt: "Dow Chemical Co",
    it: "Dow Chemical Co",
    'zh-TW': "陶氏化學公司",
    'zh-HK': "陶氏化學公司",
    ru: "Дау Кемикал Ко"
  },
  "Consulting/Business services": {
    en: "Consulting/Business services",
    zh: "咨询/商业服务",
    es: "Consultoría/Servicios empresariales",
    fr: "Consulting/Services aux entreprises",
    de: "Beratung/Geschäftsdienstleistungen",
    ja: "コンサルティング/ビジネスサービス",
    ko: "컨설팅/비즈니스 서비스",
    pt: "Consultoria/Serviços empresariais",
    it: "Consulenza/Servizi aziendali",
    'zh-TW': "諮詢/商業服務",
    'zh-HK': "諮詢/商業服務",
    ru: "Консалтинг/Бизнес-услуги"
  },
  "Management consulting": {
    en: "Management consulting",
    zh: "管理咨询",
    es: "Consultoría de gestión",
    fr: "Conseil en gestion",
    de: "Managementberatung",
    ja: "経営コンサルティング",
    ko: "경영 컨설팅",
    pt: "Consultoria de gestão",
    it: "Consulenza manageriale",
    'zh-TW': "管理諮詢",
    'zh-HK': "管理諮詢",
    ru: "Управленческий консалтинг"
  },
  "Innovation": {
    en: "Innovation",
    zh: "创新",
    es: "Innovación",
    fr: "Innovation",
    de: "Innovation",
    ja: "イノベーション",
    ko: "혁신",
    pt: "Inovação",
    it: "Innovazione",
    'zh-TW': "創新",
    'zh-HK': "創新",
    ru: "Инновации"
  },
  "Corporate culture": {
    en: "Corporate culture",
    zh: "企业文化",
    es: "Cultura corporativa",
    fr: "Culture d'entreprise",
    de: "Unternehmenskultur",
    ja: "企業文化",
    ko: "기업 문화",
    pt: "Cultura corporativa",
    it: "Cultura aziendale",
    'zh-TW': "企業文化",
    'zh-HK': "企業文化",
    ru: "Корпоративная культура"
  },
  "Managing yourself": {
    en: "Managing yourself",
    zh: "自我管理",
    es: "Gestionarse a sí mismo",
    fr: "Gestion de soi",
    de: "Selbstmanagement",
    ja: "自己管理",
    ko: "자기 관리",
    pt: "Gerenciando a si mesmo",
    it: "Gestione di sé",
    'zh-TW': "自我管理",
    'zh-HK': "自我管理",
    ru: "Самоменеджмент"
  },
  "Women in business": {
    en: "Women in business",
    zh: "商界女性",
    es: "Mujeres en los negocios",
    fr: "Femmes dans les affaires",
    de: "Frauen im Geschäftsleben",
    ja: "ビジネス界の女性",
    ko: "비즈니스 분야의 여성",
    pt: "Mulheres nos negócios",
    it: "Donne in affari",
    'zh-TW': "商界女性",
    'zh-HK': "商界女性",
    ru: "Женщины в бизнесе"
  },
  "Education/Academia": {
    en: "Education/Academia",
    zh: "教育/学术",
    es: "Educación/Academia",
    fr: "Éducation/Académie",
    de: "Bildung/Akademie",
    ja: "教育/学術",
    ko: "교육/학계",
    pt: "Educação/Academia",
    it: "Educazione/Accademia",
    'zh-TW': "教育/學術",
    'zh-HK': "教育/學術",
    ru: "Образование/Академия"
  },
  "Education": {
    en: "Education",
    zh: "教育",
    es: "Educación",
    fr: "Éducation",
    de: "Bildung",
    ja: "教育",
    ko: "교육",
    pt: "Educação",
    it: "Educazione",
    'zh-TW': "教育",
    'zh-HK': "教育",
    ru: "Образование"
  },
  "Business education": {
    en: "Business education",
    zh: "商业教育",
    es: "Educación empresarial",
    fr: "Éducation commerciale",
    de: "Wirtschaftsbildung",
    ja: "ビジネス教育",
    ko: "비즈니스 교육",
    pt: "Educação empresarial",
    it: "Formazione commerciale",
    'zh-TW': "商業教育",
    'zh-HK': "商業教育",
    ru: "Бизнес-образование"
  },
  "Online learning": {
    en: "Online learning",
    zh: "在线学习",
    es: "Aprendizaje en línea",
    fr: "Apprentissage en ligne",
    de: "Online-Lernen",
    ja: "オンライン学習",
    ko: "온라인 학습",
    pt: "Aprendizado online",
    it: "Apprendimento online",
    'zh-TW': "在線學習",
    'zh-HK': "在線學習",
    ru: "Онлайн-обучение"
  },
  "Economics books": {
    en: "Economics books",
    zh: "经济学书籍",
    es: "Libros de economía",
    fr: "Livres d'économie",
    de: "Wirtschaftsbücher",
    ja: "経済学の書籍",
    ko: "경제학 서적",
    pt: "Livros de economia",
    it: "Libri di economia",
    'zh-TW': "經濟學書籍",
    'zh-HK': "經濟學書籍",
    ru: "Книги по экономике"
  },
  "Energy/Utilities": {
    en: "Energy/Utilities",
    zh: "能源/公用事业",
    es: "Energía/Utilidades",
    fr: "Énergie/Services publics",
    de: "Energie/Versorgungsunternehmen",
    ja: "エネルギー/公共事業",
    ko: "에너지/유틸리티",
    pt: "Energia/Utilidades",
    it: "Energia/Utility",
    'zh-TW': "能源/公用事業",
    'zh-HK': "能源/公用事業",
    ru: "Энергетика/Коммунальные услуги"
  },
  "Nuclear energy": {
    en: "Nuclear energy",
    zh: "核能",
    es: "Energía nuclear",
    fr: "Énergie nucléaire",
    de: "Kernenergie",
    ja: "核エネルギー",
    ko: "핵 에너지",
    pt: "Energia nuclear",
    it: "Energia nucleare",
    'zh-TW': "核能",
    'zh-HK': "核能",
    ru: "Ядерная энергия"
  },
  "Engineering/Construction": {
    en: "Engineering/Construction",
    zh: "工程/建设",
    es: "Ingeniería/Construcción",
    fr: "Ingénierie/Construction",
    de: "Ingenieurwesen/Bauwesen",
    ja: "エンジニアリング/建設",
    ko: "공학/건설",
    pt: "Engenharia/Construção",
    it: "Ingegneria/Costruzione",
    'zh-TW': "工程/建設",
    'zh-HK': "工程/建設",
    ru: "Инженерия/Строительство"
  },
  "Infrastructure investment": {
    en: "Infrastructure investment",
    zh: "基础设施投资",
    es: "Inversión en infraestructura",
    fr: "Investissement en infrastructure",
    de: "Infrastrukturinvestitionen",
    ja: "インフラ投資",
    ko: "인프라 투자",
    pt: "Investimento em infraestrutura",
    it: "Investimenti in infrastrutture",
    'zh-TW': "基礎設施投資",
    'zh-HK': "基礎設施投資",
    ru: "Инвестиции в инфраструктуру"
  },
  "Urban planning": {
    en: "Urban planning",
    zh: "城市规划",
    es: "Planeación urbana",
    fr: "Urbanisme",
    de: "Stadtplanung",
    ja: "都市計画",
    ko: "도시 계획",
    pt: "Planejamento urbano",
    it: "Pianificazione urbana",
    'zh-TW': "城市規劃",
    'zh-HK': "城市規劃",
    ru: "Градостроительство"
  },
  "Architecture": {
    en: "Architecture",
    zh: "建筑",
    es: "Arquitectura",
    fr: "Architecture",
    de: "Architektur",
    ja: "建築",
    ko: "건축",
    pt: "Arquitetura",
    it: "Architettura",
    'zh-TW': "建築",
    'zh-HK': "建築",
    ru: "Архитектура"
  },
  "UK housebuilding": {
    en: "UK housebuilding",
    zh: "英国住宅建设",
    es: "Construcción de viviendas en el Reino Unido",
    fr: "Construction de maisons au Royaume-Uni",
    de: "Hausbau im Vereinigten Königreich",
    ja: "英国の住宅建設",
    ko: "영국 주택 건설",
    pt: "Construção de casas no Reino Unido",
    it: "Costruzione di abitazioni nel Regno Unito",
    'zh-TW': "英國住宅建設",
    'zh-HK': "英國住宅建設",
    ru: "Строительство жилья в Великобритании"
  },
  "Financial services": {
    en: "Financial services",
    zh: "金融服务",
    es: "Servicios financieros",
    fr: "Services financiers",
    de: "Finanzdienstleistungen",
    ja: "金融サービス",
    ko: "금융 서비스",
    pt: "Serviços financeiros",
    it: "Servizi finanziari",
    'zh-TW': "金融服務",
    'zh-HK': "金融服務",
    ru: "Финансовые услуги"
  },
  "Hedge funds": {
    en: "Hedge funds",
    zh: "对冲基金",
    es: "Fondos de cobertura",
    fr: "Fonds de couverture",
    de: "Hedgefonds",
    ja: "ヘッジファンド",
    ko: "헤지 펀드",
    pt: "Fundos de hedge",
    it: "Fondi hedge",
    'zh-TW': "對衝基金",
    'zh-HK': "對衝基金",
    ru: "Хедж-фонды"
  },
  "Private equity": {
    en: "Private equity",
    zh: "私募股权",
    es: "Capital privado",
    fr: "Capital-investissement",
    de: "Private Equity",
    ja: "プライベートエクイティ",
    ko: "사모 펀드",
    pt: "Capital privado",
    it: "Private equity",
    'zh-TW': "私募股權",
    'zh-HK': "私募股權",
    ru: "Частный капитал"
  },
  "Emerging market investing": {
    en: "Emerging market investing",
    zh: "新兴市场投资",
    es: "Inversión en mercados emergentes",
    fr: "Investissement dans les marchés émergents",
    de: "Investitionen in Schwellenländern",
    ja: "新興市場投資",
    ko: "신흥 시장 투자",
    pt: "Investimento em mercados emergentes",
    it: "Investimenti nei mercati emergenti",
    'zh-TW': "新興市場投資",
    'zh-HK': "新興市場投資",
    ru: "Инвестиции в развивающиеся рынки"
  },
  "Govt/Public service/NGOs": {
    en: "Govt/Public service/NGOs",
    zh: "政府/公共服务/非政府组织",
    es: "Gobierno/Servicio público/ONGs",
    fr: "Gouvernement/Service public/ONG",
    de: "Regierung/Öffentlicher Dienst/NGOs",
    ja: "政府/公共サービス/NGO",
    ko: "정부/공공 서비스/NGO",
    pt: "Governo/Serviço público/ONGs",
    it: "Governo/Servizio pubblico/ONG",
    'zh-TW': "政府/公共服務/非政府組織",
    'zh-HK': "政府/公共服務/非政府組織",
    ru: "Правительство/Общественные службы/НГО"
  },
  "Geopolitics": {
    en: "Geopolitics",
    zh: "地缘政治",
    es: "Geopolítica",
    fr: "Géopolitique",
    de: "Geopolitik",
    ja: "地政学",
    ko: "지정학",
    pt: "Geopolítica",
    it: "Geopolitica",
    'zh-TW': "地緣政治",
    'zh-HK': "地緣政治",
    ru: "Геополитика"
  },
  "Populism": {
    en: "Populism",
    zh: "民粹主义",
    es: "Populismo",
    fr: "Populisme",
    de: "Populismus",
    ja: "ポピュリズム",
    ko: "대중주의",
    pt: "Populismo",
    it: "Populismo",
    'zh-TW': "民粹主義",
    'zh-HK': "民粹主義",
    ru: "Популизм"
  },
  "Globalisation": {
    en: "Globalisation",
    zh: "全球化",
    es: "Globalización",
    fr: "Mondialisation",
    de: "Globalisierung",
    ja: "グローバリゼーション",
    ko: "세계화",
    pt: "Globalização",
    it: "Globalizzazione",
    'zh-TW': "全球化",
    'zh-HK': "全球化",
    ru: "Глобализация"
  },
  "United Nations": {
    en: "United Nations",
    zh: "联合国",
    es: "Naciones Unidas",
    fr: "Nations Unies",
    de: "Vereinte Nationen",
    ja: "国際連合",
    ko: "유엔",
    pt: "Nações Unidas",
    it: "Nazioni Unite",
    'zh-TW': "聯合國",
    'zh-HK': "聯合國",
    ru: "Организация Объединенных Наций"
  },
  "Political books": {
    en: "Political books",
    zh: "政治书籍",
    es: "Libros políticos",
    fr: "Livres politiques",
    de: "Politische Bücher",
    ja: "政治書",
    ko: "정치 서적",
    pt: "Livros políticos",
    it: "Libri politici",
    'zh-TW': "政治書籍",
    'zh-HK': "政治書籍",
    ru: "Политические книги"
  },
  "Health & pharmaceuticals": {
    en: "Health & pharmaceuticals",
    zh: "健康与制药",
    es: "Salud y farmacéutica",
    fr: "Santé et pharmaceutique",
    de: "Gesundheit und Pharmazie",
    ja: "健康と製薬",
    ko: "건강 및 제약",
    pt: "Saúde e farmacêutica",
    it: "Salute e farmaceutica",
    'zh-TW': "健康與製藥",
    'zh-HK': "健康與製藥",
    ru: "Здравоохранение и фармацевтика"
  },
  "Digital health": {
    en: "Digital health",
    zh: "数字健康",
    es: "Salud digital",
    fr: "Santé numérique",
    de: "Digitale Gesundheit",
    ja: "デジタルヘルス",
    ko: "디지털 헬스",
    pt: "Saúde digital",
    it: "Salute digitale",
    'zh-TW': "數字健康",
    'zh-HK': "數字健康",
    ru: "Цифровое здравоохранение"
  },
  "Drugs research": {
    en: "Drugs research",
    zh: "药物研究",
    es: "Investigación farmacéutica",
    fr: "Recherche pharmaceutique",
    de: "Arzneimittelforschung",
    ja: "医薬品研究",
    ko: "약물 연구",
    pt: "Pesquisa de drogas",
    it: "Ricerca farmaceutica",
    'zh-TW': "藥物研究",
    'zh-HK': "藥物研究",
    ru: "Фармацевтические исследования"
  },
  "Drug prices": {
    en: "Drug prices",
    zh: "药品价格",
    es: "Precios de medicamentos",
    fr: "Prix des médicaments",
    de: "Medikamentenpreise",
    ja: "医薬品の価格",
    ko: "의약품 가격",
    pt: "Preços de medicamentos",
    it: "Prezzi dei farmaci",
    'zh-TW': "藥品價格",
    'zh-HK': "藥品價格",
    ru: "Цены на лекарства"
  },
  "Medical science": {
    en: "Medical science",
    zh: "医学科学",
    es: "Ciencia médica",
    fr: "Science médicale",
    de: "Medizinische Wissenschaft",
    ja: "医学",
    ko: "의학 과학",
    pt: "Ciência médica",
    it: "Scienza medica",
    'zh-TW': "醫學科學",
    'zh-HK': "醫學科學",
    ru: "Медицинская наука"
  },
  "Disease control and prevention": {
    en: "Disease control and prevention",
    zh: "疾病控制与预防",
    es: "Control y prevención de enfermedades",
    fr: "Contrôle et prévention des maladies",
    de: "Krankheitskontrolle und -prävention",
    ja: "疾病管理と予防",
    ko: "질병 통제 및 예방",
    pt: "Controle e prevenção de doenças",
    it: "Controllo e prevenzione delle malattie",
    'zh-TW': "疾病控制與預防",
    'zh-HK': "疾病控制與預防",
    ru: "Контроль и профилактика заболеваний"
  },
  "Industrial goods & services": {
    en: "Industrial goods & services",
    zh: "工业品与服务",
    es: "Bienes y servicios industriales",
    fr: "Biens et services industriels",
    de: "Industriegüter und -dienstleistungen",
    ja: "産業用商品・サービス",
    ko: "산업용품 및 서비스",
    pt: "Bens e serviços industriais",
    it: "Beni e servizi industriali",
    'zh-TW': "工業品與服務",
    'zh-HK': "工業品與服務",
    ru: "Промышленные товары и услуги"
  },
  "Industrials": {
    en: "Industrials",
    zh: "工业",
    es: "Industriales",
    fr: "Industriels",
    de: "Industrie",
    ja: "産業",
    ko: "산업",
    pt: "Industriais",
    it: "Industriali",
    'zh-TW': "工業",
    'zh-HK': "工業",
    ru: "Промышленность"
  },
  "Industrial goods": {
    en: "Industrial goods",
    zh: "工业品",
    es: "Bienes industriales",
    fr: "Produits industriels",
    de: "Industriegüter",
    ja: "産業用品",
    ko: "산업용품",
    pt: "Produtos industriais",
    it: "Beni industriali",
    'zh-TW': "工業品",
    'zh-HK': "工業品",
    ru: "Промышленные товары"
  },
  "Global economic growth": {
    en: "Global economic growth",
    zh: "全球经济增长",
    es: "Crecimiento económico global",
    fr: "Croissance économique mondiale",
    de: "Globales Wirtschaftswachstum",
    ja: "世界経済の成長",
    ko: "글로벌 경제 성장",
    pt: "Crescimento econômico global",
    it: "Crescita economica globale",
    'zh-TW': "全球經濟增長",
    'zh-HK': "全球經濟增長",
    ru: "Глобальный экономический рост"
  },
  "US economy": {
    en: "US economy",
    zh: "美国经济",
    es: "Economía de los Estados Unidos",
    fr: "Économie américaine",
    de: "US-Wirtschaft",
    ja: "アメリカ経済",
    ko: "미국 경제",
    pt: "Economia dos EUA",
    it: "Economia degli Stati Uniti",
    'zh-TW': "美國經濟",
    'zh-HK': "美國經濟",
    ru: "Экономика США"
  },
  "Eurozone economy": {
    en: "Eurozone economy",
    zh: "欧元区经济",
    es: "Economía de la zona euro",
    fr: "Économie de la zone euro",
    de: "Wirtschaft der Eurozone",
    ja: "ユーロ圏経済",
    ko: "유로존 경제",
    pt: "Economia da zona do euro",
    it: "Economia della zona euro",
    'zh-TW': "歐元區經濟",
    'zh-HK': "歐元區經濟",
    ru: "Экономика еврозоны"
  },
  "Insurance": {
    en: "Insurance",
    zh: "保险",
    es: "Seguros",
    fr: "Assurance",
    de: "Versicherung",
    ja: "保険",
    ko: "보험",
    pt: "Seguros",
    it: "Assicurazioni",
    'zh-TW': "保險",
    'zh-HK': "保險",
    ru: "Страхование"
  },
  "Cyber Security": {
    en: "Cyber Security",
    zh: "网络安全",
    es: "Ciberseguridad",
    fr: "Cybersécurité",
    de: "Cybersicherheit",
    ja: "サイバーセキュリティ",
    ko: "사이버 보안",
    pt: "Cibersegurança",
    it: "Cybersicurezza",
    'zh-TW': "網絡安全",
    'zh-HK': "網絡安全",
    ru: "Кибербезопасность"
  },
  "Pensions crisis": {
    en: "Pensions crisis",
    zh: "养老金危机",
    es: "Crisis de pensiones",
    fr: "Crise des pensions",
    de: "Rentenkrise",
    ja: "年金危機",
    ko: "연금 위기",
    pt: "Crise das pensões",
    it: "Crisi delle pensioni",
    'zh-TW': "養老金危機",
    'zh-HK': "養老金危機",
    ru: "Кризис пенсионного обеспечения"
  },
  "Personal Insurance": {
    en: "Personal Insurance",
    zh: "个人保险",
    es: "Seguro personal",
    fr: "Assurance personnelle",
    de: "Persönliche Versicherung",
    ja: "個人保険",
    ko: "개인 보험",
    pt: "Seguro pessoal",
    it: "Assicurazione personale",
    'zh-TW': "個人保險",
    'zh-HK': "個人保險",
    ru: "Личное страхование"
  },
  "IT/Computing": {
    en: "IT/Computing",
    zh: "信息技术/计算",
    es: "TI/Computación",
    fr: "TI/Informatique",
    de: "IT/Informatik",
    ja: "IT/コンピューティング",
    ko: "IT/컴퓨팅",
    pt: "TI/Computação",
    it: "IT/Informatica",
    'zh-TW': "資訊技術/計算",
    'zh-HK': "資訊技術/計算",
    ru: "ИТ/Вычислительная техника"
  },
  "Blockchain": {
    en: "Blockchain",
    zh: "区块链",
    es: "Blockchain",
    fr: "Blockchain",
    de: "Blockchain",
    ja: "ブロックチェーン",
    ko: "블록체인",
    pt: "Blockchain",
    it: "Blockchain",
    'zh-TW': "區塊鏈",
    'zh-HK': "區塊鏈",
    ru: "Блокчейн"
  },
  "IT/Tech/Telecoms": {
    en: "IT/Tech/Telecoms",
    zh: "信息技术/科技/电信",
    es: "TI/Tecnología/Telecomunicaciones",
    fr: "TI/Technologie/Télécommunications",
    de: "IT/Technik/Telekommunikation",
    ja: "IT/テクノロジー/通信",
    ko: "IT/기술/통신",
    pt: "TI/Tecnologia/Telecomunicações",
    it: "IT/Tecnologia/Telecomunicazioni",
    'zh-TW': "資訊技術/科技/電信",
    'zh-HK': "資訊技術/科技/電信",
    ru: "ИТ/Технологии/Телекоммуникации"
  },
  "Internet of things": {
    en: "Internet of things",
    zh: "物联网",
    es: "Internet de las cosas",
    fr: "Internet des objets",
    de: "Internet der Dinge",
    ja: "モノのインターネット",
    ko: "사물 인터넷",
    pt: "Internet das coisas",
    it: "Internet delle cose",
    'zh-TW': "物聯網",
    'zh-HK': "物聯網",
    ru: "Интернет вещей"
  },
  "Legal services": {
    en: "Legal services",
    zh: "法律服务",
    es: "Servicios legales",
    fr: "Services juridiques",
    de: "Rechtsdienstleistungen",
    ja: "法律サービス",
    ko: "법률 서비스",
    pt: "Serviços jurídicos",
    it: "Servizi legali",
    'zh-TW': "法律服務",
    'zh-HK': "法律服務",
    ru: "Юридические услуги"
  },
  "Law": {
    en: "Law",
    zh: "法律",
    es: "Ley",
    fr: "Droit",
    de: "Recht",
    ja: "法",
    ko: "법",
    pt: "Lei",
    it: "Legge",
    'zh-TW': "法律",
    'zh-HK': "法律",
    ru: "Закон"
  },
  "Corporate governance": {
    en: "Corporate governance",
    zh: "公司治理",
    es: "Gobierno corporativo",
    fr: "Gouvernance d'entreprise",
    de: "Unternehmensführung",
    ja: "コーポレートガバナンス",
    ko: "기업 지배 구조",
    pt: "Governança corporativa",
    it: "Governance aziendale",
    'zh-TW': "公司治理",
    'zh-HK': "公司治理",
    ru: "Корпоративное управление"
  },
  "Data protection": {
    en: "Data protection",
    zh: "数据保护",
    es: "Protección de datos",
    fr: "Protection des données",
    de: "Datenschutz",
    ja: "データ保護",
    ko: "데이터 보호",
    pt: "Proteção de dados",
    it: "Protezione dei dati",
    'zh-TW': "數據保護",
    'zh-HK': "數據保護",
    ru: "Защита данных"
  },
  "Oil/Gas/Mining": {
    en: "Oil/Gas/Mining",
    zh: "石油/天然气/采矿",
    es: "Petróleo/Gas/Minería",
    fr: "Pétrole/Gaz/Mines",
    de: "Öl/Gas/Bergbau",
    ja: "石油/ガス/鉱業",
    ko: "석유/가스/광업",
    pt: "Petróleo/Gás/Mineração",
    it: "Petrolio/Gas/Miniere",
    'zh-TW': "石油/天然氣/採礦",
    'zh-HK': "石油/天然氣/採礦",
    ru: "Нефть/Газ/Добыча"
  },
  "Natural gas": {
    en: "Natural gas",
    zh: "天然气",
    es: "Gas natural",
    fr: "Gaz naturel",
    de: "Erdgas",
    ja: "天然ガス",
    ko: "천연 가스",
    pt: "Gás natural",
    it: "Gas naturale",
    'zh-TW': "天然氣",
    'zh-HK': "天然氣",
    ru: "Природный газ"
  },
  "Mining": {
    en: "Mining",
    zh: "采矿",
    es: "Minería",
    fr: "Minage",
    de: "Bergbau",
    ja: "鉱業",
    ko: "광업",
    pt: "Mineração",
    it: "Miniere",
    'zh-TW': "採礦",
    'zh-HK': "採礦",
    ru: "Добыча"
  },
  "Personal & household goods": {
    en: "Personal & household goods",
    zh: "个人及家庭用品",
    es: "Bienes personales y del hogar",
    fr: "Biens personnels et ménagers",
    de: "Persönliche und Haushaltswaren",
    ja: "個人用および家庭用品",
    ko: "개인 및 가정용품",
    pt: "Bens pessoais e domésticos",
    it: "Beni personali e per la casa",
    'zh-TW': "個人及家庭用品",
    'zh-HK': "個人及家庭用品",
    ru: "Личные и бытовые товары"
  },
  "Consumer trends": {
    en: "Consumer trends",
    zh: "消费趋势",
    es: "Tendencias del consumidor",
    fr: "Tendances de consommation",
    de: "Verbrauchertrends",
    ja: "消費者トレンド",
    ko: "소비자 추세",
    pt: "Tendências de consumo",
    it: "Tendenze dei consumatori",
    'zh-TW': "消費趨勢",
    'zh-HK': "消費趨勢",
    ru: "Тенденции потребления"
  },
  "UK retail sales": {
    en: "UK retail sales",
    zh: "英国零售销售",
    es: "Ventas minoristas en el Reino Unido",
    fr: "Ventes au détail au Royaume-Uni",
    de: "Einzelhandelsumsätze in Großbritannien",
    ja: "イギリスの小売売上",
    ko: "영국 소매 판매",
    pt: "Vendas a retalho no Reino Unido",
    it: "Vendite al dettaglio nel Regno Unito",
    'zh-TW': "英國零售銷售",
    'zh-HK': "英國零售銷售",
    ru: "Розничные продажи в Великобритании"
  },
  "Commercial property": {
    en: "Commercial property",
    zh: "商业地产",
    es: "Propiedad comercial",
    fr: "Propriété commerciale",
    de: "Gewerbeimmobilien",
    ja: "商業用不動産",
    ko: "상업용 부동산",
    pt: "Propriedade comercial",
    it: "Proprietà commerciale",
    'zh-TW': "商業地產",
    'zh-HK': "商業地產",
    ru: "Коммерческая недвижимость"
  },
  "Residential property": {
    en: "Residential property",
    zh: "住宅地产",
    es: "Propiedad residencial",
    fr: "Propriété résidentielle",
    de: "Wohnimmobilien",
    ja: "住宅用不動産",
    ko: "주거용 부동산",
    pt: "Propriedade residencial",
    it: "Proprietà residenziale",
    'zh-TW': "住宅地產",
    'zh-HK': "住宅地產",
    ru: "Жилая недвижимость"
  },
  "Global property": {
    en: "Global property",
    zh: "全球地产",
    es: "Propiedad global",
    fr: "Propriété mondiale",
    de: "Globale Immobilien",
    ja: "グローバル不動産",
    ko: "글로벌 부동산",
    pt: "Propriedade global",
    it: "Proprietà globale",
    'zh-TW': "全球地產",
    'zh-HK': "全球地產",
    ru: "Глобальная недвижимость"
  },
  "Property funds": {
    en: "Property funds",
    zh: "地产基金",
    es: "Fondos inmobiliarios",
    fr: "Fonds immobiliers",
    de: "Immobilienfonds",
    ja: "不動産ファンド",
    ko: "부동산 펀드",
    pt: "Fundos imobiliários",
    it: "Fondi immobiliari",
    'zh-TW': "地產基金",
    'zh-HK': "地產基金",
    ru: "Инвестиционные фонды недвижимости"
  },
  "Retail": {
    en: "Retail",
    zh: "零售",
    es: "Minorista",
    fr: "Détail",
    de: "Einzelhandel",
    ja: "小売り",
    ko: "소매",
    pt: "Varejo",
    it: "Vendita al dettaglio",
    'zh-TW': "零售",
    'zh-HK': "零售",
    ru: "Розничная торговля"
  },
  "Wearable technology": {
    en: "Wearable technology",
    zh: "可穿戴技术",
    es: "Tecnología vestible",
    fr: "Technologie portable",
    de: "Wearable Technology",
    ja: "ウェアラブルテクノロジー",
    ko: "웨어러블 기술",
    pt: "Tecnologia vestível",
    it: "Tecnologia indossabile",
    'zh-TW': "可穿戴技術",
    'zh-HK': "可穿戴技術",
    ru: "Носимые устройства"
  },
  "Telecommunications": {
    en: "Telecommunications",
    zh: "电信",
    es: "Telecomunicaciones",
    fr: "Télécommunications",
    de: "Telekommunikation",
    ja: "通信",
    ko: "통신",
    pt: "Telecomunicações",
    it: "Telecomunicazioni",
    'zh-TW': "電信",
    'zh-HK': "電信",
    ru: "Телекоммуникации"
  },
  "Mobile devices": {
    en: "Mobile devices",
    zh: "移动设备",
    es: "Dispositivos móviles",
    fr: "Appareils mobiles",
    de: "Mobile Geräte",
    ja: "モバイルデバイス",
    ko: "모바일 장치",
    pt: "Dispositivos móveis",
    it: "Dispositivi mobili",
    'zh-TW': "移動設備",
    'zh-HK': "移動設備",
    ru: "Мобильные устройства"
  },
  "Trade/Import/Export": {
    en: "Trade/Import/Export",
    zh: "贸易/进口/出口",
    es: "Comercio/Importación/Exportación",
    fr: "Commerce/Importation/Exportation",
    de: "Handel/Import/Export",
    ja: "貿易/輸入/輸出",
    ko: "무역/수입/수출",
    pt: "Comércio/Importação/Exportação",
    it: "Commercio/Importazione/Esportazione",
    'zh-TW': "貿易/進口/出口",
    'zh-HK': "貿易/進口/出口",
    ru: "Торговля/Импорт/Экспорт"
  },
  "Global trade": {
    en: "Global trade",
    zh: "全球贸易",
    es: "Comercio global",
    fr: "Commerce mondial",
    de: "Globaler Handel",
    ja: "グローバル貿易",
    ko: "글로벌 무역",
    pt: "Comércio global",
    it: "Commercio globale",
    'zh-TW': "全球貿易",
    'zh-HK': "全球貿易",
    ru: "Глобальная торговля"
  },
  "FT Commodities Global Summit": {
    en: "FT Commodities Global Summit",
    zh: "FT全球商品峰会",
    es: "Cumbre Global de Commodities de FT",
    fr: "Sommet mondial des matières premières de FT",
    de: "FT Globale Rohstoffgipfel",
    ja: "FTコモディティーズグローバルサミット",
    ko: "FT 상품 글로벌 서밋",
    pt: "Cúpula Global de Commodities da FT",
    it: "Summit Globale delle Commodities di FT",
    'zh-TW': "FT全球商品峰會",
    'zh-HK': "FT全球商品峰會",
    ru: "Глобальный саммит FT по товарным рынкам"
  },
  "Transport/Logistics": {
    en: "Transport/Logistics",
    zh: "运输/物流",
    es: "Transporte/Logística",
    fr: "Transport/Logistique",
    de: "Transport/Logistik",
    ja: "運送/物流",
    ko: "운송/물류",
    pt: "Transporte/Logística",
    it: "Trasporto/Logistica",
    'zh-TW': "運輸/物流",
    'zh-HK': "運輸/物流",
    ru: "Транспорт/Логистика"
  },
  "Container shipping": {
    en: "Container shipping",
    zh: "集装箱运输",
    es: "Envío de contenedores",
    fr: "Expédition de conteneurs",
    de: "Container-Verschiffung",
    ja: "コンテナ輸送",
    ko: "컨테이너 선적",
    pt: "Transporte de contêineres",
    it: "Spedizione di container",
    'zh-TW': "集裝箱運輸",
    'zh-HK': "集裝箱運輸",
    ru: "Контейнерные перевозки"
  },
  "Travel & Leisure": {
    en: "Travel & Leisure",
    zh: "旅游与休闲",
    es: "Viajes y ocio",
    fr: "Voyages et loisirs",
    de: "Reisen und Freizeit",
    ja: "旅行とレジャー",
    ko: "여행 및 레저",
    pt: "Viagens e Lazer",
    it: "Viaggi e tempo libero",
    'zh-TW': "旅遊與休閒",
    'zh-HK': "旅遊與休閒",
    ru: "Путешествия и отдых"
  },
  "Travel & leisure industry": {
    en: "Travel & leisure industry",
    zh: "旅游与休闲产业",
    es: "Industria de viajes y ocio",
    fr: "Industrie du voyage et des loisirs",
    de: "Reise- und Freizeitindustrie",
    ja: "旅行・レジャー産業",
    ko: "여행 및 레저 산업",
    pt: "Indústria de viagens e lazer",
    it: "Industria dei viaggi e del tempo libero",
    'zh-TW': "旅遊與休閒產業",
    'zh-HK': "旅遊與休閒產業",
    ru: "Индустрия путешествий и отдыха"
  },
  "Work-life balance": {
    en: "Work-life balance",
    zh: "工作生活平衡",
    es: "Equilibrio trabajo-vida",
    fr: "Équilibre travail-vie",
    de: "Work-Life-Balance",
    ja: "ワークライフバランス",
    ko: "일과 생활의 균형",
    pt: "Equilíbrio entre trabalho e vida pessoal",
    it: "Equilibrio tra lavoro e vita privata",
    'zh-TW': "工作生活平衡",
    'zh-HK': "工作生活平衡",
    ru: "Баланс работы и личной жизни"
  },
  "Food & beverages": {
    en: "Food & beverages",
    zh: "食品与饮料",
    es: "Alimentos y bebidas",
    fr: "Alimentation et boissons",
    de: "Lebensmittel und Getränke",
    ja: "食品と飲料",
    ko: "음식 및 음료",
    pt: "Alimentos e bebidas",
    it: "Cibo e bevande",
    'zh-TW': "食品與飲料",
    'zh-HK': "食品與飲料",
    ru: "Продукты питания и напитки"
  },
  "Food Prices": {
    en: "Food Prices",
    zh: "食品价格",
    es: "Precios de los alimentos",
    fr: "Prix des aliments",
    de: "Lebensmittelpreise",
    ja: "食品価格",
    ko: "식품 가격",
    pt: "Preços dos alimentos",
    it: "Prezzi degli alimenti",
    'zh-TW': "食品價格",
    'zh-HK': "食品價格",
    ru: "Цены на продукты"
  },
  "Food diet": {
    en: "Food diet",
    zh: "饮食",
    es: "Dieta alimenticia",
    fr: "Régime alimentaire",
    de: "Ernährungsdiät",
    ja: "食事制限",
    ko: "식이 요법",
    pt: "Dieta alimentar",
    it: "Dieta alimentare",
    'zh-TW': "飲食",
    'zh-HK': "飲食",
    ru: "Пищевая диета"
  },
  "Beer and spirits": {
    en: "Beer and spirits",
    zh: "啤酒与烈酒",
    es: "Cerveza y licores",
    fr: "Bière et spiritueux",
    de: "Bier und Spirituosen",
    ja: "ビールとスピリッツ",
    ko: "맥주와 증류주",
    pt: "Cerveja e destilados",
    it: "Birra e distillati",
    'zh-TW': "啤酒與烈酒",
    'zh-HK': "啤酒與烈酒",
    ru: "Пиво и крепкие напитки"
  },
  "Agricultural production": {
    en: "Agricultural production",
    zh: "农业生产",
    es: "Producción agrícola",
    fr: "Production agricole",
    de: "Landwirtschaftliche Produktion",
    ja: "農業生産",
    ko: "농업 생산",
    pt: "Produção agrícola",
    it: "Produzione agricola",
    'zh-TW': "農業生產",
    'zh-HK': "農業生產",
    ru: "Сельскохозяйственное производство"
  },
  "Ecommerce": {
    en: "Ecommerce",
    zh: "电子商务",
    es: "Comercio electrónico",
    fr: "Commerce électronique",
    de: "E-Commerce",
    ja: "電子商取引",
    ko: "전자 상거래",
    pt: "Comércio eletrônico",
    it: "E-commerce",
    'zh-TW': "電子商務",
    'zh-HK': "電子商務",
    ru: "Электронная коммерция"
  },
  "Fund/Asset management": {
    en: "Fund/Asset management",
    zh: "基金/资产管理",
    es: "Gestión de fondos/activos",
    fr: "Gestion de fonds/d'actifs",
    de: "Fonds-/Vermögensverwaltung",
    ja: "ファンド/アセットマネジメント",
    ko: "펀드/자산 관리",
    pt: "Gestão de fundos/ativos",
    it: "Gestione di fondi/attività",
    'zh-TW': "基金/資產管理",
    'zh-HK': "基金/資產管理",
    ru: "Управление фондами/активами"
  },
  "Politics": {
    en: "Politics",
    zh: "政治",
    es: "Política",
    fr: "Politique",
    de: "Politik",
    ja: "政治",
    ko: "정치",
    pt: "Política",
    it: "Politica",
    'zh-TW': "政治",
    'zh-HK': "政治",
    ru: "Политика"
  },
  "Investments": {
    en: "Investments",
    zh: "投资",
    es: "Inversiones",
    fr: "Investissements",
    de: "Investitionen",
    ja: "投資",
    ko: "투자",
    pt: "Investimentos",
    it: "Investimenti",
    'zh-TW': "投資",
    'zh-HK': "投資",
    ru: "Инвестиции"
  },
  "Books": {
    en: "Books",
    zh: "读书",
    es: "Libros",
    fr: "Livres",
    de: "Bücher",
    ja: "本",
    ko: "책",
    pt: "Livros",
    it: "Libri",
    'zh-TW': "讀書",
    'zh-HK': "讀書",
    ru: "Книги"
  },
  "Management": {
    en: "Management",
    zh: "管理",
    es: "Gestión",
    fr: "Gestion",
    de: "Management",
    ja: "管理",
    ko: "관리",
    pt: "Gestão",
    it: "Gestione",
    'zh-TW': "管理",
    'zh-HK': "管理",
    ru: "Управление"
  },
  "Personal Finance": {
    en: "Personal Finance",
    zh: "个人理财",
    es: "Finanzas personales",
    fr: "Finances personnelles",
    de: "Persönliche Finanzen",
    ja: "個人ファイナンス",
    ko: "개인 재무",
    pt: "Finanças pessoais",
    it: "Finanza personale",
    'zh-TW': "個人理財",
    'zh-HK': "個人理財",
    ru: "Личные финансы"
  },
  "Sport": {
    en: "Sport",
    zh: "体育",
    es: "Deporte",
    fr: "Sport",
    de: "Sport",
    ja: "スポーツ",
    ko: "스포츠",
    pt: "Desporto",
    it: "Sport",
    'zh-TW': "體育",
    'zh-HK': "體育",
    ru: "Спорт"
  },  
  input_custom_interests: {
    zh: '您是否有特别关注的话题？请在下方输入您感兴趣的关键词。',
    en: 'Do you have any specific topics of interest? Please enter your keywords below.',
    es: '¿Tiene algún tema de interés específico? Por favor, introduzca sus palabras clave a continuación.',
    fr: 'Avez-vous des sujets d’intérêt particuliers ? Veuillez saisir vos mots-clés ci-dessous.',
    de: 'Haben Sie spezielle Interessengebiete? Bitte geben Sie Ihre Schlüsselwörter unten ein.',
    ja: '特に関心のあるトピックはありますか？以下にキーワードを入力してください。',
    ko: '특별히 관심 있는 주제가 있으신가요? 아래에 키워드를 입력해 주세요.',
    pt: 'Você tem algum tópico de interesse específico? Por favor, insira suas palavras-chave abaixo.',
    it: 'Hai degli argomenti di interesse specifico? Inserisci le tue parole chiave qui sotto.',
    'zh-TW': '您是否有特別關注的話題？請在下方輸入您感興趣的關鍵詞。',
    'zh-HK': '您是否有特別關注的話題？請在下方輸入您感興趣的關鍵詞。',
    ru: 'Есть ли у вас конкретные темы, которые вас интересуют? Пожалуйста, введите ваши ключевые слова ниже.'
  },
  input_custom_interests_detail: {
    zh: '<p>您可以通过输入话题、类型、人物、地域、栏目、作者等关键词来定制您关注的FT内容，确保不错过任何重要资讯。</p><p>即使您关注的话题在FT没有对应的标签，你也可以关注自己输入的话题，让我们的人工智能语义搜索来协助您发现相关的最新内容。</p>',
    en: `<p>You can customize the FT content you follow by entering keywords such as topics, types, people, regions, columns, authors, etc., ensuring you don't miss any important information.</p><p>Even if the topic you are interested in doesn't have a direct label in FT, you can still follow it by entering it, and our AI semantic search will assist you in discovering the most relevant latest content.</p>`,
    es: `<p>Puede personalizar el contenido de FT que sigue introduciendo palabras clave como temas, tipos, personas, regiones, columnas, autores, etc., asegurándose de no perderse ninguna información importante.</p><p>Incluso si el tema que le interesa no tiene una etiqueta directa en FT, aún puede seguirlo ingresándolo, y nuestra búsqueda semántica de IA le ayudará a descubrir el contenido más reciente y relevante.</p>`,
    fr: `<p>Vous pouvez personnaliser le contenu FT que vous suivez en saisissant des mots-clés tels que sujets, types, personnes, régions, colonnes, auteurs, etc., pour ne manquer aucune information importante.</p><p>Même si le sujet qui vous intéresse n'a pas d'étiquette directe dans FT, vous pouvez le suivre en le saisissant, et notre recherche sémantique IA vous aidera à découvrir le contenu le plus pertinent et le plus récent.</p>`,
    de: '<p>Sie können den FT-Inhalt, den Sie verfolgen, anpassen, indem Sie Schlüsselwörter wie Themen, Typen, Personen, Regionen, Spalten, Autoren usw. eingeben, um sicherzustellen, dass Sie keine wichtigen Informationen verpassen.</p><p>Auch wenn das Thema, das Sie interessiert, keine direkte Kennzeichnung in FT hat, können Sie es dennoch verfolgen, indem Sie es eingeben, und unsere KI-basierte semantische Suche wird Ihnen helfen, die relevantesten neuesten Inhalte zu entdecken.</p>',
    ja: '<p>話題、タイプ、人物、地域、コラム、著者などのキーワードを入力することで、フォローしているFTコンテンツをカスタマイズし、重要な情報を逃さないようにできます。</p><p>興味のあるトピックにFT内で直接的なラベルがなくても、それを入力することでフォローでき、当社のAIセマンティック検索が最も関連性の高い最新コンテンツを発見するお手伝いをします。</p>',
    ko: '<p>주제, 유형, 인물, 지역, 칼럼, 저자 등의 키워드를 입력함으로써, 당신이 팔로우하는 FT 콘텐츠를 맞춤화하여 중요한 정보를 놓치지 않도록 할 수 있습니다.</p><p>관심 있는 주제에 FT에서 직접적인 라벨이 없더라도 그 주제를 입력하여 팔로우할 수 있으며, 우리의 AI 의미 검색이 가장 관련성 높은 최신 콘텐츠를 발견하는 데 도움을 줄 것입니다.</p>',
    pt: '<p>Você pode personalizar o conteúdo da FT que segue, inserindo palavras-chave como tópicos, tipos, pessoas, regiões, colunas, autores, etc., garantindo que você não perca nenhuma informação importante.</p><p>Mesmo que o tema de seu interesse não tenha uma etiqueta direta na FT, você ainda pode segui-lo inserindo-o, e nossa busca semântica de IA ajudará você a descobrir o conteúdo mais relevante e atualizado.</p>',
    it: `<p>Puoi personalizzare i contenuti FT che segui inserendo parole chiave come argomenti, tipi, persone, regioni, colonne, autori, ecc., per assicurarti di non perdere nessuna informazione importante.</p><p>Anche se l'argomento di tuo interesse non ha un'etichetta diretta in FT, puoi comunque seguirlo inserendolo, e la nostra ricerca semantica AI ti aiuterà a scoprire i contenuti più recenti e rilevanti.</p>`,
    'zh-TW': '<p>您可以透過輸入話題、類型、人物、地區、專欄、作者等關鍵詞來訂製您關注的FT內容，確保不錯過任何重要資訊。</p><p>即使您關注的話題在FT沒有對應的標籤，您也可以輸入自己關注的話題，讓我們的人工智能語義搜尋協助您發現相關的最新內容。</p>',
    'zh-HK': '<p>您可以透過輸入話題、類型、人物、地區、專欄、作者等關鍵詞來訂製您關注的FT內容，確保不錯過任何重要資訊。</p><p>即使您關注的話題在FT沒有對應的標籤，您也可以輸入自己關注的話題，讓我們的人工智能語義搜尋協助您發現相關的最新內容。</p>',
    ru: '<p>Вы можете настраивать содержание FT, которое вы следите, вводя ключевые слова, такие как темы, типы, люди, регионы, колонки, авторы и т. д., чтобы не пропустить никакой важной информации.</p><p>Даже если интересующая вас тема не имеет прямой метки в FT, вы можете следить за ней, вводя ее, и наш семантический поиск на базе ИИ поможет вам обнаружить наиболее актуальный и релевантный контент.</p>'
  },
  'Introduction': {
    en: 'Welcome to your personal finance and business news assistant! My goal is to help you get the most value out of your subscription by providing customized recommendations and insights. ',
    es: '¡Bienvenido a tu asistente personal de finanzas y noticias empresariales! Mi objetivo es ayudarte a obtener el mayor valor de tu suscripción mediante recomendaciones e ideas personalizadas.',
    fr: 'Bienvenue à votre assistant personnel pour les actualités financières et commerciales ! Mon objectif est de vous aider à tirer le meilleur parti de votre abonnement en vous fournissant des recommandations et des informations personnalisées.',
    de: 'Willkommen bei Ihrem persönlichen Finanz- und Wirtschaftsnachrichtenassistenten! Mein Ziel ist es, Ihnen durch maßgeschneiderte Empfehlungen und Einblicke den größten Nutzen aus Ihrem Abonnement zu verschaffen.',
    ja: 'ファイナンシャル•タイムズのパーソナル•ファイナンス＆ビジネスニュース•アシスタントへようこそ！私の目標は、カスタマイズされた推奨事項と洞察力を提供することで、あなたのサブスクリプションから最大の価値を得ることをお手伝いすることです。',
    ko: '개인 맞춤형 금융 및 비즈니스 뉴스 어시스턴트에 오신 것을 환영합니다! 나의 목표는 맞춤형 추천 및 인사이트를 제공하여 구독에서 최대 가치를 끌어내는 것입니다.',
    pt: 'Bem-vindo ao seu assistente pessoal de finanças e notícias empresariais! Meu objetivo é ajudá-lo a obter o máximo valor da sua assinatura, fornecendo recomendações e insights personalizados.',
    it: 'Benvenuto nel tuo assistente personale per le notizie finanziarie e commerciali! Il mio obiettivo è aiutarti a ottenere il massimo valore dalla tua sottoscrizione fornendo raccomandazioni e informazioni personalizzate.',
    'zh-TW': '歡迎使用專屬於您的財經新聞助理！我的目標是透過提供量身定制的建議和深度分析，協助您充分利用您的訂閱。',
    'zh-HK': '歡迎使用專屬於您的財經新聞助理！我的目標是透過提供量身定制的建議和深度分析，協助您充分利用您的訂閱。',
    zh: '欢迎使用专属于您的财经新闻助理！我的目标是通过提供量身定制的推荐和深度分析，协助您充分利用您的订阅。',
    ru: 'Добро пожаловать в вашего персонального помощника по финансовым и бизнес-новостям! Моя цель - помочь вам получить максимальную пользу от вашей подписки, предоставляя настраиваемые рекомендации и аналитику.'
  },
  "premium": {
    "zh": "高端",
    "en": "premium",
    "es": "premium",
    "fr": "premium",
    "de": "Premium",
    "ja": "プレミアム",
    "ko": "프리미엄",
    "pt": "premium",
    "it": "premium",
    "zh-TW": "高端",
    "zh-HK": "高端",
    "ru": "премиум"
  },
  "standard": {
    "zh": "标准",
    "en": "standard",
    "es": "estándar",
    "fr": "standard",
    "de": "Standard",
    "ja": "スタンダード",
    "ko": "스탠다드",
    "pt": "padrão",
    "it": "standard",
    "zh-TW": "標準",
    "zh-HK": "標準",
    "ru": "стандартный"
  },
  "PromotionInIntroduction": {
      "zh": "从2024年8月19日至9月1日，邀请您免费体验，专门为高端会员量身定制的内容推荐和深度分析，协助您充分利用您的订阅。",
      "en": "From August 19 to September 1, 2024, we invite you to a free trial experience, offering customized content recommendations and in-depth analysis specifically tailored for premium members to help you fully utilize your subscription.",
      "es": "Del 19 de agosto al 1 de septiembre de 2024, te invitamos a una experiencia gratuita, ofreciendo recomendaciones de contenido personalizadas y análisis profundos específicamente adaptados para los miembros premium para ayudarte a aprovechar al máximo tu suscripción.",
      "fr": "Du 19 août au 1er septembre 2024, nous vous invitons à une expérience d'essai gratuite, offrant des recommandations de contenu personnalisées et des analyses approfondies spécialement adaptées pour les membres premium afin de vous aider à tirer pleinement parti de votre abonnement.",
      "de": "Vom 19. August bis 1. September 2024 laden wir Sie zu einem kostenlosen Testangebot ein, das speziell für Premium-Mitglieder maßgeschneiderte Inhaltsvorschläge und tiefgehende Analysen bietet, um Ihnen zu helfen, Ihr Abonnement voll auszunutzen.",
      "ja": "2024年8月19日から9月1日まで、プレミアム会員向けに特別にカスタマイズされたコンテンツの推薦と深い分析を提供する無料体験をご招待します。",
      "ko": "2024년 8월 19일부터 9월 1일까지, 프리미엄 회원을 위해 특별히 맞춤화된 콘텐츠 추천과 심층 분석을 제공하는 무료 체험을 초대합니다.",
      "pt": "De 19 de agosto a 1 de setembro de 2024, convidamos você para uma experiência gratuita, oferecendo recomendações de conteúdo personalizadas e análises aprofundadas especialmente adaptadas para membros premium, para ajudá-lo a aproveitar ao máximo sua assinatura.",
      "it": "Dal 19 agosto al 1 settembre 2024, ti invitiamo a una prova gratuita, offrendo raccomandazioni di contenuto personalizzate e analisi approfondite specificamente adattate per i membri premium per aiutarti a sfruttare al massimo la tua sottoscrizione.",
      "zh-TW": "從2024年8月19日至9月1日，邀請您免費體驗，專門為高端會員量身定制的內容推薦和深度分析，協助您充分利用您的訂閱。",
      "zh-HK": "從2024年8月19日至9月1日，邀請您免費體驗，專門為高端會員量身定制的內容推薦和深度分析，協助您充分利用您的訂閱。",
      "ru": "С 19 августа по 1 сентября 2024 года мы приглашаем вас на бесплатный пробный период, предлагая индивидуальные рекомендации по контенту и глубокий анализ, специально адаптированные для премиум-участников, чтобы помочь вам максимально использовать вашу подписку."
  },
  "PromotionActionButton": {
      "zh": "现在订阅，享5折优惠",
      "en": "Subscribe now and enjoy a 50% discount",
      "es": "Suscríbete ahora y disfruta de un 50% de descuento",
      "fr": "Abonnez-vous maintenant et bénéficiez de 50% de réduction",
      "de": "Abonnieren Sie jetzt und genießen Sie 50% Rabatt",
      "ja": "今すぐ購読して、50％割引をお楽しみください",
      "ko": "지금 구독하고 50% 할인 받으세요",
      "pt": "Assine agora e aproveite um desconto de 50%",
      "it": "Abbonati ora e goditi il 50% di sconto",
      "zh-TW": "現在訂閱，享50%優惠",
      "zh-HK": "現在訂閱，享50%優惠",
      "ru": "Подпишитесь сейчас и получите скидку 50%"
  },
  'IntroductionFTAcademy': {
    zh: '欢迎使用专属于您的FT商学院助理！我的目标是利用FT的优质内容，帮助您提升自己。',
    en:  `Welcome to your personal FT Academy assistant! My goal is to leverage FT's high-quality content to help you improve yourself.`,
    es: '¡Bienvenido a tu asistente personal de la Academia FT! Mi objetivo es aprovechar el contenido de alta calidad de FT para ayudarte a mejorar.',
    fr: `Bienvenue à votre assistant personnel de l'Académie FT ! Mon objectif est de tirer parti du contenu de haute qualité du FT pour vous aider à vous améliorer.`,
    de: 'Willkommen bei Ihrem persönlichen FT Academy-Assistenten! Mein Ziel ist es, die hochwertigen Inhalte des FT zu nutzen, um Ihnen bei Ihrer Weiterentwicklung zu helfen.',
    ja: 'FTアカデミーのパーソナルアシスタントへようこそ！私の目標は、FTの高品質なコンテンツを活用して、あなたの成長を支援することです。',
    ko: '개인 FT 아카데미 어시스턴트에 오신 것을 환영합니다! 나의 목표는 FT의 고품질 콘텐츠를 활용하여 여러분의 성장을 돕는 것입니다.',
    pt: 'Bem-vindo ao seu assistente pessoal da Academia FT! Meu objetivo é aproveitar o conteúdo de alta qualidade da FT para ajudá-lo a melhorar.',
    it: `Benvenuto nel tuo assistente personale dell'Accademia FT! Il mio obiettivo è sfruttare i contenuti di alta qualità del FT per aiutarti a migliorare te stesso.`,
    'zh-TW': '歡迎使用專屬於您的FT商學院助理！我的目標是利用FT的優質內容，幫助您提升自己。',
    'zh-HK': '歡迎使用專屬於您的FT商學院助理！我的目標是利用FT的優質內容，幫助您提升自己。',
    ru: 'Добро пожаловать в вашего персонального помощника от FT Academy! Моя цель - использовать качественный контент FT, чтобы помочь вам улучшить себя.'
  },
  'IntroductionDailyEnglish': {
    zh: '欢迎使用专属于您的每日英语小助理！我的目标是利用FT的优质英文内容来为您服务。',
    en: `Welcome to your personal Daily English assistant! My goal is to leverage FT's high-quality English content to serve you.`,
    es: '¡Bienvenido a tu asistente personal de inglés diario! Mi objetivo es aprovechar el contenido en inglés de alta calidad de FT para servirte.',
    fr: `Bienvenue à votre assistant personnel d'anglais quotidien ! Mon objectif est de tirer parti du contenu en anglais de haute qualité du FT pour vous servir.`,
    de: 'Willkommen bei Ihrem persönlichen Daily English-Assistenten! Mein Ziel ist es, die hochwertigen englischen Inhalte des FT zu nutzen, um Sie zu unterstützen.',
    ja: '毎日の英語の個人アシスタントへようこそ！私の目標は、FTの高品質な英語コンテンツを活用してあなたに役立つことです。',
    ko: '개인 맞춤형 매일 영어 어시스턴트에 오신 것을 환영합니다! 나의 목표는 FT의 고품질 영어 콘텐츠를 활용하여 여러분을 지원하는 것입니다.',
    pt: 'Bem-vindo ao seu assistente pessoal de inglês diário! Meu objetivo é aproveitar o conteúdo em inglês de alta qualidade do FT para servi-lo.',
    it: 'Benvenuto nel tuo assistente personale di inglese quotidiano! Il mio obiettivo è sfruttare i contenuti in inglese di alta qualità del FT per servirti.',
    'zh-TW': '歡迎使用專屬於您的每日英語小助理！我的目標是利用FT的優質英文內容來為您服務。',
    'zh-HK': '歡迎使用專屬於您的每日英語小助理！我的目標是利用FT的優質英文內容來為您服務。',
    ru: 'Добро пожаловать в вашего персонального помощника по ежедневному английскому языку! Моя цель - использовать качественный английский контент FT, чтобы помочь вам.'
  },
  'IntroductionVideoAudio': {
    zh: '欢迎使用专属于您的视频和音频助理！您可以在这里查看FT最新发布的视频和音频内容。',
    en: `Welcome to your personal Video and Audio assistant! Here, you can view the latest videos and audio content released by FT.`,
    es: '¡Bienvenido a tu asistente personal de video y audio! Aquí, puedes ver los últimos videos y contenido de audio publicados por FT.',
    fr: `Bienvenue à votre assistant personnel vidéo et audio ! Ici, vous pouvez visionner les dernières vidéos et contenus audio publiés par FT.`,
    de: 'Willkommen bei Ihrem persönlichen Video- und Audio-Assistenten! Hier können Sie die neuesten Videos und Audio-Inhalte von FT ansehen.',
    ja: '動画と音声のパーソナルアシスタントへようこそ！こちらでは、FTが最新に公開した動画や音声コンテンツをご覧いただけます。',
    ko: '개인 맞춤형 비디오 및 오디오 어시스턴트에 오신 것을 환영합니다! 여기에서 FT가 최신으로 게시한 비디오 및 오디오 콘텐츠를 확인할 수 있습니다.',
    pt: 'Bem-vindo ao seu assistente pessoal de vídeo e áudio! Aqui, você pode visualizar os vídeos e conteúdos de áudio mais recentes lançados pela FT.',
    it: 'Benvenuto nel tuo assistente personale per video e audio! Qui puoi visualizzare gli ultimi video e contenuti audio pubblicati da FT.',
    'zh-TW': '歡迎使用專屬於您的影音助理！您可以在這裡查看FT最新發布的影音內容。',
    'zh-HK': '歡迎使用專屬於您的影音助理！您可以在這裡查看FT最新發布的影音內容。',
    ru: 'Добро пожаловать в вашего персонального помощника по видео и аудио! Здесь вы можете просматривать последние видео- и аудиоконтенты, выпущенные FT.'
  },
  'IntroductionCustomerService': {
    zh: '欢迎使用专属于您的FT客户服务助理！您对于我们的服务有任何问题，让我尽力为您解答。',
    en: `Welcome to your personal FT Customer Service assistant! If you have any questions regarding our services, let me do my best to assist you.`,
    es: '¡Bienvenido a tu asistente personal de Servicio al Cliente de FT! Si tienes alguna pregunta sobre nuestros servicios, permíteme hacer todo lo posible para ayudarte.',
    fr: `Bienvenue à votre assistant personnel du Service Client FT ! Si vous avez des questions concernant nos services, laissez-moi faire de mon mieux pour vous aider.`,
    de: 'Willkommen bei Ihrem persönlichen FT-Kundenservice-Assistenten! Wenn Sie Fragen zu unseren Dienstleistungen haben, lassen Sie mich mein Bestes tun, um Ihnen zu helfen.',
    ja: 'FTカスタマーサービスのパーソナルアシスタントへようこそ！私はいつでもご質問にお答えするため、最善を尽くします。',
    ko: '개인 맞춤형 FT 고객 서비스 어시스턴트에 오신 것을 환영합니다! 서비스에 관한 질문이 있으시면 최선을 다해 도와드리겠습니다.',
    pt: 'Bem-vindo ao seu assistente pessoal de Serviço de Atendimento ao Cliente da FT! Se você tiver alguma dúvida sobre nossos serviços, deixe-me fazer o meu melhor para ajudá-lo.',
    it: 'Benvenuto nel tuo assistente personale del Servizio Clienti FT! Se hai domande sui nostri servizi, lascia che faccia del mio meglio per assisterti.',
    'zh-TW': '歡迎使用專屬於您的FT客戶服務助理！您對於我們的服務有任何問題，讓我盡力為您解答。',
    'zh-HK': '歡迎使用專屬於您的FT客戶服務助理！您對於我們的服務有任何問題，讓我盡力為您解答。',
    ru: 'Добро пожаловать в вашего персонального помощника по обслуживанию клиентов FT! Если у вас есть вопросы по нашим услугам, позвольте мне сделать все возможное, чтобы помочь вам.'
  },
  'OtherReason': {
    en: 'Other reason',
    es: 'Otra razón',
    fr: 'Autre raison',
    de: 'Andere Gründe',
    ja: 'その他の理由',
    ko: '기타 이유',
    pt: 'Outro motivo',
    it: 'Altro motivo',
    'zh-TW': '其他原因',
    'zh-HK': '其他原因',
    zh: '其他原因',
    ru: 'Другая причина'
  },
  'Error': {
    en: 'Error',
    es: 'Error',
    fr: 'Erreur',
    de: 'Fehler',
    ja: 'エラー',
    ko: '오류',
    pt: 'Erro',
    it: 'Errore',
    'zh-TW': '錯誤',
    'zh-HK': '錯誤',
    zh: '错误',
    ru: 'Ошибка'
  },
  CustomerService: {
    zh: '帮助中心',
    en: 'Help Center',
    es: 'Centro de Ayuda',
    fr: `Centre d'Aide`,
    de: 'Hilfezentrum',
    ja: 'ヘルプセンター',
    ko: '도움말 센터',
    pt: 'Centro de Ajuda',
    it: 'Centro Assistenza',
    'zh-TW': '幫助中心',
    'zh-HK': '幫助中心',
    ru: 'Центр Поддержки'
  },
  'DailyEnglish': {
    zh: '每日英语',
    en: 'Daily English',
    es: 'Inglés Diario',
    fr: `Anglais Quotidien`,
    de: 'Tägliches Englisch',
    ja: 'デイリー・イングリッシュ',
    ko: '매일 영어',
    pt: 'Inglês Diário',
    it: 'Inglese Quotidiano',
    'zh-TW': '每日英語',
    'zh-HK': '每日英語',
    ru: 'Ежедневный Английский'
  },
  'FTAcademy': {
    zh: 'FT商学院',
    en: 'FT Academy',
    es: 'FT Academia',
    fr: `Académie FT`,
    de: 'FT Akademie',
    ja: 'FTアカデミー',
    ko: 'FT 아카데미',
    pt: 'Academia FT',
    it: 'Accademia FT',
    'zh-TW': 'FT商學院',
    'zh-HK': 'FT商學院',
    ru: 'Академия FT'
  },
  'VideoAudio': {
    zh: '视频与音频',
    en: 'Video and Audio',
    es: 'Video y Audio',
    fr: `Vidéo et Audio`,
    de: 'Video und Audio',
    ja: 'ビデオとオーディオ',
    ko: '비디오와 오디오',
    pt: 'Vídeo e Áudio',
    it: 'Video e Audio',
    'zh-TW': '影片和音頻',
    'zh-HK': '影片和音頻',
    ru: 'Видео и Аудио'
  },
  ProcessingRequest: {
    en: 'Your request is being processed...',
    zh: '您的请求正在处理中...',
    es: 'Su solicitud está siendo procesada...',
    fr: 'Votre demande est en cours de traitement...',
    de: 'Ihre Anfrage wird bearbeitet...',
    ja: 'リクエストを処理中です...',
    ko: '요청이 처리 중입니다...',
    pt: 'Sua solicitação está sendo processada...',
    it: 'La tua richiesta è in elaborazione...',
    'zh-TW': '您的請求正在處理中...',
    'zh-HK': '您的請求正在處理中...',
    ru: 'Ваш запрос обрабатывается...'
  },
  NEXT: {
    en: 'NEXT',
    zh: '下一步',
    es: 'SIGUIENTE',
    fr: 'SUIVANT',
    de: 'WEITER',
    ja: '次へ',
    ko: '다음',
    pt: 'PRÓXIMO', 
    it: 'PROSSIMO',
    'zh-TW': '下一步',
    'zh-HK': '下一步',
    ru: 'ДАЛЕЕ'
  },
  FINISH: {
    en: 'FINISH',
    zh: '完成',
    es: 'FINALIZAR',
    fr: 'TERMINER',
    de: 'BEENDEN',
    ja: '終了',
    ko: '완료',
    pt: 'FINALIZAR',
    it: 'FINIRE',
    'zh-TW': '完成',
    'zh-HK': '完成',
    ru: 'ЗАВЕРШИТЬ'
  },
  'Top News For Me': {
    zh: '有什么值得我关注的要闻吗？',
    en: 'What are the top news for me?',
    es: '¿Cuáles son las noticias principales para mí?',
    fr: 'Quelles sont les principales nouvelles pour moi ?',
    de: 'Was sind die Top-Nachrichten für mich?',
    ja: '私にとってのトップニュースは何ですか？',
    ko: '내게 맞는 주요 뉴스는 무엇인가요?',
    pt: 'Quais são as principais notícias para mim?',
    it: 'Quali sono le principali notizie per me?',
    'zh-TW': '有什麼值得我關注的要聞嗎？',
    'zh-HK': '有什麼值得我關注的要聞嗎？',
    ru: 'Какие главные новости для меня?'
  },
  'Most Popular': {
    zh: 'FT上有哪些热门文章？',
    en: 'What are the most popular articles on the FT?',
    es: '¿Cuáles son los artículos más populares en el FT?',
    fr: 'Quels sont les articles les plus populaires sur le FT?',
    de: 'Was sind die beliebtesten Artikel auf der FT?',
    ja: 'FTで最も人気のある記事は何ですか？',
    ko: 'FT에서 가장 인기 있는 기사는 무엇인가요?',
    pt: 'Quais são os artigos mais populares no FT?',
    it: 'Quali sono gli articoli più popolari su FT?',
    'zh-TW': 'FT上有哪些熱門文章？',
    'zh-HK': 'FT上有哪些熱門文章？',
    ru: 'Какие самые популярные статьи в FT?'
  },
  'MyFT': {
    zh: '我的FT',
    en: 'MyFT',
    es: 'MiFT',
    fr: 'MonFT',
    de: 'MeinFT',
    ja: 'マイFT',
    ko: '마이FT',
    pt: 'MeuFT',
    it: 'MioFT',
    'zh-TW': '我的FT',
    'zh-HK': '我的FT',
    ru: 'МойFT'
  },
  'chat_error_content_filter': {
  zh: `<p>我们的AI服务提供商返回了以下错误：</p><p>由于提示触发了Azure OpenAI的内容管理策略，响应被过滤。请修改您的提示并重试。</p><p>这可能不是您的错。但目前，请刷新并与我们的聊天机器人讨论其他事情。</p>`,
  en: `<p>Our AI service vendor returns the following error: </p><p>The response was filtered due to the prompt triggering Azure OpenAI’s content management policy. Please modify your prompt and retry. </p><p>It's probably not your fault. But for the time being, please refresh and talk about other things with our chat bot. </p>`,
  es: `<p>Nuestro proveedor de servicios de IA devolvió el siguiente error:</p><p>La respuesta fue filtrada debido a que el mensaje activó la política de gestión de contenido de Azure OpenAI. Por favor, modifique su mensaje e intente nuevamente.</p><p>Probablemente no sea su culpa. Pero por el momento, por favor recargue la página y hable de otras cosas con nuestro chat bot.</p>`,
  fr: `<p>Notre fournisseur de services IA a retourné l'erreur suivante :</p><p>La réponse a été filtrée en raison du déclenchement par la requête de la politique de gestion de contenu d'Azure OpenAI. Veuillez modifier votre requête et réessayer.</p><p>Ce n'est probablement pas de votre faute. Mais pour le moment, veuillez rafraîchir et parler d'autres choses avec notre chatbot.</p>`,
  de: `<p>Unser AI-Dienstanbieter hat den folgenden Fehler zurückgegeben:</p><p>Die Antwort wurde gefiltert, da die Eingabe die Inhaltsverwaltungsrichtlinie von Azure OpenAI ausgelöst hat. Bitte ändern Sie Ihre Eingabe und versuchen Sie es erneut.</p><p>Es ist wahrscheinlich nicht Ihre Schuld. Aber im Moment, bitte aktualisieren Sie und sprechen Sie mit unserem Chat-Bot über andere Dinge.</p>`,
  ja: `<p>当社のAIサービスベンダーは次のエラーを返しました：</p><p>プロンプトがAzure OpenAIのコンテンツ管理ポリシーをトリガーしたため、応答がフィルタリングされました。プロンプトを修正して再試行してください。</p><p>おそらくあなたのせいではありません。しかし、今のところ、リフレッシュして、チャットボットと他のことについて話してください。</p>`,
  ko: `<p>우리의 AI 서비스 제공업체가 다음 오류를 반환했습니다:</p><p>프롬프트가 Azure OpenAI의 콘텐츠 관리 정책을 트리거했기 때문에 응답이 필터링되었습니다. 프롬프트를 수정하고 다시 시도하십시오.</p><p>아마 당신의 잘못이 아닐 것입니다. 하지만 지금은 새로고침하고 채팅 봇과 다른 주제로 이야기해 주세요.</p>`,
  pt: `<p>Nosso fornecedor de serviços de IA retornou o seguinte erro:</p><p>A resposta foi filtrada devido ao acionamento da política de gerenciamento de conteúdo do Azure OpenAI. Por favor, modifique seu prompt e tente novamente.</p><p>Provavelmente não é sua culpa. Mas, por enquanto, por favor, atualize a página e fale sobre outras coisas com o nosso chat bot.</p>`,
  it: `<p>Il nostro fornitore di servizi AI ha restituito il seguente errore:</p><p>La risposta è stata filtrata a causa del prompt che ha attivato la politica di gestione dei contenuti di Azure OpenAI. Modifica il tuo prompt e riprova.</p><p>Probabilmente non è colpa tua. Ma per il momento, aggiorna la pagina e parla di altre cose con il nostro chat bot.</p>`,
  'zh-TW': `<p>我們的AI服務提供商返回了以下錯誤：</p><p>由於提示觸發了Azure OpenAI的內容管理策略，響應被過濾。請修改您的提示並重試。</p><p>這可能不是您的錯。但目前，請刷新並與我們的聊天機器人討論其他事情。</p>`,
  'zh-HK': `<p>我們的AI服務提供商返回了以下錯誤：</p><p>由於提示觸發了Azure OpenAI的內容管理策略，響應被過濾。請修改您的提示並重試。</p><p>這可能不是您的錯。但目前，請刷新並與我們的聊天機器人討論其他事情。</p>`,
  ru: `<p>Наш поставщик услуг AI возвращает следующую ошибку:</p><p>Ответ был отфильтрован из-за того, что запрос вызвал политику управления контентом Azure OpenAI. Пожалуйста, измените ваш запрос и попробуйте снова.</p><p>Вероятно, это не ваша вина. Но пока, пожалуйста, обновите страницу и поговорите с нашим чат-ботом о других вещах.</p>`
  },
  'chat_error_other': {
    zh: `AI模型返回了一个错误。可能是因为您的提示或我们提供的上下文。对不起，请刷新并尝试新的内容。`,
    en: `The AI model returns an error. It could be because of something in your prompt or the context that we provide. Sorry, please refresh and try something new.`,
    es: `El modelo de IA devuelve un error. Podría ser por algo en su solicitud o en el contexto que proporcionamos. Lo siento, por favor recargue e intente algo nuevo.`,
    fr: `Le modèle d'IA renvoie une erreur. Cela pourrait être dû à quelque chose dans votre requête ou dans le contexte que nous fournissons. Désolé, veuillez rafraîchir et essayer quelque chose de nouveau.`,
    de: `Das AI-Modell gibt einen Fehler zurück. Es könnte an etwas in Ihrer Eingabe oder dem von uns bereitgestellten Kontext liegen. Entschuldigung, bitte aktualisieren Sie die Seite und versuchen Sie etwas Neues.`,
    ja: `AIモデルがエラーを返します。あなたのプロンプトまたは私たちが提供するコンテキストに何か問題がある可能性があります。申し訳ありませんが、リフレッシュして新しいことを試してください。`,
    ko: `AI 모델이 오류를 반환합니다. 프롬프트나 제공된 컨텍스트에 무언가 문제가 있을 수 있습니다. 죄송합니다. 새로고침하고 새 시도를 해보세요.`,
    pt: `O modelo de IA retorna um erro. Pode ser por causa de algo em sua solicitação ou no contexto que fornecemos. Desculpe, por favor, atualize e tente algo novo.`,
    it: `Il modello di IA restituisce un errore. Potrebbe essere dovuto a qualcosa nella tua richiesta o nel contesto che forniamo. Scusa, aggiorna e prova qualcosa di nuovo.`,
    'zh-TW': `AI模型返回了一個錯誤。可能是因為您的提示或我們提供的上下文。對不起，請刷新並嘗試新的內容。`,
    'zh-HK': `AI模型返回了一個錯誤。可能是因為您的提示或我們提供的上下文。對不起，請刷新並嘗試新的內容。`,
    ru: `Модель AI возвращает ошибку. Это может быть связано с чем-то в вашем запросе или предоставленном нами контексте. Извините, пожалуйста, обновите страницу и попробуйте что-нибудь новое.`
  },
  'chat_error_network': {
    zh: `我们的AI服务提供商出现了问题，我们现在无法连接。请稍后再试。`,
    en: `Something is wrong with our AI vendor, we can't seem to connect to it right now. Please try later.`,
    es: `Hay un problema con nuestro proveedor de IA, no podemos conectarnos en este momento. Por favor, intente más tarde.`,
    fr: `Il y a un problème avec notre fournisseur de services IA, nous ne pouvons pas nous connecter pour le moment. Veuillez réessayer plus tard.`,
    de: `Es gibt ein Problem mit unserem AI-Anbieter, wir können momentan keine Verbindung herstellen. Bitte versuchen Sie es später erneut.`,
    ja: `AIサービス提供者に問題があり、現在接続できません。後でもう一度お試しください。`,
    ko: `AI 제공업체에 문제가 있어 현재 연결할 수 없습니다. 나중에 다시 시도해 주세요.`,
    pt: `Algo está errado com nosso fornecedor de IA, não conseguimos nos conectar no momento. Por favor, tente mais tarde.`,
    it: `C'è qualcosa che non va con il nostro fornitore di IA, al momento non possiamo connetterci. Per favore, riprova più tardi.`,
    'zh-TW': `我們的AI服務提供商出現了問題，我們現在無法連接。請稍後再試。`,
    'zh-HK': `我們的AI服務提供商出現了問題，我們現在無法連接。請稍後再試。`,
    ru: `Что-то не так с нашим поставщиком AI, мы не можем подключиться к нему прямо сейчас. Пожалуйста, попробуйте позже.`
  },
  'WaitMessage': {
    zh: '请等待[minutes]分钟再试。',
    en: 'Please wait [minutes] minutes before resuming.',
    es: 'Por favor, espere [minutes] minutos antes de reanudar.',
    fr: 'Veuillez patienter [minutes] minutes avant de reprendre.',
    de: 'Bitte warten Sie [minutes] Minuten, bevor Sie fortfahren.',
    ja: '[minutes]分間お待ちください。',
    ko: '[minutes]분 후에 다시 시도해주세요.',
    pt: 'Por favor, espere [minutes] minutos antes de retomar.',
    it: 'Si prega di attendere [minutes] minuti prima di riprendere.',
    'zh-TW': '請等待[minutes]分鐘再試。',
    'zh-HK': '請等待[minutes]分鐘再試。',
    ru: 'Пожалуйста, подождите [minutes] минут, прежде чем продолжить.'
  },
  'RateLimitReminder': {
    zh: '您好！您已达到我们的聊天限制，即每3小时30条消息。',
    en: 'Hi there! You’ve reached our chat limit of 30 messages every 3 hours.',
    es: '¡Hola! Ha alcanzado nuestro límite de 30 mensajes cada 3 horas.',
    fr: 'Bonjour! Vous avez atteint notre limite de 30 messages toutes les 3 heures.',
    de: 'Hallo! Sie haben unser Limit von 30 Nachrichten alle 3 Stunden erreicht.',
    ja: 'こんにちは！3時間ごとに30通のメッセージの制限に達しました。',
    ko: '안녕하세요! 3시간마다 30개의 메시지 제한에 도달했습니다.',
    pt: 'Olá! Você atingiu nosso limite de 30 mensagens a cada 3 horas.',
    it: 'Ciao! Hai raggiunto il nostro limite di 30 messaggi ogni 3 ore.',
    'zh-TW': '您好！您已達到我們的聊天限制，即每3小時30條消息。',
    'zh-HK': '您好！您已達到我們的聊天限制，即每3小時30條消息。',
    ru: 'Здравствуйте! Вы достигли нашего лимита в 30 сообщений каждые 3 часа.'
  },
  'OtherFeaturesReminder': {
    zh: '同时，您仍然可以查看您关注的最新新闻。感谢您的耐心等待！',
    en: 'Meanwhile, you can still check the latest news you’ve followed. Thanks for your patience!',
    es: 'Mientras tanto, aún puede consultar las últimas noticias que ha seguido. ¡Gracias por su paciencia!',
    fr: 'En attendant, vous pouvez toujours consulter les dernières nouvelles que vous suivez. Merci pour votre patience!',
    de: 'In der Zwischenzeit können Sie die neuesten Nachrichten, denen Sie folgen, weiterhin überprüfen. Vielen Dank für Ihre Geduld!',
    ja: 'その間、フォローしている最新ニュースを確認できます。ご理解いただきありがとうございます！',
    ko: '그동안 팔로우한 최신 뉴스를 확인할 수 있습니다. 감사합니다!',
    pt: 'Enquanto isso, você ainda pode verificar as últimas notícias que seguiu. Obrigado pela sua paciência!',
    it: 'Nel frattempo, puoi comunque controllare le ultime notizie che hai seguito. Grazie per la tua pazienza!',
    'zh-TW': '同時，您仍然可以查看您關注的最新新聞。感謝您的耐心等待！',
    'zh-HK': '同時，您仍然可以查看您關注的最新新聞。感謝您的耐心等待！',
    ru: 'Тем временем вы можете проверить последние новости, которые вы следили. Спасибо за ваше терпение!'
  },
  'BackToTop': {
    zh: '从头开始',
    en: 'Start Over',
    es: 'Empezar de nuevo',
    fr: 'Recommencer',
    de: 'Von vorn beginnen',
    ja: '最初からやり直す',
    ko: '처음부터 다시 시작',
    pt: 'Começar de novo',
    it: 'Ricominciare',
    'zh-TW': '',
    'zh-HK': '',
    ru: 'Начать заново'
  },
  'DiscussArticle': {
    zh: '讨论文章',
    en: 'Discuss Article',
    es: 'Discutir Artículo',
    fr: `Discuter de l'article`,
    de: 'Artikel diskutieren',
    ja: '記事を議論する',
    ko: '글 논의',
    pt: 'Discutir Artigo',
    it: 'Discutere Articolo',
    'zh-TW': '討論文章',
    'zh-HK': '討論文章',
    ru: 'Обсудить статью'
  },
  'ChangeSubject': {
    en: 'Change Subject',
    zh:'改变话题',
    es: 'Cambiar Tema',
    fr: 'Changer de sujet',
    de: 'Thema ändern',
    ja: '話題を変える',
    ko: '주제 변경',
    pt: 'Mudar de assunto',
    it: 'Cambiare argomento',
    'zh-TW': '改變話題',
    'zh-HK': '改變話題',
    ru: 'Изменить тему'
  },
  'socratic': {
    zh: '苏格拉底诘问',
    en: 'Socratic Method',
    es: 'Método socrático',
    fr: 'Méthode socratique',
    de: 'Sokratische Methode',
    ja: 'ソクラテス式問い掛け法',
    ko: '소크라테스적 방법',
    pt: 'Método socrático',
    it: 'Metodo socratico',
    'zh-TW': '蘇格拉底質問',
    'zh-HK': '蘇格拉底式質問',
    ru: 'Сократический метод'
  },
  'SearchFTAPI': {
    en: 'Search FT',
    es: 'Buscar FT',
    fr: 'Rechercher FT',
    de: 'FT suchen',
    ja: 'FT検索',
    ko: 'FT 검색',
    pt: 'Pesquisar FT',
    it: 'Cerca FT',
    'zh-TW': '搜尋FT',
    'zh-HK': '搜尋FT',
    zh: '搜索FT',
    ru: 'Поиск FT'
  },
  'DiscussContent': {
    en: 'Talk about FT Content',
    zh: '讨论FT内容',
    es: 'Hablar sobre el contenido de FT',
    fr: 'Parler du contenu de FT',
    de: 'Über FT-Inhalte sprechen',
    ja: 'FTコンテンツについて話す',
    ko: 'FT 콘텐츠에 대해 이야기하다',
    pt: 'Falar sobre o conteúdo do FT',
    it: 'Parlare del contenuto di FT',
    'zh-TW': '討論FT內容',
    'zh-HK': '討論FT內容',
    ru: 'Обсуждение контента FT'
  },
  'Other': {
    en: 'Just Chat',
    es: 'Solo Chat',
    fr: 'Discussion',
    de: 'Nur Chatten',
    ja: 'チャットのみ',
    ko: '채팅만',
    pt: 'Apenas Bate-papo',
    it: 'Solo Chat',
    'zh-TW': '隨便聊聊',
    'zh-HK': '隨便聊聊',
    zh: '随便聊聊',
    ru: 'Просто чат'
  },
  Detail: {
    en: 'Detail',
    es: 'Detalle',
    fr: 'Détail',
    de: 'Detail',
    ja: '詳細',
    ko: '세부사항',
    pt: 'Detalhe',
    it: 'Dettaglio',
    'zh-TW': '詳細',
    'zh-HK': '詳情',
    zh: '详情',
    ru: 'Детали'
  },
  DetectingIntent: {
    zh: '正在尝试理解您的问题',
    en: 'Attempting to understand your question',
    es: 'Intentando entender su pregunta',
    fr: 'Tentative de comprendre votre question',
    de: 'Versuche, Ihre Frage zu verstehen',
    ja: 'ご質問を理解しようとしています',
    ko: '질문을 이해하려고 시도 중',
    pt: 'Tentando entender sua pergunta',
    it: 'Tentativo di capire la tua domanda',
    'zh-TW': '正在嘗試理解您的問題',
    'zh-HK': '正在嘗試理解您的問題',
    ru: 'Пытаюсь понять ваш вопрос'
  },
  GettingContext: {
    zh: '正在检查相关信息',
    en: 'Checking relevant information',
    es: 'Revisando la información relevante',
    fr: 'Vérification des informations pertinentes',
    de: 'Überprüfen relevanter Informationen',
    ja: '関連情報を確認しています',
    ko: '관련 정보 확인 중',
    pt: 'Verificando informações relevantes',
    it: 'Verificando le informazioni pertinenti',
    'zh-TW': '正在檢查相關信息',
    'zh-HK': '正在檢查相關信息',
    ru: 'Проверка соответствующей информации'
  },
  GettingContextFromDB: {
    zh: '在相关档案中进行检索，寻找和您的问题相关的内容',
    en: 'Retrieving from the database, looking for content related to your question',
    es: 'Recuperando de la base de datos, buscando contenido relacionado con su pregunta',
    fr: 'Récupération de la base de données, recherche de contenu lié à votre question',
    de: 'Abrufen aus der Datenbank, Suche nach Inhalten, die sich auf Ihre Frage beziehen',
    ja: 'データベースから情報を取得して、ご質問に関連する内容を探しています',
    ko: '데이터베이스에서 검색하여 귀하의 질문과 관련된 내용을 찾고 있습니다',
    pt: 'Recuperando do banco de dados, procurando conteúdo relacionado à sua pergunta',
    it: 'Recuperando dal database, cercando contenuti relativi alla tua domanda',
    'zh-TW': '在相關檔案中進行檢索，尋找和您的問題相關的內容',
    'zh-HK': '在相關檔案中進行檢索，尋找和您的問題相關的內容',
    ru: 'Извлекаю из базы данных, ищу содержание, связанное с вашим вопросом'
  },
  GettingResult: {
    zh: '让我再想一下',
    en: 'Let me think again',
    es: 'Déjame pensar de nuevo',
    fr: 'Laissez-moi réfléchir à nouveau',
    de: 'Lass mich nochmal überlegen',
    ja: 'もう一度考えさせてください',
    ko: '다시 생각해보겠습니다',
    pt: 'Deixe-me pensar novamente',
    it: 'Fammi pensare ancora',
    'zh-TW': '讓我再想一下',
    'zh-HK': '讓我再想一下',
    ru: 'Дайте мне подумать еще раз'
  },
  "Checking Content": {
    "en": "Checking Content...",
    "zh": "检查内容...",
    "es": "Verificando contenido...",
    "fr": "Vérification du contenu...",
    "de": "Inhalt prüfen...",
    "ja": "コンテンツを確認中...",
    "ko": "내용 확인 중...",
    "pt": "Verificando conteúdo...",
    "it": "Verifica del contenuto...",
    "zh-TW": "檢查內容...",
    "zh-HK": "檢查內容...",
    "ru": "Проверка содержимого..."
  },
  'No Search Result': {
    "zh": "很抱歉，没有找到符合条件的结果，请您更换一下搜索的条件，或者今后再试。",
    "en": "Sorry, no results were found that match your criteria. Please try changing your search terms or try again later.",
    "es": "Lo sentimos, no se encontraron resultados que coincidan con sus criterios. Intente cambiar sus términos de búsqueda o inténtelo de nuevo más tarde.",
    "fr": "Désolé, aucun résultat correspondant à vos critères n'a été trouvé. Veuillez essayer de modifier vos termes de recherche ou réessayez plus tard.",
    "de": "Entschuldigung, es wurden keine Ergebnisse gefunden, die Ihren Kriterien entsprechen. Bitte versuchen Sie, Ihre Suchbegriffe zu ändern oder versuchen Sie es später noch einmal.",
    "ja": "申し訳ありませんが、条件に合う結果が見つかりませんでした。検索条件を変更するか、後でもう一度試してください。",
    "ko": "죄송합니다, 기준에 맞는 결과를 찾을 수 없습니다. 검색 조건을 변경하거나 나중에 다시 시도해주세요.",
    "pt": "Desculpe, não foram encontrados resultados que correspondam aos seus critérios. Tente alterar os termos da pesquisa ou tente novamente mais tarde.",
    "it": "Spiacente, non sono stati trovati risultati che corrispondono ai tuoi criteri. Si prega di provare a cambiare i termini di ricerca o riprovare più tardi.",
    "zh-TW": "很抱歉，沒有找到符合條件的結果，請您更換一下搜索的條件，或者今後再試。",
    "zh-HK": "很抱歉，沒有找到符合條件的結果，請您更換一下搜尋的條件，或者稍後再試。",
    "ru": "Извините, не найдено результатов, соответствующих вашим критериям. Пожалуйста, попробуйте изменить условия поиска или повторите попытку позже."
  },
  'Final Score': {
    en: 'Final Score',
    es: 'Puntuación Final',
    fr: 'Score Final',
    de: 'Endstand',
    ja: '最終スコア',
    ko: '최종 점수',
    pt: 'Pontuação Final',
    it: 'Punteggio Finale',
    'zh-TW': '最終得分',
    'zh-HK': '最終得分',
    zh: '最终得分',
    ru: 'Итоговый счет'
  },
  'Ask Me': {
    en: 'Ask me about the latest in markets, economics, and politics...',
    es: 'Pregúntame sobre lo último en mercados, economía y política...',
    fr: `Demandez-moi les dernières nouvelles sur les marchés, l'économie et la politique...`,
    de: `Fragen Sie mich nach den neuesten Entwicklungen in Märkten, Wirtschaft und Politik...`,
    ja: '市場、経済、政治の最新情報について聞いてください...',
    ko: '시장, 경제, 정치에 대한 최신 정보를 물어보세요...',
    pt: 'Pergunte-me sobre as últimas novidades em mercados, economia e política...',
    it: `Chiedimi delle ultime novità sui mercati, l'economia e la politica...`,
    'zh-TW': '問我關於市場、經濟和政治的最新消息...',
    'zh-HK': '問我關於市場、經濟和政治的最新消息...',
    zh: '问我关于市场、经济和政治的最新消息...',
    ru: 'Спросите меня о последних новостях на рынках, в экономике и политике...'
  },
  share_quiz: {
    en: 'The link has been copied to your clipboard! 🎉\n\nDon’t forget to share it with your friends!',
    es: '¡El enlace se ha copiado a tu portapapeles! 🎉\n\n¡No olvides compartirlo con tus amigos!',
    fr: 'Le lien a été copié dans votre presse-papiers ! 🎉\n\nN’oubliez pas de le partager avec vos amis !',
    de: 'Der Link wurde in Ihre Zwischenablage kopiert! 🎉\n\nVergessen Sie nicht, ihn mit Ihren Freunden zu teilen!',
    ja: 'リンクがクリップボードにコピーされました！ 🎉\n\n友達と共有するのを忘れないでください！',
    ko: '링크가 클립보드에 복사되었습니다! 🎉\n\n친구들과 공유하는 것을 잊지 마세요!',
    pt: 'O link foi copiado para a sua área de transferência! 🎉\n\nNão se esqueça de compartilhá-lo com seus amigos!',
    it: 'Il link è stato copiato negli appunti! 🎉\n\nNon dimenticare di condividerlo con i tuoi amici!',
    'zh-TW': '連結已複製到您的剪貼簿！🎉\n\n別忘了分享給您的朋友！',
    'zh-HK': '連結已複製到您的剪貼簿！🎉\n\n別忘了分享給您的朋友！',
    zh: '链接已复制到您的剪贴板！🎉\n\n别忘了分享给您的朋友！',
    ru: 'Ссылка скопирована в ваш буфер обмена! 🎉\n\nНе забудьте поделиться ею с друзьями!'
  },
  'byline': {
    zh: '作者',
    en: 'author',
    es: 'autor',
    fr: 'auteur',
    de: 'Autor',
    ja: '著者',
    ko: '저자',
    pt: 'autor',
    it: 'autore',
    'zh-TW': '作者',
    'zh-HK': '作者',
    ru: 'автор'
},
correct_vs_all: {
  zh: '答对个数',
  en: 'Correct vs. All',
  es: 'Correcto vs. Total',
  fr: 'Correct vs. Total',
  de: 'Korrekt vs. Gesamt',
  ja: '正解数対全数',
  ko: '정답 vs 전체',
  pt: 'Correto vs. Total',
  it: 'Corretto vs. Totale',
  'zh-TW': '答對個數',
  'zh-HK': '答對個數',
  ru: 'Правильные против Всего'
},
'Add to Calendar': {
  zh: '添加到日历',
  en: 'Add to Calendar',
  es: 'Añadir al calendario',
  fr: 'Ajouter au calendrier',
  de: 'Zum Kalender hinzufügen',
  ja: 'カレンダーに追加',
  ko: '캘린더에 추가',
  pt: 'Adicionar ao Calendário',
  it: 'Aggiungi al Calendario',
  'zh-TW': '添加到日曆',
  'zh-HK': '添加到日曆',
  ru: 'Добавить в календарь'
},
average_score: {
  en: 'Average Score',
  es: 'Puntuación media',
  fr: 'Score moyen',
  de: 'Durchschnittliche Punktzahl',
  ja: '平均スコア',
  ko: '평균 점수',
  pt: 'Pontuação Média',
  it: 'Punteggio medio',
  'zh-TW': '平均分數',
  'zh-HK': '平均分數',
  zh: '平均分数',
  ru: 'Средний балл'
},
statistics: {
  en: 'Statistics',
  es: 'Estadísticas',
  fr: 'Statistiques',
  de: 'Statistiken',
  ja: '統計',
  ko: '통계',
  pt: 'Estatísticas',
  it: 'Statistiche',
  'zh-TW': '統計',
  'zh-HK': '統計',
  zh: '统计',
  ru: 'Статистика'
},
played: {
  zh: '参与次数',
  en: 'Played',
  es: 'Jugado',
  fr: 'Joué',
  de: 'Gespielt',
  ja: 'プレイ済み',
  ko: '플레이한 게임 수',
  pt: 'Jogadas',
  it: 'Giocato',
  'zh-TW': '參與次數',
  'zh-HK': '參與次數',
  ru: 'Сыграно'
},
current_streak: {
  en: 'Current Streak',
  es: 'Racha actual',
  fr: 'Série en cours',
  de: 'Aktuelle Serie',
  ja: '現在の連勝記録',
  ko: '현재 연승',
  pt: 'Sequência Atual',
  it: 'Serie attuale',
  'zh-TW': '當前連勝',
  'zh-HK': '當前連勝',
  zh: '当前连胜',
  ru: 'Текущая серия'
},
max_streak: {
  en: 'Max Streak',
  es: 'Racha máxima',
  fr: 'Meilleure série',
  de: 'Maximale Serie',
  ja: '最大連勝記録',
  ko: '최대 연승',
  pt: 'Sequência Máxima',
  it: 'Massima serie',
  'zh-TW': '最高連勝',
  'zh-HK': '最高連勝',
  zh: '最高连胜',
  ru: 'Максимальная серия'
},
'people': {
    en: 'people',
    zh: '人物',
    es: 'personas',
    fr: 'personnes',
    de: 'Personen',
    ja: '人々',
    ko: '사람들',
    pt: 'pessoas',
    it: 'persone',
    'zh-TW': '人物',
    'zh-HK': '人物',
    ru: 'люди'
},
'organisations': {
    en: 'organisation',
    zh: '组织',
    es: 'organización',
    fr: 'organisation',
    de: 'Organisation',
    ja: '組織',
    ko: '조직',
    pt: 'organização',
    it: 'organizzazione',
    'zh-TW': '組織',
    'zh-HK': '組織',
    ru: 'организация'
},
'brand': {
  en: 'brand',
  zh: '品牌',
  es: 'marca',
  fr: 'marque',
  de: 'Marke',
  ja: 'ブランド',
  ko: '브랜드',
  pt: 'marca',
  it: 'marca',
  'zh-TW': '品牌',
  'zh-HK': '品牌',
  ru: 'бренд'
},
'genre': {
  en: 'genre',
  zh: '类型',
  es: 'género',
  fr: 'genre',
  de: 'Genre',
  ja: 'ジャンル',
  ko: '장르',
  pt: 'gênero',
  it: 'genere',
  'zh-TW': '類型',
  'zh-HK': '類型',
  ru: 'жанр'
},
'Sectors': {
  'en': 'Sectors',
  'zh': '板块',
  'es': 'Sectores',
  'fr': 'Secteurs',
  'de': 'Sektoren',
  'ja': 'セクター',
  'ko': '섹터',
  'pt': 'Setores',
  'it': 'Settori',
  'zh-TW': '板塊',
  'zh-HK': '板塊',
  'ru': 'Сектора'
},
Genres: {
  'en': 'Genres',
  'zh': '类型',
  'es': 'Géneros',
  'fr': 'Genres',
  'de': 'Genres',
  'ja': 'ジャンル',
  'ko': '장르',
  'pt': 'Gêneros',
  'it': 'Generi',
  'zh-TW': '類型',
  'zh-HK': '類型',
  'ru': 'Жанры'
},
'<!--space-->': {
  en: ' ',
  zh: '',
  es: ' ',
  fr: ' ',
  de: ' ',
  ja: '',
  ko: '',
  pt: ' ',
  it: ' ',
  'zh-TW': '',
  'zh-HK': '',
  ru: ' '
},
'Quiz Me': {
    en: 'Test my understanding',
    es: 'Prueba mi comprensión',
    fr: 'Testez ma compréhension',
    de: 'Teste mein Verständnis',
    ja: '私の理解をテストしてください',
    ko: '내 이해를 테스트하세요',
    pt: 'Teste a minha compreensão',
    it: 'Testa la mia comprensione',
    'zh-TW': '測試我的理解',
    'zh-HK': '測試我的理解',
    zh: '测试我的理解',
    ru: 'Проверьте мое понимание'
  },
  'Socratic Method': {
    zh: '苏格拉底诘问',
    en: 'Socratic method',
    es: 'Método socrático',
    fr: 'Méthode socratique',
    de: 'Sokratische Methode',
    ja: 'ソクラテス式問い掛け法',
    ko: '소크라테스적 방법',
    pt: 'Método socrático',
    it: 'Metodo socratico',
    'zh-TW': '蘇格拉底質問',
    'zh-HK': '蘇格拉底式質問',
    ru: 'Сократический метод'
  },
  'Socratic Method Explained': {
    en: 'The Socratic Method is a way of questioning and discussing ideas to challenge assumptions and arrive at a better understanding. It involves asking questions to uncover underlying beliefs and test the logic of responses given. It is used to promote critical thinking, problem-solving, and creativity in various fields.',
    zh: '苏格拉底诘问方法是一种质疑和讨论观念的方式，旨在挑战假设并达到更好的理解。它涉及提出问题以揭示潜在信念并测试所给出的响应的逻辑。它用于在各个领域中促进批判性思维、问题解决和创造力。',
    es: 'El Método Socrático es una forma de cuestionar y discutir ideas para desafiar suposiciones y llegar a una mejor comprensión. Implica hacer preguntas para descubrir creencias subyacentes y poner a prueba la lógica de las respuestas dadas. Se utiliza para promover el pensamiento crítico, la resolución de problemas y la creatividad en varios campos.',
    fr: 'La méthode socratique est une façon de questionner et de discuter des idées afin de remettre en question les hypothèses et parvenir à une meilleure compréhension. Elle consiste à poser des questions pour découvrir les croyances sous-jacentes et tester la logique des réponses données. Elle est utilisée pour promouvoir la pensée critique, la résolution de problèmes et la créativité dans différents domaines.',
    de: 'Die sokratische Methode ist eine Art des Fragens und Diskutierens von Ideen, um Annahmen herauszufordern und zu einem besseren Verständnis zu gelangen. Es beinhaltet Fragen, um zugrunde liegende Überzeugungen aufzudecken und die Logik der gegebenen Antworten zu prüfen. Es wird verwendet, um kritisches Denken, Problemlösung und Kreativität in verschiedenen Bereichen zu fördern.',
    ja: 'ソクラテスの方法は、仮定に挑戦し、より良い理解に到達するための問いかけと議論の方法です。潜在的な信念を明らかにするために質問し、回答の論理をテストすることが含まれます。さまざまな分野で批判的思考、問題解決、創造性を促進するために使用されています。',
    ko: '소크라테스 방법은 가정을 도전하고 더 나은 이해를 도출하기 위해 아이디어에 대한 질문과 토론하는 방법입니다. 대답의 논리를 검증하고, 깔끔하지 않은 논리를 깨우쳐 신념을 드러냅니다. 다양한 분야에서 비판적 사고, 문제 해결 및 창의성을 촉진하는 데 사용됩니다.',
    pt: 'O Método Socrático é uma maneira de questionar e discutir ideias para desafiar pressupostos e chegar a uma melhor compreensão. Envolve fazer perguntas para descobrir crenças subjacentes e testar a lógica das respostas dadas. É usado para promover o pensamento crítico, a resolução de problemas e a criatividade em várias áreas.',
    it: `Il Metodo Socratico è un modo di interrogare e discutere le idee per mettere in discussione le assunzioni e arrivare ad una migliore comprensione. Comprende l'arte di porre domande per scoprire le credenze sottostanti e testare la logica delle risposte date. È utilizzato per promuovere il pensiero critico, la risoluzione dei problemi e la creatività in vari campi.`,
    'zh-TW': '蘇格拉底詰問方法是一種質疑和討論觀念的方式，旨在挑戰假設並達到更好的理解。它涉及提出問題以揭示潛在信念並測試所給出的回應的邏輯。它用於在各個領域中促進批判性思維、問題解決和創造力。',
    'zh-HK': '蘇格拉底詰問方法是一種質疑和討論觀念的方式，旨在挑戰假設並達到更好的理解。它涉及提出問題以揭示潛在信念並測試所給出的回應的邏輯。它用於在各個領域中促進批判性思維、問題解決和創造力。',
    ru: 'Метод Сократа - это способ вопросительного и обсуждающего подхода к идеям, направленный на оспаривание предположений и достижение лучшего понимания. Он включает в себя задавание вопросов, чтобы раскрыть скрытые убеждения и проверить логику полученных ответов. Используется для развития критического мышления, решения проблем и креативности в различных областях.'
  },
  Regions: {
    en: 'Regions',
    es: 'Regiones',
    fr: 'Régions',
    de: 'Regionen',
    ja: '地域',
    ko: '지역',
    pt: 'Regiões',
    it: 'Regioni',
    ru: 'Регионы',
    'zh-TW': '地區',
    'zh-HK': '地區',
    zh: '地区'
  },
'Columns': {
    en: 'Columns',
    es: 'Columnas',
    fr: 'Colonnes',
    de: 'Spalten',
    ja: 'カラム',
    ko: '컬럼',
    pt: 'Colunas',
    it: 'Colonne',
    ru: 'Колонки',
    'zh-TW': '欄目',
    'zh-HK': '欄目',
    zh: '栏目'
  },
  'Authors': {
    en: 'Authors',
    es: 'Autores',
    fr: 'Auteurs',
    de: 'Autoren',
    ja: '著者',
    ko: '저자',
    pt: 'Autores',
    it: 'Autori',
    ru: 'Авторы',
    'zh-TW': '作者',
    'zh-HK': '作者',
    zh: '作者'
  },

  'The Big Read': {
    en: 'The Big Read',
    es: 'La Gran Lectura',
    fr: 'La Grande Lecture',
    de: 'Die Große Lektüre',
    ja: 'FT大視野',
    ko: 'FT 대시야',
    pt: 'A Grande Leitura',
    it: 'La Grande Lettura',
    ru: 'Большое чтение',
    'zh-TW': 'FT大視野',
    'zh-HK': 'FT大視野',
    zh: 'FT大视野'
  },
  'FT Magazine': {
    en: 'FT Magazine',
    es: 'Revista FT',
    fr: 'Magazine FT',
    de: 'FT Magazin',
    ja: 'FT杂志',
    ko: 'FT 잡지',
    pt: 'Revista FT',
    it: 'Rivista FT',
    ru: 'Журнал FT',
    'zh-TW': 'FT雜誌',
    'zh-HK': 'FT雜誌',
    zh: 'FT杂志'
  },
  'Lunch with the FT': {
    en: 'Lunch with the FT',
    es: 'Almuerzo con el FT',
    fr: 'Déjeuner avec le FT',
    de: 'Mittagessen mit dem FT',
    ja: '与FT共进午餐',
    ko: 'FT와의 점심',
    pt: 'Almoço com o FT',
    it: 'Pranzo con il FT',
    ru: 'Обед с FT',
    'zh-TW': '與FT共進午餐',
    'zh-HK': '與FT共進午餐',
    zh: '与FT共进午餐'
  },
  'Unhedged': {
    en: 'Unhedged',
    es: 'Sin Cobertura',
    fr: 'Non Couvert',
    de: 'Ungehedges',
    ja: '风险管不管',
    ko: '헤지되지 않음',
    pt: 'Sem Cobertura',
    it: 'Non Coperto',
    ru: 'Без Хеджирования',
    'zh-TW': '風險管不管',
    'zh-HK': '風險管不管',
    zh: '风险管不管'
  },
  'Undercover Economist': {
    en: 'Undercover Economist',
    es: 'Economista Encubierto',
    fr: 'Économiste Sous Couverture',
    de: 'Undercover-Ökonom',
    ja: '卧底经济学家',
    ko: '은밀한 경제학자',
    pt: 'Economista Disfarçado',
    it: 'Economista Undercover',
    ru: 'Экономист под прикрытием',
    'zh-TW': '臥底經濟學家',
    'zh-HK': '臥底經濟學家',
    zh: '卧底经济学家'
  },
  'The Weekend Essay': {
    en: 'The Weekend Essay',
    es: 'El Ensayo de Fin de Semana',
    fr: `L'Essai du Weekend`,
    de: 'Der Wochenend-Essay',
    ja: '周末随笔',
    ko: '주말 에세이',
    pt: 'O Ensaio do Fim de Semana',
    it: 'Il Saggio del Weekend',
    ru: 'Эссе на выходные',
    'zh-TW': '周末隨筆',
    'zh-HK': '周末隨筆',
    zh: '周末随笔'
  },
  'techAsia': {
    en: 'techAsia',
    es: 'techAsia',
    fr: 'techAsia',
    de: 'techAsia',
    ja: '亚洲科技聚焦',
    ko: '아시아 기술 집중',
    pt: 'techAsia',
    it: 'techAsia',
    ru: 'techAsia',
    'zh-TW': '亞洲科技聚焦',
    'zh-HK': '亞洲科技聚焦',
    zh: '亚洲科技聚焦'
  },
  'Martin Wolf': {
    en: 'Martin Wolf',
    es: 'Martin Wolf',
    fr: 'Martin Wolf',
    de: 'Martin Wolf',
    ja: 'マーティン•ウルフ',
    ko: '마틴 울프',
    pt: 'Martin Wolf',
    it: 'Martin Wolf',
    ru: 'Мартин Вольф',
    'zh-TW': '馬丁•沃爾夫',
    'zh-HK': '馬丁•沃爾夫',
    zh: '马丁•沃尔夫'
  },
  "Gillian Tett": {
    en: "Gillian Tett",
    zh: "吉莲•邰蒂",
    es: "Gillian Tett",
    fr: "Gillian Tett",
    de: "Gillian Tett",
    ja: "ギリアン・テット",
    ko: "질리언 테트",
    pt: "Gillian Tett",
    it: "Gillian Tett",
    'zh-TW': "吉蓮•邰蒂",
    'zh-HK': "吉蓮•邰蒂",
    ru: "Джиллиан Тетт"
  },
  'Madhumita Murgia': {
    en: 'Madhumita Murgia',
    es: 'Madhumita Murgia',
    fr: 'Madhumita Murgia',
    de: 'Madhumita Murgia',
    ja: 'マドゥミタ•ムルジア',
    ko: '마두미타 무르지아',
    pt: 'Madhumita Murgia',
    it: 'Madhumita Murgia',
    ru: 'Мадхумита Мургия',
    'zh-TW': '馬杜米塔•穆爾賈',
    'zh-HK': '馬杜米塔•穆爾賈',
    zh: '马杜米塔•穆尔贾'
  },
  'Richard Waters': {
    en: 'Richard Waters',
    es: 'Richard Waters',
    fr: 'Richard Waters',
    de: 'Richard Waters',
    ja: 'リチャード•ウォーターズ',
    ko: '리처드 워터스',
    pt: 'Richard Waters',
    it: 'Richard Waters',
    ru: 'Ричард Уотерс',
    'zh-TW': '理查德•沃特斯',
    'zh-HK': '理查德•沃特斯',
    zh: '理查德•沃特斯'
  },
  'Mohamed El-Erian': {
    en: 'Mohamed El-Erian',
    es: 'Mohamed El-Erian',
    fr: 'Mohamed El-Erian',
    de: 'Mohamed El-Erian',
    ja: 'モハメド•エル＝エリアン',
    ko: '모하메드 엘-에리안',
    pt: 'Mohamed El-Erian',
    it: 'Mohamed El-Erian',
    ru: 'Мохамед Эль-Эриан',
    'zh-TW': '穆罕默德•埃里安',
    'zh-HK': '穆罕默德•埃里安',
    zh: '穆罕默德•埃里安'
  },

  'Stephen Roach': {
    en: 'Stephen Roach',
    es: 'Stephen Roach',
    fr: 'Stephen Roach',
    de: 'Stephen Roach',
    ja: 'スティーブン•ローチ',
    ko: '스티븐 로치',
    pt: 'Stephen Roach',
    it: 'Stephen Roach',
    ru: 'Стивен Роуч',
    'zh-TW': '史蒂芬•羅奇',
    'zh-HK': '史蒂芬•羅奇',
    zh: '史蒂芬•罗奇'
  },

  'Lawrence Summers': {
    en: 'Lawrence Summers',
    es: 'Lawrence Summers',
    fr: 'Lawrence Summers',
    de: 'Lawrence Summers',
    ja: 'ローレンス•サマーズ',
    ko: '로렌스 서머스',
    pt: 'Lawrence Summers',
    it: 'Lawrence Summers',
    ru: 'Лоуренс Саммерс',
    'zh-TW': '勞倫斯•薩默斯',
    'zh-HK': '勞倫斯•薩默斯',
    zh: '劳伦斯•萨默斯'
  },

  'Robert Armstrong': {
    en: 'Robert Armstrong',
    es: 'Robert Armstrong',
    fr: 'Robert Armstrong',
    de: 'Robert Armstrong',
    ja: 'ロバート•アームストロング',
    ko: '로버트 암스트롱',
    pt: 'Robert Armstrong',
    it: 'Robert Armstrong',
    ru: 'Роберт Армстронг',
    'zh-TW': '羅伯特•阿姆斯特朗',
    'zh-HK': '羅伯特•阿姆斯特朗',
    zh: '罗伯特•阿姆斯特朗'
  },

  'George Soros': {
    en: 'George Soros',
    es: 'George Soros',
    fr: 'George Soros',
    de: 'George Soros',
    ja: 'ジョージ•ソロス',
    ko: '조지 소로스',
    pt: 'George Soros',
    it: 'George Soros',
    ru: 'Джордж Сорос',
    'zh-TW': '喬治•索羅斯',
    'zh-HK': '喬治•索羅斯',
    zh: '乔治•索罗斯'
  },

  China: {
    en: 'China',
    es: 'China',
    fr: 'Chine',
    de: 'China',
    ja: '中国',
    ko: '중국',
    pt: 'China',
    it: 'Cina',
    ru: 'Китай',
    'zh-TW': '中國',
    'zh-HK': '中國',
    zh: '中国'
  },
  'United States': {
    en: 'United States',
    es: 'Estados Unidos',
    fr: 'États-Unis',
    de: 'Vereinigte Staaten',
    ja: 'アメリカ合衆国',
    ko: '미국',
    pt: 'Estados Unidos',
    it: 'Stati Uniti',
    ru: 'Соединённые Штаты',
    zh: '美国',
    'zh-TW': '美國',
    'zh-HK': '美國'
  },
  'Japan': {
    en: 'Japan',
    es: 'Japón',
    fr: 'Japon',
    de: 'Japan',
    ja: '日本',
    ko: '일본',
    pt: 'Japão',
    it: 'Giappone',
    ru: 'Япония',
    zh: '日本',
    'zh-TW': '日本',
    'zh-HK': '日本'
  },
  'Germany': {
    en: 'Germany',
    es: 'Alemania',
    fr: 'Allemagne',
    de: 'Deutschland',
    ja: 'ドイツ',
    ko: '독일',
    pt: 'Alemanha',
    it: 'Germania',
    ru: 'Германия',
    zh: '德国',
    'zh-TW': '德國',
    'zh-HK': '德國'
  },
  'India': {
    en: 'India',
    es: 'India',
    fr: 'Inde',
    de: 'Indien',
    ja: 'インド',
    ko: '인도',
    pt: 'Índia',
    it: 'India',
    ru: 'Индия',
    zh: '印度',
    'zh-TW': '印度',
    'zh-HK': '印度'
  },
  'France': {
    en: 'France',
    es: 'Francia',
    fr: 'France',
    de: 'Frankreich',
    ja: 'フランス',
    ko: '프랑스',
    pt: 'França',
    it: 'Francia',
    ru: 'Франция',
    zh: '法国',
    'zh-TW': '法國',
    'zh-HK': '法國'
  },
  'Brazil': {
    en: 'Brazil',
    es: 'Brasil',
    fr: 'Brésil',
    de: 'Brasilien',
    ja: 'ブラジル',
    ko: '브라질',
    pt: 'Brasil',
    it: 'Brasile',
    ru: 'Бразилия',
    zh: '巴西',
    'zh-TW': '巴西',
    'zh-HK': '巴西'
  },
  'Russia': {
    en: 'Russia',
    es: 'Rusia',
    fr: 'Russie',
    de: 'Russland',
    ja: 'ロシア',
    ko: '러시아',
    pt: 'Rússia',
    it: 'Russia',
    ru: 'Россия',
    zh: '俄罗斯',
    'zh-TW': '俄國',
    'zh-HK': '俄羅斯'
  },  
  'Canada': {
    en: 'Canada',
    es: 'Canadá',
    fr: 'Canada',
    de: 'Kanada',
    ja: 'カナダ',
    ko: '캐나다',
    pt: 'Canadá',
    it: 'Canada',
    ru: 'Канада',
    zh: '加拿大',
    'zh-TW': '加拿大',
    'zh-HK': '加拿大'
  },
  'Australia': {
    en: 'Australia',
    es: 'Australia',
    fr: 'Australie',
    de: 'Australien',
    ja: 'オーストラリア',
    ko: '오스트레일리아',
    pt: 'Austrália',
    it: 'Australia',
    ru: 'Австралия',
    zh: '澳大利亚',
    'zh-TW': '澳洲',
    'zh-HK': '澳洲'
  },
  'South Africa': {
    en: 'South Africa',
    es: 'Sudáfrica',
    fr: 'Afrique du Sud',
    de: 'Südafrika',
    ja: '南アフリカ',
    ko: '남아프리카 공화국',
    pt: 'África do Sul',
    it: 'Sud Africa',
    ru: 'Южная Африка',
    zh: '南非',
    'zh-TW': '南非',
    'zh-HK': '南非'
    },
    'South Korea': {
    en: 'South Korea',
    es: 'Corea del Sur',
    fr: 'Corée du Sud',
    de: 'Südkorea',
    ja: '大韓民国',
    ko: '한국',
    pt: 'Coreia do Sul',
    it: 'Corea del Sud',
    ru: 'Южная Корея',
    zh: '韩国',
    'zh-TW': '南韓',
    'zh-HK': '南韓'
  },
  'Spain': {
    en: 'Spain',
    es: 'España',
    fr: 'Espagne',
    de: 'Spanien',
    ja: 'スペイン',
    ko: '스페인',
    pt: 'Espanha',
    it: 'Spagna',
    ru: 'Испания',
    zh: '西班牙',
    'zh-TW': '西班牙',
    'zh-HK': '西班牙'
  },
  'United Kingdom': {
    en: 'United Kingdom',
    es: 'Reino Unido',
    fr: 'Royaume-Uni',
    de: 'Vereinigtes Königreich',
    ja: 'イギリス',
    ko: '영국',
    pt: 'Reino Unido',
    it: 'Regno Unito',
    ru: 'Великобритания',
    zh: '英国',
    'zh-TW': '英國',
    'zh-HK': '英國'
  },
  'Italy': {
    en: 'Italy',
    es: 'Italia',
    fr: 'Italie',
    de: 'Italien',
    ja: 'イタリア',
    ko: '이탈리아',
    pt: 'Itália',
    it: 'Italia',
    ru: 'Италия',
    zh: '意大利',
    'zh-TW': '義大利',
    'zh-HK': '意大利'
  },
  'Mexico': {
    en: 'Mexico',
    es: 'México',
    fr: 'Mexique',
    de: 'Mexiko',
    ja: 'メキシコ',
    ko: '멕시코',
    pt: 'México',
    it: 'Messico',
    ru: 'Мексика',
    zh: '墨西哥',
    'zh-TW': '墨西哥',
    'zh-HK': '墨西哥'
  },
  'Indonesia': {
    en: 'Indonesia',
    es: 'Indonesia',
    fr: 'Indonésie',
    de: 'Indonesien',
    ja: 'インドネシア',
    ko: '인도네시아',
    pt: 'Indonésia',
    it: 'Indonesia',
    ru: 'Индонезия',
    zh: '印度尼西亚',
    'zh-TW': '印尼',
    'zh-HK': '印尼'
  },
  'Turkey': {
    en: 'Turkey',
    es: 'Turquía',
    fr: 'Turquie',
    de: 'Türkei',
    ja: 'トルコ',
    ko: '터키',
    pt: 'Turquia',
    it: 'Turchia',
    ru: 'Турция',
    zh: '土耳其',
    'zh-TW': '土耳其',
    'zh-HK': '土耳其'
  },
  'Netherlands': {
    en: 'Netherlands',
    es: 'Países Bajos',
    fr: 'Pays-Bas',
    de: 'Niederlande',
    ja: 'オランダ',
    ko: '네덜란드',
    pt: 'Holanda',
    it: 'Paesi Bassi',
    ru: 'Нидерланды',
    zh: '荷兰',
    'zh-TW': '荷蘭',
    'zh-HK': '荷蘭'
  },
  'Saudi Arabia': {
    en: 'Saudi Arabia',
    es: 'Arabia Saudita',
    fr: 'Arabie saoudite',
    de: 'Saudi-Arabien',
    ja: 'サウジアラビア',
    ko: '사우디아라비아',
    pt: 'Arábia Saudita',
    it: 'Arabia Saudita',
    ru: 'Саудовская Аравия',
    zh: '沙特阿拉伯',
    'zh-TW': '沙烏地阿拉伯',
    'zh-HK': '沙特阿拉伯'
  },
  'Switzerland': {
    en: 'Switzerland',
    es: 'Suiza',
    fr: 'Suisse',
    de: 'Schweiz',
    ja: 'スイス',
    ko: '스위스',
    pt: 'Suíça',
    it: 'Svizzera',
    ru: 'Швейцария',
    zh: '瑞士',
    'zh-TW': '瑞士',
    'zh-HK': '瑞士'
  },
  'Sweden': {
    en: 'Sweden',
    es: 'Suecia',
    fr: 'Suède',
    de: 'Schweden',
    ja: 'スウェーデン',
    ko: '스웨덴',
    pt: 'Suécia',
    it: 'Svezia',
    ru: 'Швеция',
    zh: '瑞典',
    'zh-TW': '瑞典',
    'zh-HK': '瑞典'
  },
  'Nigeria': {
    en: 'Nigeria',
    es: 'Nigeria',
    fr: 'Nigéria',
    de: 'Nigeria',
    ja: 'ナイジェリア',
    ko: '나이지리아',
    pt: 'Nigéria',
    it: 'Nigeria',
    ru: 'Нигерия',
    zh: '尼日利亚',
    'zh-TW': '奈及利亞',
    'zh-HK': '尼日利亚'
  },
  'Poland': {
    en: 'Poland',
    es: 'Polonia',
    fr: 'Pologne',
    de: 'Polen',
    ja: 'ポーランド',
    ko: '폴란드',
    pt: 'Polônia',
    it: 'Polonia',
    ru: 'Польша',
    zh: '波兰',
    'zh-TW': '波蘭',
    'zh-HK': '波兰'
  },
  'Argentina': {
    en: 'Argentina',
    es: 'Argentina',
    fr: 'Argentine',
    de: 'Argentinien',
    ja: 'アルゼンチン',
    ko: '아르헨티나',
    pt: 'Argentina',
    it: 'Argentina',
    ru: 'Аргентина',
    zh: '阿根廷',
    'zh-TW': '阿根廷',
    'zh-HK': '阿根廷'
  },
  'Norway': {
    en: 'Norway',
    es: 'Noruega',
    fr: 'Norvège',
    de: 'Norwegen',
    ja: 'ノルウェー',
    ko: '노르웨이',
    pt: 'Noruega',
    it: 'Norvegia',
    ru: 'Норвегия',
    zh: '挪威',
    'zh-TW': '挪威',
    'zh-HK': '挪威'
  },
  'Iran': {
    en: 'Iran',
    es: 'Irán',
    fr: 'Iran',
    de: 'Iran',
    ja: 'イラン',
    ko: '이란',
    pt: 'Irã',
    it: 'Iran',
    ru: 'Иран',
    zh: '伊朗',
    'zh-TW': '伊朗',
    'zh-HK': '伊朗'
  },
  'Egypt': {
    en: 'Egypt',
    es: 'Egipto',
    fr: 'Égypte',
    de: 'Ägypten',
    ja: 'エジプト',
    ko: '이집트',
    pt: 'Egito',
    it: 'Egitto',
    ru: 'Египет',
    zh: '埃及',
    'zh-TW': '埃及',
    'zh-HK': '埃及'
  },
    'Belgium': {
    en: 'Belgium',
    es: 'Bélgica',
    fr: 'Belgique',
    de: 'Belgien',
    ja: 'ベルギー',
    ko: '벨기에',
    pt: 'Bélgica',
    it: 'Belgio',
    ru: 'Бельгия',
    zh: '比利时',
    'zh-TW': '比利時',
    'zh-HK': '比利時'
  },
  'Bangladesh': {
    en: 'Bangladesh',
    es: 'Bangladesh',
    fr: 'Bangladesh',
    de: 'Bangladesch',
    ja: 'バングラデシュ',
    ko: '방글라데시',
    pt: 'Bangladesh',
    it: 'Bangladesh',
    ru: 'Бангладеш',
    zh: '孟加拉国',
    'zh-TW': '孟加拉',
    'zh-HK': '孟加拉'
  },
  'Thailand': {
    en: 'Thailand',
    es: 'Tailandia',
    fr: 'Thaïlande',
    de: 'Thailand',
    ja: 'タイ',
    ko: '태국',
    pt: 'Tailândia',
    it: 'Tailandia',
    ru: 'Таиланд',
    zh: '泰国',
    'zh-TW': '泰國',
    'zh-HK': '泰國'
  },
  'Greece': {
    en: 'Greece',
    es: 'Grecia',
    fr: 'Grèce',
    de: 'Griechenland',
    ja: 'ギリシャ',
    ko: '그리스',
    pt: 'Grécia',
    it: 'Grecia',
    ru: 'Греция',
    zh: '希腊',
    'zh-TW': '希臘',
    'zh-HK': '希臘'
  },
  'Pakistan': {
    en: 'Pakistan',
    es: 'Pakistán',
    fr: 'Pakistan',
    de: 'Pakistan',
    ja: 'パキスタン',
    ko: '파키스탄',
    pt: 'Paquistão',
    it: 'Pakistan',
    ru: 'Пакистан',
    zh: '巴基斯坦',
    'zh-TW': '巴基斯坦',
    'zh-HK': '巴基斯坦'
  },
  'Vietnam': {
    en: 'Vietnam',
    es: 'Vietnam',
    fr: 'Vietnam',
    de: 'Vietnam',
    ja: 'ベトナム',
    ko: '베트남',
    pt: 'Vietnã',
    it: 'Vietnam',
    ru: 'Вьетнам',
    zh: '越南',
    'zh-TW': '越南',
    'zh-HK': '越南'
  },
  'Philippines': {
    en: 'Philippines',
    es: 'Filipinas',
    fr: 'Philippines',
    de: 'Philippinen',
    ja: 'フィリピン',
    ko: '필리핀',
    pt: 'Filipinas',
    it: 'Filippine',
    ru: 'Филиппины',
    zh: '菲律宾',
    'zh-TW': '菲律賓',
    'zh-HK': '菲律宾'
  },
  'New Zealand': {
    en: 'New Zealand',
    es: 'Nueva Zelanda',
    fr: 'Nouvelle-Zélande',
    de: 'Neuseeland',
    ja: 'ニュージーランド',
    ko: '뉴질랜드',
    pt: 'Nova Zelândia',
    it: 'Nuova Zelanda',
    ru: 'Новая Зеландия',
    zh: '新西兰',
    'zh-TW': '紐西蘭',
    'zh-HK': '新西兰'
  },
  'Malaysia': {
    en: 'Malaysia',
    es: 'Malasia',
    fr: 'Malaisie',
    de: 'Malaysia',
    ja: 'マレーシア',
    ko: '말레이시아',
    pt: 'Malásia',
    it: 'Malesia',
    ru: 'Малайзия',
    zh: '马来西亚',
    'zh-TW': '馬來西亞',
    'zh-HK': '馬來西亞'
  },  
  'Ukraine': {
    en: 'Ukraine',
    es: 'Ucrania',
    fr: 'Ukraine',
    de: 'Ukraine',
    ja: 'ウクライナ',
    ko: '우크라이나',
    pt: 'Ucrânia',
    it: 'Ucraina',
    ru: 'Украина',
    zh: '乌克兰',
    'zh-TW': '烏克蘭',
    'zh-HK': '乌克兰'
  },
  'Europe': {
    en: 'Europe',
    es: 'Europa',
    fr: 'Europe',
    de: 'Europa',
    ja: 'ヨーロッパ',
    ko: '유럽',
    pt: 'Europa',
    it: 'Europa',
    ru: 'Европа',
    zh: '欧洲',
    'zh-TW': '歐洲',
    'zh-HK': '歐洲'
  },
  'Singapore': {
    en: 'Singapore',
    es: 'Singapur',
    fr: 'Singapour',
    de: 'Singapur',
    ja: 'シンガポール',
    ko: '싱가포르',
    pt: 'Singapura',
    it: 'Singapore',
    ru: 'Сингапур',
    zh: '新加坡',
    'zh-TW': '新加坡',
    'zh-HK': '新加坡'
  },
  'Israel': {
    en: 'Israel',
    es: 'Israel',
    fr: 'Israël',
    de: 'Israel',
    ja: 'イスラエル',
    ko: '이스라엘',
    pt: 'Israel',
    it: 'Israele',
    ru: 'Израиль',
    zh: '以色列',
    'zh-TW': '以色列',
    'zh-HK': '以色列'
  },
  'Portugal': {
    en: 'Portugal',
    es: 'Portugal',
    fr: 'Portugal',
    de: 'Portugal',
    ja: 'ポルトガル',
    ko: '포르투갈',
    pt: 'Portugal',
    it: 'Portogallo',
    ru: 'Португалия',
    zh: '葡萄牙',
    'zh-TW': '葡萄牙',
    'zh-HK': '葡萄牙'  
  },
  'United Arab Emirates': {
    en: 'United Arab Emirates',
    es: 'Emiratos Árabes Unidos',
    fr: 'Émirats arabes unis',
    de: 'Vereinigte Arabische Emirate',
    ja: 'アラブ首長国連邦',
    ko: '아랍에미리트',
    pt: 'Emirados Árabes Unidos',
    it: 'Emirati Arabi Uniti',
    ru: 'Объединенные Арабские Эмираты',
    zh: '阿拉伯联合酋长国',
    'zh-TW': '阿拉伯聯合大公國',
    'zh-HK': '阿拉伯联合酋长国'
  },
  'Hong Kong': {
    en: 'Hong Kong',
    es: 'Hong Kong',
    fr: 'Hong Kong',
    de: 'Hongkong',
    ja: '香港',
    ko: '홍콩',
    pt: 'Hong Kong',
    it: 'Hong Kong',
    ru: 'Гонконг',
    zh: '香港',
    'zh-TW': '香港',
    'zh-HK': '香港'
  },
  'Colombia': {
    en: 'Colombia',
    es: 'Colombia',
    fr: 'Colombie',
    de: 'Kolumbien',
    ja: 'コロンビア',
    ko: '콜롬비아',
    pt: 'Colômbia',
    it: 'Colombia',
    ru: 'Колумбия',
    zh: '哥伦比亚',
    'zh-TW': '哥倫比亞',
    'zh-HK': '哥伦比亚'
  },
  'Denmark': {
    en: 'Denmark',
    es: 'Dinamarca',
    fr: 'Danemark',
    de: 'Dänemark',
    ja: 'デンマーク',
    ko: '덴마크',
    pt: 'Dinamarca',
    it: 'Danimarca',
    ru: 'Дания',
    zh: '丹麦',
    'zh-TW': '丹麥',
    'zh-HK': '丹麦'
  },
  'Finland': {
    en: 'Finland',
    es: 'Finlandia',
    fr: 'Finlande',
    de: 'Finnland',
    ja: 'フィンランド',
    ko: '핀란드',
    pt: 'Finlândia',
    it: 'Finlandia',
    ru: 'Финляндия',
    zh: '芬兰',
    'zh-TW': '芬蘭',
    'zh-HK': '芬兰'
  },
  'Ireland': {
    en: 'Ireland',
    es: 'Irlanda',
    fr: 'Irlande',
    de: 'Irland',
    ja: 'アイルランド',
    ko: '아일랜드',
    pt: 'Irlanda',
    it: 'Irlanda',
    ru: 'Ирландия',
    zh: '爱尔兰',
    'zh-TW': '愛爾蘭',
    'zh-HK': '爱尔兰'  
  },
  'Austria': {
    en: 'Austria',
    es: 'Austria',
    fr: 'Autriche',
    de: 'Österreich',
    ja: 'オーストリア',
    ko: '오스트리아',
    pt: 'Áustria',
    it: 'Austria',
    ru: 'Австрия',
    zh: '奥地利',
    'zh-TW': '奧地利',
    'zh-HK': '奥地利'
  },
  'Taiwan': {
    en: 'Taiwan',
    es: 'Taiwán',
    fr: 'Taïwan',
    de: 'Taiwan',
    ja: '台湾',
    ko: '대만',
    pt: 'Taiwan',
    it: 'Taiwan',
    ru: 'Тайвань',
    zh: '台湾',
    'zh-TW': '臺灣',
    'zh-HK': '台灣'
  },
  'Asia': {
    en: 'Asia',
    es: 'Asia',
    fr: 'Asie',
    de: 'Asien',
    ja: 'アジア',
    ko: '아시아',
    pt: 'Ásia',
    it: 'Asia',
    ru: 'Азия',
    zh: '亚洲',
    'zh-TW': '亞洲',
    'zh-HK': '亞洲'
  },
  'Americas': {
    en: 'Americas',
    es: 'Américas',
    fr: 'Amériques',
    de: 'Amerika',
    ja: 'アメリカ大陸',
    ko: '아메리카 대륙',
    pt: 'Américas',
    it: 'Americhe',
    ru: 'Америка',
    zh: '美洲',
    'zh-TW': '美洲',
    'zh-HK': '美洲'
  },
  'Africa': {
    en: 'Africa',
    es: 'África',
    fr: 'Afrique',
    de: 'Afrika',
    ja: 'アフリカ',
    ko: '아프리카',
    pt: 'África',
    it: 'Africa',
    ru: 'Африка',
    zh: '非洲',
    'zh-TW': '非洲',
    'zh-HK': '非洲'
  },
  'Middle East': {
    en: 'Middle East',
    es: 'Medio Oriente',
    fr: 'Moyen-Orient',
    de: 'Naher Osten',
    ja: '中東',
    ko: '중동',
    pt: 'Médio Oriente',
    it: 'Medio Oriente',
    ru: 'Ближний Восток',
    zh: '中东',
    'zh-TW': '中東',
    'zh-HK': '中東'
  },
  'China News': {
    en: 'China',
    es: 'China',
    fr: 'Chine',
    de: 'China',
    ja: '中国',
    ko: '중국',
    pt: 'China',
    it: 'Cina',
    ru: 'Китай',
    'zh-TW': '中國',
    'zh-HK': '中國',
    zh: '中国'
  },
  'Companies': {
    en: 'Companies',
    es: 'Empresas',
    fr: 'Entreprises',
    de: 'Unternehmen',
    ja: '企業',
    ko: '회사',
    pt: 'Empresas',
    it: 'Aziende',
    ru: 'Компании',
    'zh-TW': '公司',
    'zh-HK': '公司',
    zh: '公司'
  },
  'Markets': {
    en: 'Markets',
    es: 'Mercados',
    fr: 'Marchés',
    de: 'Märkte',
    ja: '市場',
    ko: '시장',
    pt: 'Mercados',
    it: 'Mercati',
    ru: 'Рынки',
    'zh-TW': '市場',
    'zh-HK': '市場',
    zh: '市场'
  },
  'Economy': {
    en: 'Economy',
    es: 'Economía',
    fr: 'Économie',
    de: 'Wirtschaft',
    ja: '経済',
    ko: '경제',
    pt: 'Economia',
    it: 'Economia',
    ru: 'Экономика',
    'zh-TW': '經濟',
    'zh-HK': '經濟',
    zh: '经济'
  },
  'Feature': {
    en: 'Feature',
    es: 'Reportaje',
    fr: 'Reportage',
    de: 'Reportage',
    ja: '特集',
    ko: '특집',
    pt: 'Reportagem',
    it: 'Servizio',
    ru: 'Репортаж',
    'zh-TW': '特寫',
    'zh-HK': '特寫',
    zh: '特写'
  },
  'Obituary': {
    en: 'Obituary',
    es: 'Obituario',
    fr: 'Nécrologie',
    de: 'Nachruf',
    ja: '訃報',
    ko: '부고',
    pt: 'Obituário',
    it: 'Necrologio',
    ru: 'Некролог',
    'zh-TW': '訃聞',
    'zh-HK': '訃聞',
    zh: '讣告'
  },
  'Opinion': {
    en: 'Opinion',
    es: 'Opinión',
    fr: 'Opinion',
    de: 'Meinung',
    ja: '意見',
    ko: '의견',
    pt: 'Opinião',
    it: 'Opinione',
    ru: 'Мнение',
    'zh-TW': '觀點',
    'zh-HK': '觀點',
    zh: '观点'
  },
  'News': {
    en: 'News',
    es: 'Noticias',
    fr: 'Nouvelles',
    de: 'Nachrichten',
    ja: 'ニュース',
    ko: '뉴스',
    pt: 'Notícias',
    it: 'Notizie',
    ru: 'Новости',
    'zh-TW': '新聞',
    'zh-HK': '新聞',
    zh: '新闻'
  },
  'Explainer': {
    en: 'Explainer',
    zh: '解读',
    es: 'Explicativo',
    fr: 'Explicatif',
    de: 'Erklärung',
    ja: '解説',
    ko: '설명자',
    pt: 'Explicativo',
    it: 'Spiegazione',
    ru: 'Пояснение',
    'zh-TW': '解讀',
    'zh-HK': '解讀'
  },
  'PODCASTS': {
    en: 'Podcasts',
    es: 'Podcasts',
    fr: 'Podcasts',
    de: 'Podcasts',
    ja: 'ポッドキャスト',
    ko: '팟캐스트',
    pt: 'Podcasts',
    it: 'Podcast',
    ru: 'Подкасты',
    'zh-TW': '播客',
    'zh-HK': '播客',
    zh: '播客'
  },
  'VIDEOS': {
    en: 'Videos',
    es: 'Videos',
    fr: 'Vidéos',
    de: 'Videos',
    ja: 'ビデオ',
    ko: '동영상',
    pt: 'Vídeos',
    it: 'Video',
    ru: 'Видео',
    'zh-TW': '視頻',
    'zh-HK': '視頻',
    zh: '视频'
  },
  'Life & Arts': {
    en: 'Life & Arts',
    es: 'Estilo de vida y arte',
    fr: 'Art de vivre',
    de: 'Leben & Kunst',
    ja: 'ライフ＆アート',
    ko: '라이프 & 아트',
    pt: 'Estilo de vida e arte',
    it: 'Vita e arte',
    ru: 'Жизнь и искусство',
    'zh-TW': '生活與藝術',
    'zh-HK': '生活與藝術',
    zh: '生活与艺术'
  },
  'Work & Careers': {
    en: 'Work & Careers',
    es: 'Trabajo y carreras',
    fr: 'Travail et carrières',
    de: 'Arbeit & Karriere',
    ja: '仕事とキャリア',
    ko: '직업 및 경력',
    pt: 'Trabalho e carreiras',
    it: 'Lavoro e carriere',
    ru: 'Работа и карьера',
    'zh-TW': '工作與職涯',
    'zh-HK': '工作與職涯',
    zh: '工作与职业'
  },
  'AI News': {
    en: 'AI',
    es: 'IA',
    fr: 'IA',
    de: 'AI',
    ja: 'AI',
    ko: 'AI',
    pt: 'IA',
    it: 'IA',
    'zh-TW': '人工智慧',
    'zh-HK': 'AI',
    zh: '人工智能',
    ru: 'ИИ'
  },
  Science: {
    en: 'Science',
    es: 'Ciencia',
    fr: 'Science',
    de: 'Wissenschaft',
    ja: '科学',
    ko: '과학',
    pt: 'Ciência',
    it: 'Scienza',
    'zh-TW': '科學',
    'zh-HK': '科學',
    zh: '科学',
    ru: 'Наука'
  },
  'US Presidential Election': {
    en: 'US Presidential Election',
    es: 'Elecciones presidenciales de EE. UU.',
    fr: 'Élection présidentielle américaine',
    de: 'US-Präsidentschaftswahl',
    ja: 'アメリカ大統領選挙',
    ko: '미국 대통령 선거',
    pt: 'Eleição presidencial dos EUA',
    it: 'Elezioni presidenziali statunitensi',
    ru: 'Президентские выборы в США',
    'zh-TW': '美國大選',
    'zh-HK': '美國大選',
    zh: '美国大选'
  },
  'Israel-Hamas War': {
    en: 'Israel-Hamas War',
    es: 'Guerra Israel-Hamas',
    fr: 'Guerre Israël-Hamas',
    de: 'Israel-Hamas-Krieg',
    ja: 'イスラエル•ハマス戦争',
    ko: '이스라엘-하마스 전쟁',
    pt: 'Guerra Israel-Hamas',
    it: 'Guerra tra Israele e Hamas',
    ru: 'Война Израиль-Хамас',
    'zh-TW': '以色列-哈馬斯戰爭',
    'zh-HK': '以色列-哈馬斯戰爭',
    zh: '以色列-哈马斯战争'
  },
  'Property': {
    en: 'Property',
    es: 'Propiedad',
    fr: 'Propriété',
    de: 'Immobilien',
    ja: '不動産',
    ko: '부동산',
    pt: 'Propriedade',
    it: 'Proprietà',
    ru: 'Недвижимость',
    'zh-TW': '房地產',
    'zh-HK': '房地產',
    zh: '房地产'
  },
  'Climate Change': {
    en: 'Climate Change',
    es: 'Cambio climático',
    fr: 'Changement climatique',
    de: 'Klimawandel',
    ja: '気候変動',
    ko: '기후 변화',
    pt: 'Mudança climática',
    it: 'Cambiamento climatico',
    ru: 'Климатические изменения',
    'zh-TW': '氣候變遷',
    'zh-HK': '氣候變遷',
    zh: '气候变化'
  },
  'Federal Reserve': {
    en: 'Federal Reserve',
    es: 'Reserva Federal',
    fr: 'Réserve fédérale',
    de: 'Federal Reserve',
    ja: '連邦準備銀行',
    ko: '연방준비제도',
    pt: 'Reserva Federal',
    it: 'Riserva Federale',
    ru: 'Федеральный резерв',
    'zh-TW': '美聯準',
    'zh-HK': '美聯儲',
    zh: '美联储'
  },
  'Chinese Economy': {
    en: 'Chinese Economy',
    es: 'Economía china',
    fr: 'Économie chinoise',
    de: 'Chinesische Wirtschaft',
    ja: '中国経済',
    ko: '중국 경제',
    pt: 'Economia chinesa',
    it: 'Economia cinese',
    ru: 'Китайская экономика',
    'zh-TW': '中國經濟',
    'zh-HK': '中國經濟',
    zh: '中国经济'
  },
  'Semiconductors': {
    en: 'Semiconductors',
    es: 'Semiconductores',
    fr: 'Semi-conducteurs',
    de: 'Halbleiter',
    ja: '半導体',
    ko: '반도체',
    pt: 'Semicondutores',
    it: 'Semiconduttori',
    ru: 'Полупроводники',
    'zh-TW': '半導體',
    'zh-HK': '半導體',
    zh: '半导体'
  },
  'Cryptocurrencies': {
    en: 'Cryptocurrencies',
    es: 'Criptomonedas',
    fr: 'Cryptomonnaies',
    de: 'Kryptowährungen',
    ja: '暗号通貨',
    ko: '암호화폐',
    pt: 'Criptomoedas',
    it: 'Criptovalute',
    ru: 'Криптовалюты',
    'zh-TW': '加密貨幣',
    'zh-HK': '加密貨幣',
    zh: '加密货币'
  },
  'Follow': {
    zh: '关注',
    en: 'Follow',
    es: 'Seguir',
    fr: 'Suivre',
    de: 'Folgen',
    ja: 'フォロー',
    ko: '팔로우',
    pt: 'Seguir',
    it: 'Segui',
    'zh-TW': '關注',
    'zh-HK': '關注',
    ru: 'Подписаться'
  }, 
  'Follow All': {
    zh: '关注所有',
    en: 'Follow All',
    es: 'Seguir a todos',
    fr: 'Suivre tout',
    de: 'Alles folgen',
    ja: 'すべてをフォロー',
    ko: '모두 팔로우',
    pt: 'Seguir todos',
    it: 'Segui tutti',
    'zh-TW': '關注所有',
    'zh-HK': '關注所有',
    ru: 'Подписаться на всех'
  }, 
  'Unfollow All': {
    zh: '取消所有关注',
    en: 'Unfollow All',
    es: 'Dejar de seguir a todos',
    fr: 'Ne plus suivre tout',
    de: 'Alles entfolgen',
    ja: 'すべてのフォローを解除',
    ko: '모두 언팔로우',
    pt: 'Deixar de seguir todos',
    it: 'Smetti di seguire tutti',
    'zh-TW': '取消所有關注',
    'zh-HK': '取消所有關注',
    ru: 'Отписаться от всех'
  },
  'Unfollow': {
    zh: '取消关注',
    en: 'Unfollow',
    es: 'Dejar de seguir',
    fr: 'Ne plus suivre',
    de: 'Entfolgen',
    ja: 'フォロー解除',
    ko: '언팔로우',
    pt: 'Deixar de seguir',
    it: 'Non seguire più',
    'zh-TW': '取消關注',
    'zh-HK': '取消關注',
    ru: 'Отписаться'
  },
  'Followed': {
    zh: '已关注',
    en: 'Followed',
    es: 'Seguido',
    fr: 'Suivi',
    de: 'Gefolgt',
    ja: 'フォロー済み',
    ko: '팔로우됨',
    pt: 'Seguido',
    it: 'Seguito',
    'zh-TW': '已關注',
    'zh-HK': '已關注',
    ru: 'Подписан'
  },
  'Follow the topics in this article': {
    en: 'Follow the topics in this article',
    zh: '相关话题',
    es: 'Sigue los temas en este artículo',
    fr: 'Suivez les sujets de cet article',
    de: 'Folgen Sie den Themen in diesem Artikel',
    ja: 'この記事のトピックをフォローする',
    ko: '이 기사의 주제를 팔로우하세요',
    pt: 'Siga os tópicos deste artigo',
    it: 'Segui gli argomenti di questo articolo',
    'zh-TW': '相關話題',
    'zh-HK': '相關話題',
    ru: 'Подписаться на темы в этой статье'
  },
  'Topics mentioned in this article': {
    en: 'Topics mentioned in this article',
    zh: '本文提及的话题',
    es: 'Temas mencionados en este artículo',
    fr: 'Sujets mentionnés dans cet article',
    de: 'In diesem Artikel erwähnte Themen',
    ja: 'この記事で言及されているトピック',
    ko: '이 기사에서 언급된 주제들',
    pt: 'Tópicos mencionados neste artigo',
    it: 'Argomenti menzionati in questo articolo',
    'zh-TW': '本文提及的話題',
    'zh-HK': '本文提及的話題',
    ru: 'Темы, упомянутые в этой статье'
  },
  Topics: {
    "zh": "话题",
    "en": "Topics",
    "es": "Temas",
    "fr": "Sujets",
    "de": "Themen",
    "ja": "トピック",
    "ko": "주제",
    "pt": "Tópicos",
    "it": "Argomenti",
    "zh-TW": "話題",
    "zh-HK": "話題",
    "ru": "Темы"
  },
  Topic: {
    "zh": "话题",
    "en": "Topic",
    "es": "Tema",
    "fr": "Sujet",
    "de": "Thema",
    "ja": "トピック",
    "ko": "주제",
    "pt": "Tópico",
    "it": "Argomento",
    "zh-TW": "話題",
    "zh-HK": "話題",
    "ru": "Тема"
  },
  'Input Interest': {
    "zh": "请输入您关注的话题",
    "en": "Please enter your topic of interest",
    "es": "Por favor, introduzca el tema de su interés",
    "fr": "Veuillez saisir le sujet qui vous intéresse",
    "de": "Bitte geben Sie Ihr Interessenthema ein",
    "ja": "興味のあるトピックを入力してください",
    "ko": "관심 있는 주제를 입력하세요",
    "pt": "Por favor, insira o tópico de seu interesse",
    "it": "Inserisci l'argomento di tuo interesse",
    "zh-TW": "請輸入您關注的話題",
    "zh-HK": "請輸入您關注的話題",
    "ru": "Пожалуйста, введите интересующую вас тему"
  },
  'Topic already followed': {
    "zh": "这个话题您已经关注了",
    "en": "You have already followed this topic",
    "es": "Ya has seguido este tema",
    "fr": "Vous avez déjà suivi ce sujet",
    "de": "Sie haben dieses Thema bereits verfolgt",
    "ja": "このトピックはすでにフォローしています",
    "ko": "이미 이 주제를 팔로우했습니다",
    "pt": "Você já seguiu este tópico",
    "it": "Hai già seguito questo argomento",
    "zh-TW": "您已經關注了這個話題",
    "zh-HK": "您已經關注了這個話題",
    "ru": "Вы уже следите за этой темой"
  },
  'PromptAdd': {
    zh: '您还没有关注任何话题，请点击这里添加',
    en: 'You have not followed any topics yet, please click here to add',
    es: 'Aún no has seguido ningún tema, haz clic aquí para añadir',
    fr: "Vous n'avez pas encore suivi de sujets, veuillez cliquer ici pour en ajouter",
    de: 'Sie haben noch keine Themen abonniert, klicken Sie hier um welche hinzuzufügen',
    ja: 'あなたはまだトピックをフォローしていません。追加するにはここをクリックしてください。',
    ko: '아직 팔로우한 주제가 없습니다. 여기를 클릭하여 추가하세요.', 
    pt: 'Você ainda não segue nenhum tópico, clique aqui para adicionar',
    it: 'Non hai ancora seguito alcun argomento, fai clic qui per aggiungerne',
    'zh-TW': '您還沒有關注任何主題，請點擊這裡添加',
    'zh-HK': '您仲未關注任何話題，請點擊呢度添加',
    ru: 'Вы пока не подписаны ни на одну тему, нажмите здесь, чтобы добавить'
  },
  'Artificial intelligence': {
    en: 'Artificial intelligence',
    es: 'Inteligencia artificial',
    fr: 'Intelligence artificielle',
    de: 'Künstliche Intelligenz',
    ja: '人工知能',
    ko: '인공 지능',
    pt: 'Inteligência artificial',
    it: 'Intelligenza artificiale',
    'zh-TW': '人工智慧',
    'zh-HK': '人工智能',
    zh: '人工智能',
    ru: 'Искусственный интеллект'
  },
  'Electric vehicles': {
    en: 'Electric vehicles',
    es: 'Vehículos eléctricos',
    fr: 'Véhicules électriques',
    de: 'Elektrofahrzeuge',
    ja: '電気自動車',
    ko: '전기 자동차',
    pt: 'Veículos elétricos',
    it: 'Veicoli elettrici',
    'zh-TW': '電動汽車',
    'zh-HK': '電動汽車',
    zh: '电动汽车',
    ru: 'Электромобили'
  },
  'Search': {
    en: 'Search',
    es: 'Buscar',
    fr: 'Chercher',
    de: 'Suchen',
    ja: '検索',
    ko: '검색',
    pt: 'Pesquisar',
    it: 'Cerca',
    'zh-TW': '搜索',
    'zh-HK': '搜尋',
    zh: '搜索',
    ru: 'Искать'
  },
  'Deep Dive': {
    zh: '深度报道',
    en: 'In-depth reports',
    es: 'Reportajes en profundidad',
    fr: 'Reportages approfondis',
    de: 'Vertiefende Berichte',
    ja: '綿密な報道',
    ko: '심층 보도',
    pt: 'Reportagens em profundidade',
    it: 'Approfondimenti',
    'zh-TW': '深度報導',
    'zh-HK': '深度報導',
    ru: 'Глубокие репортажи'
  },
  'Tech News': {
    zh: '科技',
    en: 'Technology',
    es: 'Tecnología',
    fr: 'Technologie',
    de: 'Technologie',
    ja: 'テクノロジー',
    ko: '기술',
    pt: 'Tecnologia',
    it: 'Tecnologia',
    'zh-TW': '科技',
    'zh-HK': '科技',
    ru: 'Технологии'
  },
  'Technology Sector': {
    zh: '科技',
    en: 'Technology',
    es: 'Tecnología',
    fr: 'Technologie',
    de: 'Technologie',
    ja: 'テクノロジー',
    ko: '기술',
    pt: 'Tecnologia',
    it: 'Tecnologia',
    'zh-TW': '科技',
    'zh-HK': '科技',
    ru: 'Технологии'
  },
  'Offer Help': {
    zh: '好的，需要我怎么帮助您？',
    en: 'Sure, how can I help you?',
    es: 'Claro, ¿en qué puedo ayudarle?',
    fr: 'Bien sûr, comment puis-je vous aider?',
    de: 'Ja, wie kann ich Ihnen helfen?',
    ja: '承知しました。何かお力になれることはありますか？',
    ko: '네, 무엇을 도와드릴까요?',
    pt: 'Claro, como posso ajudá-lo?',
    it: 'Certamente, in che modo posso aiutarti?',
    'zh-TW': '好的，有什麼我能幫到您的嗎？',
    'zh-HK': '好的，有什麼我能幫到您的嗎？',
    ru: 'Конечно, в чем я могу вам помочь?'
  },
  'Discuss Article': {
    zh: '关于这篇文章，如果您有任何问题，可以现在问我。',
    en: "If you have any questions about this article, feel free to ask me now.",
    es: "Si tienes alguna pregunta sobre este artículo, no dudes en preguntarme ahora.",
    fr: "Si vous avez des questions sur cet article, n'hésitez pas à me demander maintenant.",
    de: "Wenn Sie Fragen zu diesem Artikel haben, fragen Sie mich gerne jetzt.",
    ja: "この記事についての質問があれば、遠慮なくお聞きください。",
    ko: "이 기사에 대해 궁금한 점이 있으면 지금 저에게 물어보세요.",
    pt: "Se você tiver alguma dúvida sobre este artigo, sinta-se à vontade para me perguntar agora.",
    it: "Se hai domande su questo articolo, sentiti libero di chiedermi ora.",
    'zh-TW': "關於這篇文章，如果您有任何問題，可以現在問我。",
    'zh-HK': "關於呢篇文章，如果你有咩問題，可以即刻問我。",
    ru: "Если у вас есть вопросы по этой статье, не стесняйтесь спрашивать меня сейчас."
  },
  Finding: {
    zh: '好的，我来帮您查询...',
    en: 'Okay, let me help you search...',
    es: 'De acuerdo, déjame ayudarte a buscar...',
    fr: `D'accord, laissez-moi vous aider à chercher...`,
    de: 'Okay, ich helfe Ihnen gerne bei der Suche...',
    ja: '了解しました、検索をお手伝いします...',
    ko: '알겠습니다. 검색을 도와드리겠습니다...',
    pt: 'Certo, deixe-me ajudá-lo a pesquisar...',
    it: 'Va bene, lascia che ti aiuti a cercare...',
    'zh-TW': '好的，我來幫您查詢...',
    'zh-HK': '好的，我來幫您查詢...',
    ru: 'Хорошо, я помогу вам найти...'
  },
  FindingMyFT: {
    zh: '为您展示FT最新的内容，您也可以<a>点击这里设置您关注的话题和阅读偏好</a>',
    en: 'Let me show you the latest content from FT, you can also <a>click here to set the topics you are interested in and your reading preferences</a>',
    es: 'Déjame mostrarte el contenido más reciente de FT, también puedes <a>hacer clic aquí para configurar los temas que sigues y tus preferencias de lectura</a>',
    fr: 'Laissez-moi vous montrer le contenu le plus récent de FT, vous pouvez également <a>cliquer ici pour définir les sujets que vous suivez et vos préférences de lecture</a>',
    de: 'Lassen Sie mich Ihnen die neuesten Inhalte von FT zeigen, Sie können auch <a>hier klicken, um die Themen, denen Sie folgen, und Ihre Leseprioritäten festzulegen</a>',
    ja: 'FTの最新コンテンツをご覧いただけます。また、<a>こちらをクリックして、フォローするトピックと読書の嗜好を設定することもできます</a>',
    ko: 'FT의 최신 컨텐츠를 보여드리겠습니다. 또한 <a>여기를 클릭하여 관심 있는 주제와 독서 선호도를 설정할 수 있습니다</a>',
    pt: 'Deixe-me mostrar os conteúdos mais recentes da FT, você também pode <a>clicar aqui para definir os tópicos que segue e suas preferências de leitura</a>',
    it: 'Lascia che ti mostri i contenuti più recenti di FT, puoi anche <a>fare clic qui per impostare gli argomenti che segui e le tue preferenze di lettura</a>',
    'zh-TW': '讓我展示FT最新的內容，您也可以<a>點擊這裡設定您關注的話題和閱讀偏好</a>',
    'zh-HK': '讓我展示FT最新的內容，您也可以<a>點擊這裡設定您關注的話題和閱讀偏好</a>',
    ru: 'Давайте я покажу вам самые свежие материалы FT, вы также можете <a>нажать здесь, чтобы настроить темы, которые вы отслеживаете, и ваши предпочтения в чтении</a>'
  },
  FindingMostPopular: {
    zh: '好的，我来帮您查询FT的热门文章...',
    en: 'Okay, let me help you check the most popular content on the FT...',
    es: 'De acuerdo, déjame ayudarte a buscar el contenido más popular en FT...',
    fr: 'D\'accord, laissez-moi vous aider à chercher le contenu le plus populaire sur FT...',
    de: 'Okay, ich helfe Ihnen, die beliebtesten Inhalte auf FT zu finden...',
    ja: '了解しました、FTの人気コンテンツを検索のお手伝いをします...',
    ko: '알겠습니다, FT에서 가장 인기있는 콘텐츠를 검색하는 것을 도와드리겠습니다...',
    pt: 'Certo, deixe-me ajudá-lo a pesquisar o conteúdo mais popular no FT...',
    it: 'Va bene, lascia che ti aiuti a cercare il contenuto più popolare su FT...',
    'zh-TW': '好的，我來幫您查詢FT的熱門文章...',
    'zh-HK': '好的，我來幫您查詢FT的熱門文章...',
    ru: 'Хорошо, я помогу вам найти самый популярный контент на FT...'
  },
  ApplyAndStartOver: {
    zh: '应用并重启',
    en: 'Apply and Restart',
    es: 'Aplicar y Reiniciar',
    fr: 'Appliquer et Redémarrer',
    de: 'Anwenden und Neustarten',
    ja: '適用して再起動',
    ko: '적용하고 재시작',
    pt: 'Aplicar e Reiniciar',
    it: 'Applica e Riavvia',
    'zh-TW': '應用並重啟',
    'zh-HK': '應用並重啟',
    ru: 'Применить и Перезагрузить'
  },
  'ClearAndStartOver': {
    zh: '清除并重启',
    en: 'Clear and Restart',
    es: 'Limpiar y reiniciar',
    fr: 'Effacer et redémarrer',
    de: 'Zurücksetzen und Neustarten',
    ja: 'クリアして再起動',
    ko: '초기화하고 재시작',
    pt: 'Limpar e reiniciar',
    it: 'Pulisci e riavvia',
    'zh-TW': '清除並重啟',
    'zh-HK': '清除並重啟',
    ru: 'Очистить и перезагрузить'
  },
  'ConfirmDelete': {
    zh: '删除所有偏好设置和关注吗？此操作不可逆。',
    en: 'Delete all preferences and subscriptions? This action cannot be undone.',
    es: '¿Eliminar todas las preferencias y suscripciones? Esta acción no se puede deshacer.',
    fr: 'Supprimer toutes les préférences et abonnements ? Cette action est irréversible.',
    de: 'Alle Einstellungen und Abonnements löschen? Diese Aktion kann nicht rückgängig gemacht werden.',
    ja: 'すべての設定と登録を削除しますか？この操作は取り消しできません。',
    ko: '모든 선호 설정과 구독을 삭제하시겠습니까? 이 작업은 취소할 수 없습니다.',
    pt: 'Excluir todas as preferências e inscrições? Esta ação não pode ser desfeita.',
    it: 'Eliminare tutte le preferenze e le iscrizioni? Questa azione è irreversibile.',
    'zh-TW': '刪除所有偏好設定和關注嗎？此操作不可逆。',
    'zh-HK': '刪除所有偏好設定和關注嗎？此操作不可逆。',
    ru: 'Удалить все настройки и подписки? Это действие нельзя отменить.'
  },
  Read: {
    "zh": "已读",
    "en": "Read",
    "es": "Leído",
    "fr": "Lu",
    "de": "Gelesen",
    "ja": "読了",
    "ko": "읽음",
    "pt": "Lido",
    "it": "Letto",
    "zh-TW": "已讀",
    "zh-HK": "已讀",
    "ru": "Прочитано"
  },
  "Read Articles": {
    "zh": "已读文章",
    "en": "Read Articles",
    "es": "Artículos Leídos",
    "fr": "Articles Lus",
    "de": "Gelesene Artikel",
    "ja": "読了した記事",
    "ko": "읽은 기사",
    "pt": "Artigos Lidos",
    "it": "Articoli Letti",
    "zh-TW": "已讀文章",
    "zh-HK": "已讀文章",
    "ru": "Прочитанные Статьи"
  },
  "Show": {
    "zh": "显示",
    "en": "Show",
    "es": "Mostrar",
    "fr": "Afficher",
    "de": "Anzeigen",
    "ja": "表示",
    "ko": "보이기",
    "pt": "Mostrar",
    "it": "Mostra",
    "zh-TW": "顯示",
    "zh-HK": "顯示",
    "ru": "Показать"
  },
  "Collapse to Bottom": {
    "zh": "折叠到底部",
    "en": "Collapse to Bottom",
    "es": "Colapsar al Final",
    "fr": "Réduire en Bas",
    "de": "Nach Unten Einklappen",
    "ja": "下部に折りたたむ",
    "ko": "아래로 접기",
    "pt": "Colapsar para o Fundo",
    "it": "Collassare in Basso",
    "zh-TW": "摺疊到底部",
    "zh-HK": "摺疊到底部",
    "ru": "Свернуть вниз"
  },
  ReadingPreferences: {
    "zh": "阅读偏好",
    "en": "Reading Preferences",
    "es": "Preferencias de Lectura",
    "fr": "Préférences de Lecture",
    "de": "Lesepräferenzen",
    "ja": "読書の好み",
    "ko": "독서 선호도",
    "pt": "Preferências de Leitura",
    "it": "Preferenze di Lettura",
    "zh-TW": "閱讀偏好",
    "zh-HK": "閱讀偏好",
    "ru": "Предпочтения в Чтении"
  },
  PrepareingQuiz: {
    zh: '为您准备小测...',
    en: 'Preparing a quiz for you...',
    es: 'Preparando un cuestionario para ti...',
    fr: `Préparation d'un quiz pour vous...`,
    de: 'Bereite einen Quiz für dich vor...',
    ja: 'あなたのためにクイズを準備しています...',
    ko: '퀴즈를 준비 중입니다...',
    pt: 'Preparando um questionário para você...',
    it: 'Preparando un quiz per te...',
    'zh-TW': '為您準備小測...',
    'zh-HK': '為您準備小測...',
    ru: 'Подготовка викторины для вас...'
  },
  IntroducingQuiz: {
    "zh": "FT中文网每日小测试，快速梳理最新的全球财经新闻大事件。",
    "en": "FTChinese Daily News Quiz: Quickly recap the latest global financial news events.",
    "es": "Cuestionario Diario de Noticias de FTChinese: Resume rápidamente los últimos acontecimientos financieros globales.",
    "fr": "Quiz Quotidien FTChinese: Récapitulez rapidement les derniers événements financiers mondiaux.",
    "de": "FTChinese Tägliches Nachrichten-Quiz: Fassen Sie die neuesten globalen Finanznachrichten schnell zusammen.",
    "ja": "FTChineseデイリーニュースクイズ：最新の世界金融ニュースを迅速に振り返ります。",
    "ko": "FTChinese 일간 뉴스 퀴즈: 최신 글로벌 금융 뉴스를 빠르게 요약합니다.",
    "pt": "Quiz Diário de Notícias do FTChinese: Recapitule rapidamente os últimos eventos financeiros globais.",
    "it": "Quiz Quotidiano di FTChinese: Riepiloga rapidamente gli ultimi eventi finanziari globali.",
    "zh-TW": "FT中文網每日小測試，快速梳理最新的全球財經新聞大事件。",
    "zh-HK": "FT中文網每日小測試，快速梳理最新的全球財經新聞大事件。",
    "ru": "Ежедневный викторинный тест FTChinese: Быстро резюмируйте последние события в мировой финансовой сфере."
  },
  'What do you want to find?': {
    zh: '好的，您可以点击下面这些最为常见的查询，也可以告诉我您想找什么内容。',
    en: 'Okay, you can click on the most common queries below, or tell me what content you are looking for.',
    es: 'De acuerdo, puedes hacer clic en las consultas más comunes a continuación, o decirme qué contenido estás buscando.',
    fr: `D'accord, vous pouvez cliquer sur les requêtes les plus courantes ci-dessous, ou me dire quel contenu vous recherchez.`,
    de: 'Okay, Sie können auf die häufigsten Abfragen unten klicken oder mir sagen, wonach Sie suchen.',
    ja: 'わかりました。以下の最も一般的なクエリをクリックするか、お探しのコンテンツを教えてください。',
    ko: '알겠습니다. 가장 일반적인 쿼리를 아래에서 클릭하거나 찾으시는 콘텐츠를 말씀해주세요.',
    pt: 'Ok, você pode clicar nas consultas mais comuns abaixo ou me dizer que conteúdo está procurando.',
    it: 'Va bene, puoi cliccare sulle query più comuni qui sotto o dirmi quale contenuto stai cercando.',
    'zh-TW': '好的，您可以點擊下面這些最常見的查詢，或告訴我您要找什麼內容。',
    'zh-HK': '好嘢，您可以喺下面啲最常見嘅查詢入面揀一個，或者講俾我知您要搵咩嘢。',
    ru: 'Хорошо, вы можете нажать на наиболее распространенные запросы ниже или сообщить мне, какой контент вы ищете.'
  },
  'Find More': {
    zh: '您可以点击下面这些最为常见的查询，或者告诉我您想找什么内容。',
    en: 'You can click on these most common queries below, or tell me what content you are looking for.',
    es: 'Puede hacer clic en estas consultas más comunes a continuación, o decirme qué contenido está buscando.',
    fr: 'Vous pouvez cliquer sur ces requêtes les plus courantes ci-dessous, ou me dire quel contenu vous recherchez.',
    de: 'Sie können auf diese am häufigsten gestellten Abfragen unten klicken oder mir sagen, wonach Sie suchen.',
    ja: '以下の最も一般的なクエリをクリックするか、探しているコンテンツを教えてください。',
    ko: '가장 일반적인 쿼리 중 하나를 클릭하거나 찾으려는 콘텐츠를 알려주세요.',
    pt: 'Você pode clicar nas consultas mais comuns abaixo ou me dizer que conteúdo está procurando.',
    it: 'Puoi cliccare su queste query più comuni qui sotto, o dirmi quale contenuto stai cercando.',
    'zh-TW': '您可以點擊下面這些最常見的查詢，或告訴我您想要尋找什麼內容。',
    'zh-HK': '你可以按下面最常見的查詢，或者告訴我你想搵咩內容。',
    ru: 'Вы можете щелкнуть на эти самые распространенные запросы ниже или сообщить мне, какой контент вы ищете.'
  },
  'Discuss More': {
    zh: '如果有任何关于FT最新内容的问题，请立即问我。',
    en: 'If you have any questions about the latest FT content, please ask me immediately.',
    es: 'Si tiene alguna pregunta sobre el contenido más reciente de FT, pregúnteme de inmediato.',
    fr: 'Si vous avez des questions sur le contenu le plus récent de FT, demandez-moi immédiatement.',
    de: 'Wenn Sie Fragen zum neuesten FT-Inhalt haben, fragen Sie mich bitte sofort.',
    ja: '最新のFTコンテンツに関する質問がある場合は、すぐに私に尋ねてください。',
    ko: '최신 FT 콘텐츠에 대한 질문이 있으면 즉시 저에게 물어보세요.',
    pt: 'Se você tiver alguma dúvida sobre o conteúdo mais recente do FT, pergunte-me imediatamente.',
    it: 'Se hai domande sul contenuto più recente di FT, chiedimi immediatamente.',
    'zh-TW': '如果有關於FT最新內容的問題，請立即問我。',
    'zh-HK': '如果有關FT最新內容的問題，請立即問我。',
    ru: 'Если у вас есть вопросы о последних материалах FT, пожалуйста, спросите меня немедленно.'
  },
  'DoSomethingElse': {
    zh: '聊点别的',
    en: `Talk about something else`,
    es: 'Hablemos de algo más',
    fr: `Parlons d'autre chose`,
    de: 'Lass uns über etwas anderes sprechen',
    ja: '他の話題にしましょう',
    ko: '다른 얘기 좀 하자',
    pt: 'Vamos falar sobre outra coisa',
    it: `Parliamo d'altro`,
    'zh-TW': '聊點別的',
    'zh-HK': '傾下計啦',
    ru: 'Давайте поговорим о чем-то другом'
  },
  'Set Your Preferences': {
    zh: '您可以在这里修改您的偏好及设置',
    en: 'You can modify your preferences and settings here.',
    es: 'Puede modificar sus preferencias y configuraciones aquí.',
    fr: 'Vous pouvez modifier vos préférences et paramètres ici.',
    de: 'Sie können hier Ihre Einstellungen und Vorlieben ändern.',
    ja: 'ここで好みや設定を変更できます。',
    ko: '여기에서 선호도와 설정을 수정할 수 있습니다.',
    pt: 'Você pode modificar suas preferências e configurações aqui.',
    it: 'Puoi modificare le tue preferenze e impostazioni qui.',
    'zh-TW': '您可以在這裡修改您的偏好和設置。',
    'zh-HK': '您可以在這裡修改您的偏好和設置。',
    ru: 'Вы можете изменить свои предпочтения и настройки здесь.'
  },
  'Setting': {
    zh: '设置',
    en: 'Settings',
    es: 'Ajustes',
    fr: 'Paramètres',
    de: 'Einstellungen',
    ja: '設定',
    ko: '설정',
    pt: 'Configurações',
    it: 'Impostazioni',
    'zh-TW': '設定',
    'zh-HK': '設定',
    ru: 'Настройки'
  },
  'My Interests': {
    zh: '我的关注',
    en: 'My Interests',
    "es": "Mis Intereses",
    "fr": "Mes Intérêts",
    "de": "Meine Interessen",
    "ja": "私の興味",
    "ko": "내 관심사",
    "pt": "Meus Interesses",
    "it": "I Miei Interessi",
    "zh-TW": "我的興趣",
    "zh-HK": "我的興趣",
    "ru": "Мои Интересы"
  },
  'Add': {
    zh: '添加',
    en: 'Add', 
    es: 'Añadir',
    fr: 'Ajouter',
    de: 'Hinzufügen',
    ja: '追加',
    ko: '추가',
    pt: 'Adicionar',
    it: 'Aggiungi',
    'zh-TW': '新增',
    'zh-HK': '新增',
    ru: 'Добавить'
  },
  'Add Interests': {
    zh: '添加兴趣', 
    en: 'Add Interests',
    es: 'Añadir intereses',
    fr: 'Ajouter des intérêts', 
    de: 'Interessen hinzufügen',
    ja: '興味を追加',
    ko: '관심사 추가',
    pt: 'Adicionar interesses',
    it: 'Aggiungi interessi',
    'zh-TW': '新增興趣',
    'zh-HK': '新增興趣',
    ru: 'Добавить интересы'
  },
  'Popular': {
    zh: '热门',
    en: 'Popular', 
    es: 'Popular',
    fr: 'Populaire',
    de: 'Beliebt', 
    ja: '人気',
    ko: '인기',
    pt: 'Popular',
    it: 'Popolare',
    'zh-TW': '熱門',
    'zh-HK': '熱門',
    ru: 'Популярный'
  },
  'Language': {
    zh: '语言',
    en: 'Language',
    es: 'Idioma',
    fr: 'Langue',
    de: 'Sprache',
    ja: '言語',
    ko: '언어',
    pt: 'Língua',
    it: 'Lingua',
    'zh-TW': '語言',
    'zh-HK': '語言',
    ru: 'Язык'
  },
  'Font Size': {
    zh: '字体大小',
    en: 'Font Size',
    es: 'Tamaño de Fuente',
    fr: 'Taille de Police',
    de: 'Schriftgröße',
    ja: 'フォントサイズ',
    ko: '글꼴 크기',
    pt: 'Tamanho da Fonte',
    it: 'Dimensioni del Carattere',
    'zh-TW': '字型大小',
    'zh-HK': '字型大小',
    ru: 'Размер шрифта'
  },
  'Read Article': {
    zh: '阅读文章',
    en: 'Read Article',
    es: 'Leer artículo',
    fr: `Lire l'article`,
    de: 'Artikel lesen',
    ja: '記事を読む',
    ko: '기사 읽기',
    pt: 'Ler artigo',
    it: 'Leggi articolo',
    'zh-TW': '閱讀文章',
    'zh-HK': '閱讀文章',
    ru: 'Прочитать статью'
  },
  'Pop Out': {
    zh: '打开新页面',
    en: 'Pop Out',
    es: 'Abrir en nueva pestaña',
    fr: 'Ouvrir dans un nouvel onglet',
    de: 'In neuem Fenster öffnen',
    ja: '新しいウィンドウで開く',
    ko: '새 창에서 열기',
    pt: 'Abrir em nova janela',
    it: 'Apri in una nuova finestra',
    'zh-TW': '開新分頁',
    'zh-HK': '開新分頁',
    ru: 'Открыть в новом окне'
  },
  'In Chat': {
    zh: '在对话中',
    en: 'In Chat',
    es: 'En el chat',
    fr: 'Dans le chat',
    de: 'Im Chat',
    ja: 'チャット内',
    ko: '채팅에서',
    pt: 'No chat',
    it: 'In chat',
    'zh-TW': '在聊天中',
    'zh-HK': '在聊天中',
    ru: 'В чате'
  },
  'Please Select': {
    zh: '请选择',
    en: 'Please Select',
    es: 'Por favor, seleccione',
    fr: 'Veuillez sélectionner',
    de: 'Bitte auswählen',
    ja: '選択してください',
    ko: '선택하세요',
    pt: 'Por favor Selecione',
    it: 'Seleziona per favore',
    'zh-TW': '請選擇',
    'zh-HK': '請選擇',
    ru: 'Пожалуйста, выберите'
  },
  'Smallest': {
    zh: '最小',
    en: 'Smallest',
    es: 'Más Pequeño',
    fr: 'Le plus petit',
    de: 'Kleinsten',
    ja: '最小',
    ko: '가장 작은',
    pt: 'O menor',
    it: 'Il più piccolo',
    'zh-TW': '最小',
    'zh-HK': '最小',
    ru: 'Самый маленький'
  },
  'Smaller': {
    zh: '较小',
    en: 'Smaller',
    es: 'Menor',
    fr: 'Plus petit',
    de: 'Kleiner',
    ja: '小さい',
    ko: '작은',
    pt: 'Menor',
    it: 'Meno',
    'zh-TW': '較小',
    'zh-HK': '較小',
    ru: 'Меньше'
  },
  'Default': {
    zh: '默认',
    en: 'Default',
    es: 'Predeterminado',
    fr: 'Défaut',
    de: 'Standard',
    ja: 'デフォルト',
    ko: '기본값',
    pt: 'Padrão',
    it: 'Predefinito',
    'zh-TW': '預設',
    'zh-HK': '預設',
    ru: 'По умолчанию'
  },
  'Larger': {
    zh: '较大',
    en: 'Larger',
    es: 'Mayor',
    fr: 'Plus grand',
    de: 'Größer',
    ja: '大きい',
    ko: '더 큰',
    pt: 'Maior',
    it: 'Più grande',
    'zh-TW': '較大',
    'zh-HK': '較大',
    ru: 'Больше'
  },
  'Largest': {
    zh: '最大',
    en: 'Largest',
    es: 'Más Grande',
    fr: 'Le plus grand',
    de: 'Größten',
    ja: '最大',
    ko: '가장 큰',
    pt: 'O maior',
    it: 'Il più grande',
    'zh-TW': '最大',
    'zh-HK': '最大',
    ru: 'Самый большой'
  },
  'Sign In': {
    zh: '登录',
    en: 'Sign In',
    es: 'Iniciar sesión',
    fr: 'Se connecter',
    de: 'Anmelden',
    ja: 'ログイン',
    ko: '로그인',
    pt: 'Entrar',
    it: 'Accedi',
    'zh-TW': '登入',
    'zh-HK': '登入',
    ru: 'Вход'
  },
  'Sign Up': {
    zh: '注册',
    en: 'Sign Up',
    es: 'Registrarse',
    fr: 'S\'inscrire',
    de: 'Registrieren',
    ja: '新規登録',
    ko: '회원가입',
    pt: 'Registar-se',
    it: 'Registrati',
    'zh-TW': '註冊',
    'zh-HK': '註冊',
    ru: 'Регистрация'
  },
  "Translation Helper": {
    "Zh": "翻译助手",
    "en": "Translation Helper",
    "es": "Asistente de Traducción",
    "fr": "Assistant de Traduction",
    "de": "Übersetzungshilfe",
    "Ja": "翻訳ヘルパー",
    "ko": "번역 도우미",
    "pt": "Assistente de Tradução",
    "it": "Assistente di Traduzione",
    "zh-TW": "翻譯助手",
    "zh-HK": "翻譯助手",
    "ru": "Помощник перевода"
  },
  "loginOrSignUp": {
    "Zh": "请在购买前登录或注册！",
    "en": "Please login or sign up before making any purchase!",
    "es": "¡Inicia sesión o regístrate antes de realizar cualquier compra!",
    "fr": "Veuillez vous connecter ou vous inscrire avant d'effectuer un achat !",
    "de": "Bitte melden Sie sich an oder registrieren Sie sich, bevor Sie einen Kauf tätigen!",
    "Ja": "購入する前にログインまたはサインアップしてください！",
    "ko": "구매를 하기 전에 로그인 또는 회원가입을 해주세요!",
    "pt": "Por favor, faça login ou registre-se antes de fazer qualquer compra!",
    "it": "Effettua il login o registrati prima di effettuare un acquisto!",
    "zh-TW": "請在購買前登錄或註冊！",
    "zh-HK": "請在購買前登錄或註冊！",
    "ru": "Пожалуйста, войдите или зарегистрируйтесь перед покупкой!"
  },
  "checkoutFailedStatus": {
    "Zh": "无法创建结账会话。服务器响应状态为 [status]。",
    "en": "Failed to create checkout session. The server responded with [status].",
    "es": "No se pudo crear la sesión de pago. El servidor respondió con [status].",
    "fr": "Échec de la création de la session de paiement. Le serveur a répondu avec [status].",
    "de": "Fehler beim Erstellen der Checkout-Sitzung. Der Server antwortete mit [status].",
    "Ja": "チェックアウトセッションの作成に失敗しました。サーバーの応答は [status] です。",
    "ko": "결제 세션을 생성하지 못했습니다. 서버가 [status] 로 응답했습니다.",
    "pt": "Falha ao criar sessão de checkout. O servidor respondeu com [status].",
    "it": "Impossibile creare la sessione di pagamento. Il server ha risposto con [status].",
    "zh-TW": "無法創建結帳會話。伺服器回應狀態為 [status]。",
    "zh-HK": "無法創建結帳會話。伺服器回應狀態為 [status]。",
    "ru": "Не удалось создать сеанс оформления заказа. Сервер ответил со статусом [status]."
  },
  "promptUpgrade": {
    "zh": "请<a href=\"/subscription\">升级到[tierToBuy]订阅</a>以使用此功能。",
    "en": "Please <a href=\"/subscription\">upgrade to [tierToBuy] subscription</a> to access this feature.",
    "es": "Por favor, <a href=\"/subscription\">actualiza a la suscripción [tierToBuy]</a> para acceder a esta función.",
    "fr": "Veuillez <a href=\"/subscription\">passer à l'abonnement [tierToBuy]</a> pour accéder à cette fonctionnalité.",
    "de": "Bitte <a href=\"/subscription\">auf das [tierToBuy]-Abonnement upgraden</a>, um auf diese Funktion zuzugreifen.",
    "Ja": "この機能を利用するには、<a href=\"/subscription\">[tierToBuy] サブスクリプションにアップグレード</a>してください。",
    "ko": "이 기능을 사용하려면 <a href=\"/subscription\">[tierToBuy] 구독으로 업그레이드</a>하세요.",
    "pt": "Por favor, <a href=\"/subscription\">faça upgrade para a assinatura [tierToBuy]</a> para acessar este recurso.",
    "it": "Per favore, <a href=\"/subscription\">aggiorna all'abbonamento [tierToBuy]</a> per accedere a questa funzionalità.",
    "zh-TW": "請<a href=\"/subscription\">升級至[tierToBuy]訂閱</a>以使用此功能。",
    "zh-HK": "請<a href=\"/subscription\">升級至[tierToBuy]訂閱</a>以使用此功能。",
    "ru": "Пожалуйста, <a href=\"/subscription\">обновите подписку до [tierToBuy]</a>, чтобы получить доступ к этой функции."
  },
  "checkoutFailedTryAgain": {
    "Zh": "无法创建结账会话。请重试。",
    "en": "Failed to create checkout session. Please try again.",
    "es": "No se pudo crear la sesión de pago. Por favor, inténtalo de nuevo.",
    "fr": "Échec de la création de la session de paiement. Veuillez réessayer.",
    "de": "Fehler beim Erstellen der Checkout-Sitzung. Bitte versuchen Sie es erneut.",
    "Ja": "チェックアウトセッションの作成に失敗しました。もう一度お試しください。",
    "ko": "결제 세션을 생성하지 못했습니다. 다시 시도해 주세요.",
    "pt": "Falha ao criar sessão de checkout. Por favor, tente novamente.",
    "it": "Impossibile creare la sessione di pagamento. Per favore riprova.",
    "zh-TW": "無法創建結帳會話。請重試。",
    "zh-HK": "無法創建結帳會話。請重試。",
    "ru": "Не удалось создать сеанс оформления заказа. Пожалуйста, попробуйте еще раз."
  },
  'Agree and Sign Up': {
    zh: '同意并注册',
    en: 'Agree and Sign Up',
    es: 'Aceptar y Registrarse',
    fr: 'Accepter et S\'inscrire',
    de: 'Zustimmen und Registrieren',
    ja: '同意して新規登録',
    ko: '동의하고 회원가입',
    pt: 'Aceitar e Registar-se',
    it: 'Accetta e Registrati',
    'zh-TW': '同意並註冊',
    'zh-HK': '同意並註冊',
    ru: 'Согласиться и Зарегистрироваться'
  },
  'Please read and agree': {
    zh: '请您阅读我们的<a class="agreeDetail" href="/m/corp/service.html" target="_blank">用户注册协议</a>和<a class="agreeDetail" href="/m/corp/service.html#privacy" target="_blank">隐私权保护政策</a>，点击下方按钮即视为您接受。',
    en: 'Please read our <a class="agreeDetail" href="/m/corp/service.html" target="_blank">User Registration Agreement</a> and <a class="agreeDetail" href="/m/corp/service.html#privacy" target="_blank">Privacy Policy</a>. By clicking the button below, you agree to these terms.',
    es: 'Por favor lea nuestro <a class="agreeDetail" href="/m/corp/service.html" target="_blank">Acuerdo de Registro de Usuario</a> y la <a class="agreeDetail" href="/m/corp/service.html#privacy" target="_blank">Política de Privacidad</a>. Al hacer clic en el botón a continuación, acepta estos términos.',
    fr: 'Veuillez lire notre <a class="agreeDetail" href="/m/corp/service.html" target="_blank">Contrat d\'inscription</a> et notre <a class="agreeDetail" href="/m/corp/service.html#privacy" target="_blank">Politique de confidentialité</a>. En cliquant sur le bouton ci-dessous, vous acceptez ces termes.',
    de: 'Bitte lesen Sie unser <a class="agreeDetail" href="/m/corp/service.html" target="_blank">Nutzungsbedingungen</a> und unsere <a class="agreeDetail" href="/m/corp/service.html#privacy" target="_blank">Datenschutzrichtlinie</a>. Durch Klicken auf die Schaltfläche unten stimmen Sie diesen Bedingungen zu.',
    ja: '私たちの<a class="agreeDetail" href="/m/corp/service.html" target="_blank">ユーザー登録契約</a>および<a class="agreeDetail" href="/m/corp/service.html#privacy" target="_blank">プライバシーポリシー</a>をご確認ください。下のボタンをクリックすると、これらの条件に同意したことになります。',
    ko: '<a class="agreeDetail" href="/m/corp/service.html" target="_blank">사용자 등록 약관</a> 및 <a class="agreeDetail" href="/m/corp/service.html#privacy" target="_blank">개인정보 보호정책</a>을 읽어 주십시오. 아래 버튼을 클릭하면 이 약관에 동의한 것으로 간주됩니다.',
    pt: 'Por favor, leia o nosso <a class="agreeDetail" href="/m/corp/service.html" target="_blank">Acordo de Registro de Usuário</a> e a <a class="agreeDetail" href="/m/corp/service.html#privacy" target="_blank">Política de Privacidade</a>. Ao clicar no botão abaixo, você concorda com estes termos.',
    it: 'Si prega di leggere il nostro <a class="agreeDetail" href="/m/corp/service.html" target="_blank">Contratto di Registrazione</a> e la nostra <a class="agreeDetail" href="/m/corp/service.html#privacy" target="_blank">Politica sulla Privacy</a>. Facendo clic sul pulsante in basso, accetti questi termini.',
    'zh-TW': '請您閱讀我們的<a class="agreeDetail" href="/m/corp/service.html" target="_blank">用戶註冊協議</a>和<a class="agreeDetail" href="/m/corp/service.html#privacy" target="_blank">隱私權保護政策</a>，點擊下方按鈕即視為您接受。',
    'zh-HK': '請您閱讀我們的<a class="agreeDetail" href="/m/corp/service.html" target="_blank">用戶註冊協議</a>和<a class="agreeDetail" href="/m/corp/service.html#privacy" target="_blank">隱私權保護政策</a>，點擊下方按鈕即視為您接受。',
    ru: 'Пожалуйста, ознакомьтесь с нашим <a class="agreeDetail" href="/m/corp/service.html" target="_blank">Пользовательским Соглашением</a> и <a class="agreeDetail" href="/m/corp/service.html#privacy" target="_blank">Политикой Конфиденциальности</a>. Нажав кнопку ниже, вы соглашаетесь с этими условиями.'
  },
  "emailSubscriptions": {
    "zh": "电邮订阅",
    "en": "Email Subscriptions",
    "es": "Suscripciones por correo",
    "fr": "Abonnements par e-mail",
    "de": "E-Mail-Abonnements",
    "ja": "メール購読",
    "ko": "이메일 구독",
    "pt": "Assinaturas de e-mail",
    "it": "Abbonamenti e-mail",
    "ru": "Подписки по электронной почте"
  },
  "todayFocus": {
    "zh": "《今日焦点》邮件",
    "en": "Today's Focus",
    "es": "Enfoque de Hoy",
    "fr": "Focus du jour",
    "de": "Fokus des Tages",
    "ja": "今日の焦点",
    "ko": "오늘의 초점",
    "pt": "Foco de Hoje",
    "it": "Focus di Oggi",
    "ru": "Фокус дня"
  },
  "weeklySelection": {
    "zh": "《一周精选》邮件",
    "en": "Weekly Selection",
    "es": "Selección Semanal",
    "fr": "Sélection Hebdomadaire",
    "de": "Wöchentliche Auswahl",
    "ja": "今週のセレクション",
    "ko": "주간 선택",
    "pt": "Seleção Semanal",
    "it": "Selezione settimanale",
    "ru": "Еженедельная подборка"
  },
  "afternoonExpress": {
    "zh": "《午后速递》邮件",
    "en": "Afternoon Express",
    "es": "Entrega Vespertina",
    "fr": "Express de l'après-midi",
    "de": "Nachmittags-Express",
    "ja": "午後のエクスプレス",
    "ko": "오후 속보",
    "pt": "Expresso da Tarde",
    "it": "Espresso pomeridiano",
    "ru": "Дневной экспресс"
  },
  "privacySettings": {
    "zh": "隐私设置",
    "en": "Privacy Settings",
    "es": "Configuraciones de privacidad",
    "fr": "Paramètres de confidentialité",
    "de": "Datenschutzeinstellungen",
    "ja": "プライバシー設定",
    "ko": "개인정보 설정",
    "pt": "Configurações de Privacidade",
    "it": "Impostazioni della privacy",
    "ru": "Настройки конфиденциальности"
  },
  "ftNotifications": {
    "zh": "我愿意接收FT中文网新服务和产品的通知",
    "en": "I agree to receive new services and products",
    "es": "Acepto recibir nuevos servicios y productos",
    "fr": "J'accepte de recevoir de nouveaux services et produits",
    "de": "Ich stimme zu, neue Dienste und Produkte zu erhalten",
    "ja": "新サービスや製品の通知を受け取ることに同意します",
    "ko": "새로운 서비스 및 제품 알림 수신에 동의합니다",
    "pt": "Concordo em receber novos serviços e produtos",
    "it": "Accetto di ricevere nuovi servizi e prodotti",
    "ru": "Я согласен получать новые услуги и продукты"
  },
  "thirdPartyEmails": {
    "zh": "FT中文网及其谨慎选择的第三方公司可以向我发送电子邮件",
    "en": "I agree to receive emails from third parties carefully selected by FT",
    "es": "Acepto recibir correos electrónicos de terceros cuidadosamente seleccionados por FT",
    "fr": "J'accepte de recevoir des e-mails de tiers soigneusement sélectionnés par FT",
    "de": "Ich stimme zu, E-Mails von sorgfältig ausgewählten Dritten zu erhalten",
    "ja": "FTが慎重に選んだ第三者からのメールを受け取ることに同意します",
    "ko": "FT에서 신중하게 선택한 제3자로부터 이메일 수신에 동의합니다",
    "pt": "Concordo em receber e-mails de terceiros selecionados cuidadosamente pela FT",
    "it": "Accetto di ricevere e-mail da terze parti attentamente selezionate da FT",
    "ru": "Я согласен получать электронные письма от третьих лиц, тщательно отобранных FT"
  },
  "thirdPartyMailing": {
    "zh": "FT中文网及其谨慎选择的第三方公司可以向我寄发信件",
    "en": "I agree to receive postal mail from third parties carefully selected by FT",
    "es": "Acepto recibir correo postal de terceros cuidadosamente seleccionados por FT",
    "fr": "J'accepte de recevoir du courrier postal de tiers soigneusement sélectionnés par FT",
    "de": "Ich stimme zu, Post von sorgfältig ausgewählten Dritten zu erhalten",
    "ja": "FTが慎重に選んだ第三者からの郵送を受け取ることに同意します",
    "ko": "FT에서 신중하게 선택한 제3자로부터 우편물 수신에 동의합니다",
    "pt": "Concordo em receber correspondência de terceiros selecionados cuidadosamente pela FT",
    "it": "Accetto di ricevere posta da terze parti attentamente selezionate da FT",
    "ru": "Я согласен получать почтовую корреспонденцию от третьих лиц, тщательно отобранных FT"
  },
  "termsAndAgreement": {
    "zh": "请您阅读并同意我们的用户注册协议和隐私权保护政策，点击下方按钮即视为您接受。",
    "en": "Please read and agree to our User Registration Agreement and Privacy Policy. By clicking the button below, you accept these terms.",
    "es": "Por favor, lea y acepte nuestro Acuerdo de Registro de Usuario y Política de Privacidad. Al hacer clic en el botón a continuación, acepta estos términos.",
    "fr": "Veuillez lire et accepter notre Contrat d'inscription et Politique de Confidentialité. En cliquant sur le bouton ci-dessous, vous acceptez ces termes.",
    "de": "Bitte lesen und akzeptieren Sie unsere Nutzungsbedingungen und Datenschutzrichtlinie. Durch Klicken auf die Schaltfläche unten stimmen Sie diesen Bedingungen zu.",
    "ja": "ユーザー登録契約とプライバシーポリシーを読んで同意してください。下のボタンをクリックすると、これらの条件に同意したことになります。",
    "ko": "사용자 등록 약관 및 개인정보 보호정책을 읽고 동의해 주세요. 아래 버튼을 클릭하면 이 약관에 동의한 것으로 간주됩니다.",
    "pt": "Leia e concorde com o nosso Acordo de Registro de Usuário e Política de Privacidade. Ao clicar no botão abaixo, você aceita estes termos.",
    "it": "Leggi e accetta il nostro Contratto di Registrazione Utente e Politica sulla Privacy. Facendo clic sul pulsante in basso, accetti questi termini.",
    "ru": "Пожалуйста, ознакомьтесь с нашим Пользовательским Соглашением и Политикой Конфиденциальности. Нажимая кнопку ниже, вы принимаете эти условия."
  },
  'Sign Out': {
    zh: '退出',
    en: 'Sign Out',
    es: 'Cerrar sesión',
    fr: 'Se déconnecter',
    de: 'Abmelden',
    ja: 'ログアウト',
    ko: '로그아웃',
    pt: 'Sair',
    it: 'Esci',
    'zh-TW': '登出',
    'zh-HK': '登出',
    ru: 'Выход'
  },
  'Article Translation Preference': {
    zh: '文章翻译偏好',
    en: 'Article Translation Preference',
    es: 'Preferencia de Traducción de Artículo',
    fr: 'Préférence de Traduction d\'Article',
    de: 'Artikel Übersetzungsvorlieben',
    ja: '記事の翻訳の好み',
    ko: '기사 번역 선호',
    pt: 'Preferência de Tradução de Artigo',
    it: 'Preferenza di Traduzione dell\'Articolo',
    'zh-TW': '文章翻譯偏好',
    'zh-HK': '文章翻譯偏好',
    ru: 'Предпочтения Перевода Статьи'
  },
  'Both Human and Machine Translation': {
    zh: '机器翻译及人工翻译',
    en: 'Machine and Human Translation',
    es: 'Traducción Automática y Humana',
    fr: 'Traduction Automatique et Humaine',
    de: 'Maschinen- und Menschliche Übersetzung',
    ja: '機械翻訳および人間の翻訳',
    ko: '기계 번역 및 인간 번역',
    pt: 'Tradução por Máquina e Humana',
    it: 'Traduzione Automatica e Umana',
    'zh-TW': '機器翻譯及人工翻譯',
    'zh-HK': '機器翻譯及人工翻譯',
    ru: 'Машинный и Человеческий Перевод'
  },
  'Only Use Human Translation': {
    zh: '仅人工翻译',
    en: 'Only Use Human Translation',
    es: 'Usar Solo Traducción Humana',
    fr: 'Utiliser Uniquement la Traduction Humaine',
    de: 'Nur Menschliche Übersetzung Verwenden',
    ja: '人間の翻訳のみを使用',
    ko: '인간 번역만 사용',
    pt: 'Usar Apenas Tradução Humana',
    it: 'Usa Solo la Traduzione Umana',
    'zh-TW': '僅人工翻譯',
    'zh-HK': '僅人工翻譯',
    ru: 'Использовать Только Человеческий Перевод'
  },
  'Prompt Set Intention': {
    zh: '请先点击上面的按钮，选择您需要的服务',
    en: 'Please click the button above and select the service you need.',
    es: 'Por favor, haga clic en el botón de arriba y seleccione el servicio que necesita.',
    fr: 'Veuillez cliquer sur le bouton ci-dessus et sélectionner le service dont vous avez besoin.',
    de: 'Bitte klicken Sie auf die Schaltfläche oben und wählen Sie den gewünschten Service aus.',
    ja: '上のボタンをクリックして、必要なサービスを選択してください。',
    ko: '위의 버튼을 클릭하여 필요한 서비스를 선택하십시오.',
    pt: 'Por favor, clique no botão acima e selecione o serviço que você precisa.',
    it: 'Fare clic sul pulsante sopra e selezionare il servizio di cui avete bisogno.',
    'zh-TW': '請先點擊上面的按鈕，選擇您需要的服務',
    'zh-HK': '請先按上面的按鈕，選擇您需要的服務',
    ru: 'Пожалуйста, нажмите на кнопку выше и выберите нужный сервис.'
  },
  'Read_It_Later':{
    zh: '稍后再看',
    en: 'Read Later',
    es: 'Leer más tarde',
    fr: 'Lire plus tard',
    de: 'Für später merken',
    ja: '後で読む',
    ko: '나중에 읽기',
    pt: 'Ler depois',
    it: 'Leggi dopo',
    'zh-TW': '稍後再看',
    'zh-HK': '稍後再看',
    ru: 'Прочитать позже'
  },
  'Read_It_Later_Flag':{
    zh: '文章已在下方加载完毕。',
    en: 'Content has been loaded below.',
    es: 'El contenido se ha cargado a continuación.',
    fr: 'Le contenu a été chargé ci-dessous.',
    de: 'Der Inhalt wurde unten geladen.',
    ja: 'コンテンツは下に読み込まれました。',
    ko: '콘텐츠가 아래에로드되었습니다.',
    pt: 'O conteúdo foi carregado abaixo.',
    it: 'Il contenuto è stato caricato sotto.',
    'zh-TW': '內容已在下方加載。',
    'zh-HK': '內容已在下方加載。',
    ru: 'Контент загружен ниже.'
    },
    'jump-to-article':{
      zh: '跳转到文章',
      en: 'Jump to Article',
      es: 'Ir al Artículo',
      fr: 'Aller à l\'article',
      de: 'Zum Artikel springen',
      ja: '記事へ移動',
      ko: '기사로 이동',
      pt: 'Ir para o Artigo',
      it: 'Vai all\'Articolo',
      'zh-TW': '跳轉到文章',
      'zh-HK': '跳轉到文章',
      ru: 'Перейти к статье'
    },
    'FT Daily Quiz': {
        zh: 'FT每日小测',
        en: 'FT Daily Quiz',
        es: 'FT Cuestionario Diario',
        fr: 'FT Quiz Quotidien',
        de: 'FT Tägliches Quiz',
        Ja: 'FT毎日のクイズ',
        ko: 'FT 일일 퀴즈',
        pt: 'FT Quiz Diário',
        it: 'FT Quiz Giornaliero',
        'zh-TW': 'FT每日小測',
        'zh-HK': 'FT每日小測',
        ru: 'FT Ежедневная Викторина'
    },
    'Quiz Description': {
        zh: '提醒完成每日FT小测。',
        en: 'Reminder to complete your daily FT quiz.',
        es: 'Recordatorio para completar su cuestionario diario de FT.',
        fr: 'Rappel de compléter votre quiz quotidien de FT.',
        de: 'Erinnerung, Ihr tägliches FT-Quiz zu absolvieren.',
        Ja: '毎日のFTクイズを完了するリマインダー。',
        ko: '매일 FT 퀴즈를 완료하라는 알림입니다.',
        pt: 'Lembrete para completar seu quiz diário de FT.',
        it: 'Promemoria per completare il tuo quiz giornaliero di FT.',
        'zh-TW': '提醒完成每日FT小測。',
        'zh-HK': '提醒完成每日FT小測。',
        ru: 'Напоминание завершить вашу ежедневную викторину FT.'
    },
    'Quiz Prompt': {
      zh: '将每日小测提醒添加到日历，以便明天同一时间得到提醒！',
      en: 'Add a reminder to your calendar to get notified at the same time tomorrow!',
      es: '¡Agrega un recordatorio a tu calendario para recibir un recordatorio a la misma hora mañana!',
      fr: 'Ajoutez un rappel à votre calendrier pour être rappelé à la même heure demain!',
      de: 'Fügen Sie eine Erinnerung zu Ihrem Kalender hinzu, um morgen zur gleichen Zeit erinnert zu werden!',
      Ja: 'カレンダーにリマインダーを追加して、明日の同じ時間に通知を受け取りましょう!',
      ko: '내일 같은 시간에 알림을 받기 위해 캘린더에 알림을 추가하세요!',
      pt: 'Adicione um lembrete ao seu calendário para ser lembrado na mesma hora amanhã!',
      it: 'Aggiungi un promemoria al tuo calendario per essere ricordato alla stessa ora domani!',
      'zh-TW': '將每日小測提醒添加到日曆，以便明天同一時間收到通知！',
      'zh-HK': '將每日小測提醒添加到日曆，以便明天同一時間收到通知！',
      ru: 'Добавьте напоминание в календарь, чтобы получить уведомление в это же время завтра!'
    },
    'Bilingual': {
      zh: '对照',
      en: 'Bilingual',
      es: 'Bilingüe',
      fr: 'Bilingue',
      
      de: 'Zweisprachig',
      ja: 'バイリンガル',
      ko: '이중 언어',
      pt: 'Bilíngue',
      it: 'Bilingue',
      'zh-TW': '雙語',
      'zh-HK': '雙語',
      ru: 'Двуязычный'
    },
    'ai-disclaimer-proofread': {
      zh: '为了第一时间为您呈现此信息，本文内容为AI翻译和校对，仅供参考',
      en: 'To present this information to you as soon as possible, the content of this article is AI translated and proofread, for reference only',
      es: 'Para presentar esta información lo antes posible, el contenido de este artículo está traducido y revisado por IA, solo para referencia',
      fr: `Pour présenter cette information le plus rapidement possible, le contenu de cet article est traduit et relu par l'IA, uniquement à titre de référence`,
      de: 'Um Ihnen diese Informationen so schnell wie möglich zu präsentieren, ist der Inhalt dieses Artikels von AI übersetzt und korrekturgelesen, nur zur Referenz',
      ja: 'この情報をできるだけ早く提供するため、この記事の内容はAIによる翻訳と校正であり、参考のみです',
      ko: '최대한 빠르게 이 정보를 제공하기 위해이 문서의 내용은 AI 번역 및 교정으로 작성되었습니다. 참조 용도로만 사용하십시오',
      pt: 'Para apresentar esta informação o mais rapidamente possível, o conteúdo deste artigo é traduzido e revisado por AI, apenas para referência',
      it: `Per presentare queste informazioni il prima possibile, il contenuto di questo articolo è tradotto e revisionato dall'IA, solo per riferimento`,
      'zh-TW': '為了第一時間為您呈現此信息，本文內容為AI翻譯和校對，僅供參考',
      'zh-HK': '為了第一時間為您呈現此信息，本文內容為AI翻譯和校對，僅供參考',
      ru: 'Чтобы представить вам эту информацию как можно скорее, содержание этой статьи переведено и проверено на ИИ, только для справки'
    },
    'ai-disclaimer': {
      zh: '为了第一时间为您呈现此信息，本文内容为机器翻译，仅供参考',
      en: 'To present this information to you as quickly as possible, this article has been machine translated and is for reference only',
      es: 'Para presentarle esta información lo más rápido posible, este artículo ha sido traducido por máquina y es solo para referencia',
      fr: 'Pour vous présenter ces informations le plus rapidement possible, cet article a été traduit par machine et est uniquement à titre de référence',
      de: 'Um Ihnen diese Informationen so schnell wie möglich zu präsentieren, wurde dieser Artikel maschinell übersetzt und dient nur als Referenz',
      ja: 'この情報をできるだけ早く提供するため、この記事は機械翻訳され、参考のみとして提供されています',
      ko: '이 정보를 가능한 빠르게 제공하기 위해 이 기사는 기계 번역되었으며 참조만을 위한 것입니다',
      pt: 'Para apresentar esta informação o mais rápido possível, este artigo foi traduzido por máquina e é apenas para referência',
      it: 'Per presentare queste informazioni il più rapidamente possibile, questo articolo è stato tradotto con una macchina ed è solo a scopo di riferimento',
      'zh-TW': '為了第一時間為您呈現此資訊，本文內容為機器翻譯，僅供參考',
      'zh-HK': '為了第一時間為您呈現此資訊，本文內容為機器翻譯，僅供參考',
      ru: 'Чтобы предоставить вам эту информацию как можно быстрее, этот текст был переведен с помощью машины и предназначен только для справки'
    }, 
    NewsQuiz: {
      zh: '每日小测',
      en: 'News Quiz',
      es: 'Prueba de noticias',
      fr: `Quiz d'actualités`,
      de: 'Nachrichtenquiz',
      ja: 'ニュースクイズ',
      ko: '뉴스 퀴즈',
      pt: 'Quiz de notícias',
      it: 'Quiz di notizie',
      'zh-TW': '每日小測',
      'zh-HK': '每日小測',
      ru: 'Новостная викторина'
    }, 
    ChatDisclaimer: {
      "en": "Chat bots can make mistakes. Consider checking important information.",
      "zh": "聊天机器人可能会犯错误。请考虑核对重要信息。",
      "es": "Los chatbots pueden cometer errores. Considere verificar la información importante.",
      "fr": "Les chatbots peuvent faire des erreurs. Envisagez de vérifier les informations importantes.",
      "de": "Chatbots können Fehler machen. Überlegen Sie, wichtige Informationen zu überprüfen.",
      "ja": "チャットボットは間違いを犯すことがあります。重要な情報を確認することを検討してください。",
      "ko": "챗봇은 실수를 할 수 있습니다. 중요한 정보를 확인하는 것을 고려해보세요.",
      "pt": "Chatbots podem cometer erros. Considere verificar informações importantes.",
      "it": "I chatbot possono commettere errori. Considera di controllare le informazioni importanti.",
      "zh-TW": "聊天機器人可能會犯錯誤。請考慮核對重要資訊。",
      "zh-HK": "聊天機械人可能會犯錯。請考慮核對重要資訊。",
      "ru": "Чат-боты могут допускать ошибки. Рассмотрите возможность проверки важной информации."
    },
    AccountSharingReminder: {
      en: 'Your session has been securely ended on this device because your account was accessed from a new device. You can sign in on up to three different types of devices (mobile, tablet, and desktop) at a time. Please log in again if this was not you.',
      zh: '由于您的帐户已在新设备上访问，您在此设备上的会话已安全结束。您可以同时在三种不同类型的设备（手机、平板和桌面）上登录。如果这不是您的操作，请重新登录。',
      es: 'Tu sesión ha sido cerrada de forma segura en este dispositivo porque tu cuenta fue accedida desde un nuevo dispositivo. Puedes iniciar sesión en hasta tres tipos diferentes de dispositivos (móvil, tableta y escritorio) al mismo tiempo. Por favor, inicia sesión de nuevo si no fuiste tú.',
      fr: 'Votre session a été sécurisée et terminée sur cet appareil car votre compte a été accédé depuis un nouvel appareil. Vous pouvez vous connecter sur jusqu\'à trois types d\'appareils différents (mobile, tablette et ordinateur) en même temps. Veuillez vous reconnecter si ce n\'était pas vous.',
      de: 'Ihre Sitzung wurde auf diesem Gerät sicher beendet, da auf Ihr Konto von einem neuen Gerät zugegriffen wurde. Sie können sich gleichzeitig auf bis zu drei verschiedenen Gerätetypen (Mobil, Tablet und Desktop) anmelden. Bitte melden Sie sich erneut an, wenn Sie dies nicht waren.',
      ja: 'あなたのアカウントが新しいデバイスからアクセスされたため、このデバイスでのセッションは安全に終了しました。一度に最大3種類のデバイス（モバイル、タブレット、デスクトップ）でサインインできます。あなたの操作でない場合は、もう一度ログインしてください。',
      ko: '귀하의 계정이 새로운 기기에서 접근되었기 때문에 이 기기에서의 세션이 안전하게 종료되었습니다. 한 번에 최대 세 가지 유형의 기기(모바일, 태블릿, 데스크톱)에 로그인할 수 있습니다. 본인이 아닌 경우 다시 로그인해 주세요.',
      pt: 'Sua sessão foi encerrada de forma segura neste dispositivo porque sua conta foi acessada por um novo dispositivo. Você pode se conectar em até três tipos diferentes de dispositivos (móvel, tablet e desktop) ao mesmo tempo. Por favor, faça login novamente se não foi você.',
      it: 'La tua sessione è stata terminata in modo sicuro su questo dispositivo perché il tuo account è stato accesso da un nuovo dispositivo. Puoi accedere su fino a tre tipi diversi di dispositivi (mobile, tablet e desktop) contemporaneamente. Per favore, accedi nuovamente se non sei stato tu.',
      "zh-TW": '由於您的帳戶已在新設備上訪問，您在此設備上的會話已安全結束。您可以同時在三種不同類型的設備（手機、平板和桌面）上登錄。如果這不是您的操作，請重新登錄。',
      "zh-HK": '由於您的帳戶已在新設備上訪問，您在此設備上的會話已安全結束。您可以同時在三種不同類型的設備（手機、平板和桌面）上登錄。如果這不是您的操作，請重新登錄。',
      "ru": 'Ваш сеанс на этом устройстве был безопасно завершен, поскольку доступ к вашей учетной записи был осуществлен с нового устройства. Вы можете войти сразу на трех разных типах устройств (мобильный, планшет и настольный компьютер). Пожалуйста, войдите снова, если это были не вы.'
    },
    pleaseWait: {
      "zh": "请稍候...",
      "en": "Please wait for a while...",
      "es": "Por favor espera un momento...",
      "fr": "Veuillez patienter un moment...",
      "de": "Bitte warten Sie einen Moment...",
      "ja": "しばらくお待ちください...",
      "ko": "잠시만 기다려 주세요...",
      "pt": "Por favor, espere um momento...",
      "it": "Per favore, attendi un momento...",
      "zh-TW": "請稍候...",
      "zh-HK": "請稍候...",
      "ru": "Пожалуйста, подождите немного..."
    },
    sixDigitCodeFromEmail: {
      "zh": "来自您邮箱的6位数验证码",
      "en": "6-digit code from your email",
      "es": "Código de 6 dígitos de tu correo electrónico",
      "fr": "Code à 6 chiffres de votre e-mail",
      "de": "6-stelliger Code aus Ihrer E-Mail",
      "ja": "メールからの6桁のコード",
      "ko": "이메일에서 받은 6자리 코드",
      "pt": "Código de 6 dígitos do seu e-mail",
      "it": "Codice a 6 cifre dalla tua email",
      "zh-TW": "來自您郵箱的6位數驗證碼",
      "zh-HK": "來自您郵箱的6位數驗證碼",
      "ru": "6-значный код из вашего письма"
    },
    yourName: {
        "zh": "您的姓名",
        "en": "Your name",
        "es": "Tu nombre",
        "fr": "Votre nom",
        "de": "Ihr Name",
        "ja": "あなたの名前",
        "ko": "당신의 이름",
        "pt": "Seu nome",
        "it": "Il tuo nome",
        "zh-TW": "您的姓名",
        "zh-HK": "您的姓名",
        "ru": "Ваше имя"
    },
    setSecurePassword: {
        "zh": "设置安全密码：8个以上字符，包含大写、小写、数字和特殊字符",
        "en": "Set a secure password: 8+ chars, uppercase, lowercase, number, and special character",
        "es": "Establezca una contraseña segura: 8+ caracteres, mayúsculas, minúsculas, número y carácter especial",
        "fr": "Définissez un mot de passe sécurisé : 8+ caractères, majuscules, minuscules, chiffre et caractère spécial",
        "de": "Erstellen Sie ein sicheres Passwort: 8+ Zeichen, Großbuchstaben, Kleinbuchstaben, Zahl und Sonderzeichen",
        "ja": "セキュリティの高いパスワードを設定してください：8文字以上、大文字、小文字、数字、特殊文字",
        "ko": "보안 비밀번호 설정: 8자 이상, 대문자, 소문자, 숫자 및 특수 문자",
        "pt": "Defina uma senha segura: 8+ caracteres, maiúsculas, minúsculas, número e caractere especial",
        "it": "Imposta una password sicura: 8+ caratteri, maiuscole, minuscole, numero e carattere speciale",
        "zh-TW": "設置安全密碼：8個以上字符，包含大寫、小寫、數字和特殊字符",
        "zh-HK": "設置安全密碼：8個以上字符，包含大寫、小寫、數字和特殊字符",
        "ru": "Установите надежный пароль: более 8 символов, заглавные и строчные буквы, цифра и специальный символ"
    },
    passwordRuleLength: {
      "zh": "8个以上字符",
      "en": "8+ characters",
      "es": "8+ caracteres",
      "fr": "8+ caractères",
      "de": "8+ Zeichen",
      "ja": "8文字以上",
      "ko": "8자 이상",
      "pt": "8+ caracteres",
      "it": "8+ caratteri",
      "zh-TW": "8個以上字符",
      "zh-HK": "8個以上字符",
      "ru": "более 8 символов"
    },
    passwordRuleUppercase: {
        "zh": "包含大写",
        "en": "Include uppercase",
        "es": "Incluir mayúsculas",
        "fr": "Inclure des majuscules",
        "de": "Großbuchstaben einschließen",
        "ja": "大文字を含む",
        "ko": "대문자 포함",
        "pt": "Incluir maiúsculas",
        "it": "Includi maiuscole",
        "zh-TW": "包含大寫",
        "zh-HK": "包含大寫",
        "ru": "Включать заглавные буквы"
    },
    passwordRuleLowercase: {
        "zh": "包含小写",
        "en": "Include lowercase",
        "es": "Incluir minúsculas",
        "fr": "Inclure des minuscules",
        "de": "Kleinbuchstaben einschließen",
        "ja": "小文字を含む",
        "ko": "소문자 포함",
        "pt": "Incluir minúsculas",
        "it": "Includi minuscole",
        "zh-TW": "包含小寫",
        "zh-HK": "包含小寫",
        "ru": "Включать строчные буквы"
    },
    passwordRuleNumber: {
        "zh": "包含数字",
        "en": "Include a number",
        "es": "Incluir un número",
        "fr": "Inclure un chiffre",
        "de": "Eine Zahl einschließen",
        "ja": "数字を含む",
        "ko": "숫자 포함",
        "pt": "Incluir um número",
        "it": "Includi un numero",
        "zh-TW": "包含數字",
        "zh-HK": "包含數字",
        "ru": "Включать цифру"
    },
    passwordMustContain: {
      "zh": "密码必须包含",
      "en": "Password must contain",
      "es": "La contraseña debe contener",
      "fr": "Le mot de passe doit contenir",
      "de": "Passwort muss enthalten",
      "ja": "パスワードには以下が含まれている必要があります",
      "ko": "비밀번호에는 다음이 포함되어야 합니다",
      "pt": "A senha deve conter",
      "it": "La password deve contenere",
      "zh-TW": "密碼必須包含",
      "zh-HK": "密碼必須包含",
      "ru": "Пароль должен содержать"
    },
    passwordRuleSpecialCharacter: {
      "zh": "包含至少一个特殊字符，例如: !@#$%^&*",
      "en": "Include at least one special character, such as: !@#$%^&*",
      "es": "Incluye al menos un carácter especial, como: !@#$%^&*",
      "fr": "Inclure au moins un caractère spécial, tel que : !@#$%^&*",
      "de": "Schließen Sie mindestens ein Sonderzeichen ein, z.B.: !@#$%^&*",
      "ja": "少なくとも1つの特殊文字を含めてください。例：!@#$%^&*",
      "ko": "다음과 같은 특수 문자를 하나 이상 포함하십시오: !@#$%^&*",
      "pt": "Inclua pelo menos um caractere especial, como: !@#$%^&*",
      "it": "Includi almeno un carattere speciale, come: !@#$%^&*",
      "zh-TW": "包含至少一個特殊字符，例如: !@#$%^&*",
      "zh-HK": "包含至少一個特殊字符，例如: !@#$%^&*",
      "ru": "Включите как минимум один специальный символ, например: !@#$%^&*"
    },
    verificationCode: {
        "zh": "验证码",
        "en": "Verification Code",
        "es": "Código de verificación",
        "fr": "Code de vérification",
        "de": "Bestätigungscode",
        "ja": "確認コード",
        "ko": "인증 코드",
        "pt": "Código de verificação",
        "it": "Codice di verifica",
        "zh-TW": "驗證碼",
        "zh-HK": "驗證碼",
        "ru": "Код подтверждения"
    },
    name: {
        "zh": "姓名",
        "en": "Name",
        "es": "Nombre",
        "fr": "Nom",
        "de": "Name",
        "ja": "名前",
        "ko": "이름",
        "pt": "Nome",
        "it": "Nome",
        "zh-TW": "姓名",
        "zh-HK": "姓名",
        "ru": "Имя"
    },
    password: {
      "zh": "密码",
      "en": "Password",
      "es": "Contraseña",
      "fr": "Mot de passe",
      "de": "Passwort",
      "ja": "パスワード",
      "ko": "비밀번호",
      "pt": "Senha",
      "it": "Password",
      "zh-TW": "密碼",
      "zh-HK": "密碼",
      "ru": "Пароль"
    },
    resetPassword: {
        "zh": "重置密码",
        "en": "Reset Password",
        "es": "Restablecer la contraseña",
        "fr": "Réinitialiser le mot de passe",
        "de": "Passwort zurücksetzen",
        "ja": "パスワードを再設定する",
        "ko": "비밀번호 재설정",
        "pt": "Redefinir senha",
        "it": "Reimposta la password",
        "zh-TW": "重置密碼",
        "zh-HK": "重置密碼",
        "ru": "Сбросить пароль"
    },
    youAreNotSignedIn: {
        "zh": "您尚未登录！",
        "en": "You are not signed in!",
        "es": "¡No has iniciado sesión!",
        "fr": "Vous n'êtes pas connecté !",
        "de": "Sie sind nicht angemeldet!",
        "ja": "サインインしていません！",
        "ko": "로그인되지 않았습니다!",
        "pt": "Você não está conectado!",
        "it": "Non sei connesso!",
        "zh-TW": "您尚未登入！",
        "zh-HK": "您尚未登入！",
        "ru": "Вы не вошли в систему!"
    },
    youAreNotSignedInOrLoggedOut: {
        "zh": "您尚未登录或已被登出！",
        "en": "You are not signed in or have been logged out!",
        "es": "¡No has iniciado sesión o has sido desconectado!",
        "fr": "Vous n'êtes pas connecté ou vous avez été déconnecté !",
        "de": "Sie sind nicht angemeldet oder wurden abgemeldet!",
        "ja": "サインインしていないか、ログアウトしました！",
        "ko": "로그인되지 않았거나 로그아웃되었습니다!",
        "pt": "Você não está conectado ou foi desconectado!",
        "it": "Non sei connesso o sei stato disconnesso!",
        "zh-TW": "您尚未登入或已被登出！",
        "zh-HK": "您尚未登入或已被登出！",
        "ru": "Вы не вошли в систему или были выведены!"
    },
    newPassword: {
      "zh": "新密码",
      "en": "New Password",
      "es": "Nueva contraseña",
      "fr": "Nouveau mot de passe",
      "de": "Neues Passwort",
      "ja": "新しいパスワード",
      "ko": "새 비밀번호",
      "pt": "Nova senha",
      "it": "Nuova password",
      "zh-TW": "新密碼",
      "zh-HK": "新密碼",
      "ru": "Новый пароль"
    },
    pleaseLoginOrSignUp: {
        "zh": "请登录或注册以访问此页面。",
        "en": "Please login or sign up to access this page.",
        "es": "Por favor, inicia sesión o regístrate para acceder a esta página.",
        "fr": "Veuillez vous connecter ou vous inscrire pour accéder à cette page.",
        "de": "Bitte melden Sie sich an oder registrieren Sie sich, um auf diese Seite zuzugreifen.",
        "ja": "このページにアクセスするには、ログインまたはサインアップしてください。",
        "ko": "이 페이지에 액세스하려면 로그인하거나 가입하세요.",
        "pt": "Por favor, faça login ou inscreva-se para acessar esta página.",
        "it": "Per favore, accedi o registrati per accedere a questa pagina.",
        "zh-TW": "請登入或註冊以訪問此頁面。",
        "zh-HK": "請登入或註冊以訪問此頁面。",
        "ru": "Пожалуйста, войдите или зарегистрируйтесь, чтобы получить доступ к этой странице."
    },
    confirm: {
      "en": "Confirm",
      "zh": "确认",
      "es": "Confirmar",
      "fr": "Confirmer",
      "de": "Bestätigen",
      "ja": "確認",
      "ko": "확인",
      "pt": "Confirmar",
      "it": "Confermare",
      "zh-TW": "確認",
      "zh-HK": "確認",
      "ru": "Подтвердить"
  }
  
};

// MARK: - Normalize the statusDict to make it case-insensitive for lookup
let statusDict = {};

for (const key of Object.keys(statusDictOriginal)) {
  const newKey = key.toLowerCase();
  statusDict[newKey] = statusDictOriginal[key];
}
statusDictOriginal = {};

/* jshint ignore:end */